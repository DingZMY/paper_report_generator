# 系统/合成生物学文献周报 2026-W19

> **时间范围**：2026/04/27 ~ 2026/05/04
> **来源期刊**：Nature / Cell / Science 系列
> **收录论文**：35 篇（已翻译 35 篇）
> **生成时间**：2026-05-06 07:44 UTC

---



## 1. Cytoplasmic abundant heat-soluble proteins from tardigrades protect synthetic cells under stress.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-May-02 |
| **作者** | Xi Yongkang, Mao Jianming, Chen Samuel J, Moghimianavval Hossein, Lee Young Jin, Panda Ayush, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-72328-5) |
| **DOI** | `10.1038/s41467-026-72328-5` |
...
细胞质丰富热溶性（CAHS）蛋白是缓步动物中一种应激响应的固有无序蛋白，已被发现能形成凝胶状网络，在脱水过程中提供结构支撑，从而实现隐生。然而，CAHS蛋白保护脱水细胞膜的机制仍不清楚。利用巨型单层囊泡（GUVs）作为模型膜系统，我们在此表明，包封的CAHS12经历可逆的结构转变，增强了膜完整性并保留了包裹成分，模拟了自然隐生过程。含CAHS12的GUV在脱水、高温和渗透胁迫下表现出数周的稳定性和机械稳健性。分子模拟表明，CAHS12在囊泡腔内形成丝状网络，可减轻膜的塌陷并保持区室结构。具有无细胞转录翻译能力的合成细胞能够经受住干燥并恢复生物化学活性，类似于缓步动物的隐生状态。这一发现为合成细胞在生物工程、不依赖冷链的生物制造和自适应生物界面中的应用开辟了道路。


### 核心发现


- CAHS12在巨型单层囊泡内可逆性结构转变，增强膜完整性并模拟天然隐生过程。

- CAHS12在囊泡腔内形成丝状网络，减轻膜塌陷并维持区室架构。

- 含无细胞转录翻译系统的合成细胞借助CAHS12实现耐干燥和生化活性恢复，呈现类隐生态。
...
## 2. Single-molecule kinetic exploration of functional sub-states in an evolving phosphotriesterase.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-May-02 |
| **作者** | Sakuma Morito, Mahato Dhani Ram, Feixas Ferran, Jackson Colin J, Nakata Eiji, Osuna Sílvia, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-72534-1) |
...
酶可以存在于多种稳定的功能状态或亚态中，即使在来自克隆基因的表达群体内，催化速率也存在显著的分子间异质性。越来越多的证据表明，生物系统已经进化到利用这些功能亚态来调节反应速率和酶的多功能性或混杂性。因此，理解进化动力学中的这种异质性对于阐明酶适应性和可进化性的机制至关重要。为了解决这个问题，我们利用单分子动力学分析功能亚态的进化变化。我们测量了野生型磷酸三酯酶（PTE）以及跨越从天然PTE活性到混杂芳基酯酶功能轨迹的18种进化变体的功能亚态。进一步通过对选定变体的构象亚态进行分子动力学模拟进行研究。我们的结果表明，功能优化与潜在功能和构象亚态的重新分布紧密耦合，特别是涉及到功能必需loop 7的开闭动力学。这些发现为酶进化涉及构象异质性及其功能表现的协调转变提供了直接证据，共同塑造了酶的功能。


### 核心发现


- 单分子动力学揭示PTE进化中功能亚态的重新分布与催化效率优化紧密耦合。

- 功能关键loop 7的开闭构象亚态重排驱动了酶功能多样性的进化转变。

- 酶进化伴随构象异质性与功能表现的协同变化，共同塑造酶适应性和可进化性。
...
## 3. DNA-guided CRISPR-Cas12a effectors for programmable RNA recognition and cleavage.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Biotechnol |
| **发表日期** | 2026-May-01 |
| **作者** | Wu Xiaolong, Lam Wai Hei, Zhao Zibin, Cao Yumeng, Lin Haosi, Feng Xianzhen, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41587-026-03120-5) |
...
CRISPR-Cas效应器通常依赖RNA向导识别靶标。在Cas12a中，DNA上的原间隔序列相邻基序（PAM）与保守蛋白残基结合，触发靶标结合与核酸酶激活。本研究将Cas12a重编程为DNA引导、RNA靶向的效应器。利用PAM依赖性相互作用，设计合成CRISPR DNA与Cas12a形成功能性脱氧核糖核蛋白复合物，仅以RNA作为可编程靶标。结构、生物物理与生化分析揭示了DNA引导、RNA靶向构型的分子基础，并支持一条区别于经典RNA向导系统的激活通路。DNA引导的Cas12a实现了直接RNA检测与高效胞内RNA敲低，为CRISPR-Cas12a建立了模块化激活架构，拓展了可编程RNA操控的设计空间。


### 核心发现


- 将Cas12a重编程为DNA引导、RNA靶向的效应器，颠覆传统RNA引导模式。

- 揭示DNA引导RNA靶向系统的分子基础与独特激活通路。

- DNA引导Cas12a实现直接RNA检测与胞内敲低，拓展RNA操控设计空间。
...
## 4. Evidence of off-target probe binding affecting 10x Genomics Xenium gene panels compromise accuracy of spatial transcriptomic profiling.

| 字段 | 内容 |
|---|---|
| **期刊** | Elife |
| **发表日期** | 2026-May-01 |
| **作者** | Hallinan Caleb, Ji Hyun Joo, Tsou Edmund, Salzberg Steven L, Fan Jean |
| **原文链接** | [查看原文](https://doi.org/10.7554/eLife.107070) |
...
基于探针的原位空间解析转录组技术生成的空间基因表达谱准确性，取决于探针与其预期靶基因结合的特异性。脱靶结合（即探针与非靶基因结合）会扭曲基因的真实表达谱，因此探针特异性对于可靠的转录组学至关重要。本文研究了影响 10x Genomics Xenium 技术的脱靶结合问题。我们开发了软件工具 Off-target Probe Tracker (OPT)，通过比对探针靶序列并在多个参考注释中评估定位位点是否与预期靶基因对应，来识别推定的脱靶结合。将 OPT 应用于 Xenium 人乳腺基因面板，发现 313 个基因中至少有 14 个可能因脱靶结合到蛋白质编码基因而受影响。为证实预测，我们利用该基因面板生成的 Xenium 乳腺癌数据集，并与来自同个肿瘤蜡块的 Visium CytAssist 和 3' 单细胞 RNA 测序的正交空间及单细胞转录组谱进行比较。结果表明，对于某些基因，Xenium 检测到的表达模式明显反映了靶基因与预测脱靶基因的总表达量，而非单独靶基因的表达。我们进一步应用 OPT 识别定制基因面板中的潜在脱靶结合，并整合组织特异性 RNA 测序数据以评估影响。总之，这项工作增强了空间转录组数据的生物学可解释性，提高了空间转录组研究的可重复性。


### 核心发现


- 开发 OPT 工具识别 Xenium 探针脱靶结合，在人乳腺面板中发现至少 14/313 个基因受影响。

- Xenium 检测的某些基因表达实际反映靶标与脱靶基因的总表达，而非单独靶基因的表达。

- OPT 可推广至定制面板并整合组织 RNA-seq 数据，提升空间转录组数据的可解释性与重现性。



**关键词**：Humans · Gene Expression Profiling · Genomics · Breast Neoplasms · Software · Transcriptome · Female


---


## 5. Functional impact of genetic background on variable expressivity in neurodevelopmental disorders.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-May-01 |
| **作者** | Sun Jiawan, Noss Serena, Smolen Corrine, Bhavana Venkata Hemanjani, Banerjee Deepro, Das Maitreya, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-72598-z) |
...
疾病相关变异可导致神经发育障碍中出现不同的表型结果，但造成这种变异性的生物学机制仍知之甚少。本研究以16p12.1缺失为可变表现度的范例，开发了一套研究框架。利用来自患病家庭的诱导多能干细胞模型和携带16p12.1缺失的CRISPR编辑细胞系，发现该缺失与遗传背景中的罕见变异共同影响染色质可及性和神经发育基因的表达。细胞分析鉴定出家族特异性表型，包括抑制性神经元产生和神经祖细胞增殖的改变，这些表型与头围变异相关。通过CRISPR激活（CRISPRa）逐个上调16p12.1区域基因，可通过调节关键发育信号通路差异性地挽救这些缺陷。整合分析进一步识别出包括转录因子FOXG1和JUN在内的调控枢纽，介导上述效应。本研究为探究个体遗传架构如何导致神经发育障碍的表型变异性提供了一个功能性框架。


### 核心发现


- 开发以16p12.1缺失为范例的框架，揭示缺失与遗传背景中罕见变异共同影响染色质可及性和基因表达。

- 细胞分析鉴定出抑制性神经元产生和神经祖细胞增殖的家族特异性表型，与头围变异密切相关。

- CRISPR激活单个16p12.1基因可变挽救缺陷，并鉴定FOXG1和JUN作为调控枢纽介导表型变异。
...
## 6. Excitability as a design principle in the immune system.

| 字段 | 内容 |
|---|---|
| **期刊** | Sci Adv |
| **发表日期** | 2026-May |
| **作者** | Lebel Yael, Alon Uri |
| **原文链接** | [查看原文](https://doi.org/10.1126/sciadv.aeb0921) |
| **DOI** | `10.1126/sciadv.aeb0921` |
...
免疫学知识日益增长，但尚缺乏统一概念来解释分子回路如何实现免疫目标——对病原体的强响应、自身耐受及防止附带损伤。本文提出，来自动力系统的“兴奋性”（excitability）概念可在多种情境下实现这些目标。一个可兴奋系统在刺激超过阈值时产生大幅响应脉冲，随后自行关闭，并进入不应期。我们通过数学方法筛选数千种细胞因子与细胞回路，识别出少数能表现兴奋性的回路。其中，唯一一个稳健回路在速度与强度上达到帕累托最优——该效应器能诱导自身生成并诱导其抑制剂。此回路在人类免疫网络中出现数十次，而次优回路则未见。它解释了严重急性呼吸综合征冠状病毒2（SARS-CoV-2）感染、自身免疫发作及肿瘤免疫的相关数据，并提示了治疗靶点。兴奋性可能统一我们对免疫回路的理解。


### 核心发现


- 提出兴奋性概念作为免疫系统设计原则，可解释阈值响应、自行关闭和不应期等特性。

- 通过数学模型筛选，发现一个帕累托最优的回路：效应器自我诱导并诱导抑制剂，在免疫网络中反复出现。

- 该回路能统一解释新冠、自身免疫发作和肿瘤免疫等现象，并指出潜在治疗靶点。



**关键词**：Humans · Immune System · SARS-CoV-2 · COVID-19 · Cytokines · Models, Immunological · Neoplasms


---


## 7. PSGRN: Gene regulatory network inference from single-cell perturbational data through self-training with synthetic gold standards.

| 字段 | 内容 |
|---|---|
| **期刊** | Sci Adv |
| **发表日期** | 2026-May |
| **作者** | Song Xinhan, Deng Kaiwen, Chen Mandy, Guan Yuanfang |
| **原文链接** | [查看原文](https://doi.org/10.1126/sciadv.aeb3376) |
...
基因调控网络（GRN）对于理解基因如何协调细胞过程至关重要。大规模单细胞扰动研究为GRN推断提供了强大机遇，但许多当前最优方法未能充分利用干预信息。我们提出PSGRN，该方法是CausalBench挑战赛中的顶级方法，通过基于合成金标准的自训练框架整合干预性和观测性单细胞RNA测序数据。在八个数据集和六项评估指标上，PSGRN始终优于现有方法。使用干预数据，在K562细胞中，PSGRN与近期最优方法相比，Wasserstein距离最高提升43%，且错误遗漏率最低。利用实验验证的调控相互作用，PSGRN的精确度提升达30%，召回率提升超过100%。这些结果突显了PSGRN的多功能性和可扩展性，使其成为从单细胞数据进行GRN推断和生物学发现的稳健工具。


### 核心发现


- PSGRN通过合成金标准自训练框架整合干预与观测数据，在多项指标上显著优于现有方法。

- 在K562细胞中，PSGRN将Wasserstein距离提升43%，并实现最低错误遗漏率。

- 基于实验验证调控对，PSGRN的精确度提升30%以上，召回率提升超过100%。



**关键词**：Gene Regulatory Networks · Single-Cell Analysis · Humans · Computational Biology · Algorithms · Sequence Analysis, RNA · K562 Cells


---


## 8. Dynamic heterogeneity and hidden fluidity in dense epithelial tissues.

| 字段 | 内容 |
|---|---|
| **期刊** | Sci Adv |
| **发表日期** | 2026-May |
| **作者** | Shen Yuan, Xi Wang, Mège René-Marc, Kob Walter, Ladoux Benoit |
| **原文链接** | [查看原文](https://doi.org/10.1126/sciadv.aec3773) |
| **DOI** | `10.1126/sciadv.aec3773` |
...
上皮组织在形态发生、修复和疾病过程中不断重塑，同时保持器官完整性。在高细胞密度下，这些组织常表现为无序、类固体状态的力学停滞，这就提出了它们如何保留重组能力的问题。本研究表明，与热玻璃不同，致密的上皮组织并不表现为笼效应行为，而是表现为一种复杂流体。细胞展现出亚扩散蠕变，同时伴随菲克型但非高斯的动力学和压缩指数弛豫，这些都是应力驱动流动性的显著特征。这种流动性源于组织的结构和力学组织，而非细胞分裂或挤出——这些过程仅瞬时增强局部动力学。快速移动的细胞组织成集体的各向异性集群，其空间异质性与局部结构熵和软振动模式相关。总之，这些发现揭示了致密上皮组织中隐藏的流动性，它支持力学稳定性，同时保持发育、伤口愈合和早期肿瘤侵袭过程中的重塑能力。


### 核心发现


- 致密上皮组织表现为复杂流体，呈现应力驱动的亚扩散蠕变、非高斯动力学和压缩指数弛豫。

- 这种流动性源于组织的结构与力学组织，而非细胞分裂或挤出的瞬时效应。

- 快速细胞形成集体各向异性集群，其空间异质性与局部结构熵和软振动模式相关。



**关键词**：Epithelial Cells · Epithelium · Animals · Humans · Cell Movement · Biomechanical Phenomena


---


## 9. Modular engineering of thermoresponsive allosteric proteins.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Chem Biol |
| **发表日期** | 2026-May |
| **作者** | Kroell Ann-Sophie, Hoffmann Kira H, Motzkus Nikolas A, Lemmen Nina, Happ Nele, Wolf Benedict, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41589-026-02151-y) |
...
热遗传学能够对活细胞和组织中的蛋白质活性实现非侵入性的时空控制，但其应用主要局限于转录调控和膜募集。本文中，我们提出了一种通用的工程化策略，通过插入优化的燕麦LOV2结构域变体，构建温度敏感变构蛋白。将该方法应用于大肠杆菌中一组结构无关且功能各异的蛋白质，我们生成了高效的、热可切换的嵌合变体，可在狭窄温度范围（37-41°C）内受到严格控制。将这一策略扩展到哺乳动物系统，我们构建了可在生理温度范围内通过微小温度变化直接调控的CRISPR-Cas基因组编辑器。最后，我们展示了整合化学受体结构域作为替代温度感应模块，表明温度敏感性可能是受体结构域中的一个普遍特征。这项工作扩展了热遗传学的工具箱，为几乎任何目标蛋白的温度依赖性控制提供了蓝图。


### 核心发现


- 开发了通过插入优化的LOV2结构域变体构建温度敏感变构蛋白的通用模块化策略。

- 在大肠杆菌中将多种无关蛋白改造为在37-41°C狭窄范围内可严格控制的温度开关。

- 在哺乳动物中构建温度调控的CRISPR-Cas编辑器，并揭示受体结构域具有内在温敏性。



**关键词**：Allosteric Regulation · Protein Engineering · Escherichia coli · Temperature · CRISPR-Cas Systems · Avena · Gene Editing


---


## 10. Multiband-driven anisotropic vortex topology in iron-based superconductors yielding a strain-tunable x-vortex Majorana platform.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-Apr-30 |
| **作者** | Yu Si-Qi, Cheng Wei, Li Chuang, Gong Zi-Sheng, Chen Zi-Yi, Pan Xiao-Hong, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-71938-3) |
| **DOI** | `10.1038/s41467-026-71938-3` |
...
我们发现，在铁基超导体（FeSCs）中，涡旋取向与多带拓扑之间的相互作用导致了马约拉纳涡旋拓扑的显著各向异性。这种各向异性表现为两种不同的涡旋构型：z-涡旋和x-涡旋，分别垂直于和平行于狄拉克轴（对于FeSCs为z轴）。x-涡旋展现出独特的分叉，呈现出两个截然不同的拓扑相图。其中一个出奇地简单，仅由平庸和拓扑超导相组成，并且对多带纠缠具有鲁棒性。另一个则镜像了z-涡旋的复杂相图，具有交替出现的平庸、拓扑晶体和拓扑超导相。前者是x-涡旋所独有的，能在宽参数范围内支持非配对的马约拉纳涡旋，即便在正常态狄拉克节点存在的情况下也是如此。值得注意的是，单轴应变可以调控这些x-涡旋相，使得x-涡旋能够以可控的方式同时支持稳定的马约拉纳涡旋以及丰富的奇异物理。此外，我们提出x-涡旋为在铁基超导体中开发马约拉纳纳米线器件提供了有前景的优势。我们的发现为多带超导体系中的涡旋拓扑引入了新的范式，突显出x-涡旋作为探索马约拉纳物理和推进铁基超导量子器件的有前途的平台。


### 核心发现


- 发现马约拉纳涡旋拓扑的各向异性，分为z-涡旋和x-涡旋两种构型。

- x-涡旋具有独特分叉，存在一个简单拓扑相图，支持宽带非配对马约拉纳涡旋，且对多带纠缠具有鲁棒性。

- 单轴应变可调控x-涡旋相，使其成为探索马约拉纳物理和铁基超导量子器件的可控平台。
...
## 11. Anhydrotetracycline-Inducible Metabolic Flux Adaptation for 3-Hydroxypropionic Acid Production in Pichia pastoris.

| 字段 | 内容 |
|---|---|
| **期刊** | ACS Synth Biol |
| **发表日期** | 2026-Apr-30 |
| **作者** | Haneef Shabana, Zhou Yongjin J, Bai Fan |
| **原文链接** | [查看原文](https://doi.org/10.1021/acssynbio.5c00922) |
| **DOI** | `10.1021/acssynbio.5c00922` |
...
微生物细胞工厂对化学品、燃料和药物的可持续生产至关重要，但其性能取决于基因表达的精确调控，以平衡细胞生长与生产之间的代谢通量。虽然通过组成型基因表达调控代谢网络可促进高水平生产，但也会带来影响细胞生长的代谢负担。本研究为甲醇营养型酵母毕赤酵母开发了一个多功能且高效的诱导平台，实现了以甲醇为唯一碳源动态表达基因生产 3-羟基丙酸（3-HP）。我们首先构建了由组成型启动子 PGCW14 改造而来的无水四环素（aTc）诱导型启动子文库，其强度多样。该文库包含 32 个诱导型启动子，激活强度范围为 PGCW14 的 2.5% 至 84.7%。其次，我们利用 aTc 诱导型启动子动态调控 3-HP 生物合成途径的关键基因，包括以强启动子 RM5 和 RM6 表达来自橙色绿屈挠菌的丙二酰辅酶 A 还原酶基因（MCR）、以极强启动子 RM1 表达内源丙二酰辅酶 A 羧化酶基因 ACC1，以及以中等强度启动子 RM12 表达跨膜通透酶 PpMFS。最终工程菌株 SHP18 在摇瓶中以甲醇为唯一碳源可生产 3.7 g/L 的 3-HP。这一模块化的 aTc 诱导型启动子工具箱在毕赤酵母中提供了精细的分级转录控制，为细胞工厂设计提供了一种广泛适用的策略。


### 核心发现


- 构建了包含 32 个 aTc 诱导型启动子的强度多样性文库，在毕赤酵母中实现了 2.5%-84.7% 的精细分级转录控制。

- 通过 aTc 诱导系统动态调控 MCR、ACC1 和 PpMFS 基因，平衡了 3-羟基丙酸合成中的代谢通量。

- 工程菌株 SHP18 以甲醇为唯一碳源在摇瓶中生产 3.7 g/L 3-羟基丙酸，展示了该平台在细胞工厂中的应用潜力。
...
## 12. Peer influence decay and behavioral diffusion in adolescent networks: A simulation approach.

| 字段 | 内容 |
|---|---|
| **期刊** | Science |
| **发表日期** | 2026-Apr-30 |
| **作者** | Wang Cheng, Butts Carter T, Hipp John R, Lakon Cynthia M |
| **原文链接** | [查看原文](https://doi.org/10.1126/science.aea9297) |
...
同伴影响在社交网络中传播多远才会消散？本研究利用美国青少年健康纵向研究中两所学校（n=3154名学生）的纵向数据，考察吸烟行为在青少年友谊网络中的扩散。采用随机行动者导向模型，我们模拟了针对重度吸烟者的不同干预策略（随机、入度、特征向量中心性）和覆盖范围（10%至100%）。一种新的指数衰减模型量化了影响衰减，揭示间接同伴影响（即溢出效应）可达到目标个体外三步之遥。针对10%至30%的中心个体干预可最大程度减少吸烟，但覆盖范围超过40%至50%后因网络饱和而收益趋于平缓。分析显示，较密集的网络比较大稀疏网络扩散更广、衰减更慢。这一衰减度量可优化不同网络结构下的干预设计。


### 核心发现


- 吸烟行为的同伴影响可扩散至社交网络三步之外，并通过指数衰减模型量化了影响衰减过程。

- 针对10%-30%高中心性个体的干预效果最大化，超过40%-50%覆盖后因网络饱和增益不再增加。

- 密集网络中的行为扩散更广、衰减更慢，衰减度量可用于优化不同网络结构下的干预策略。



**关键词**：Adolescent · Humans · Peer Influence · Adolescent Behavior · Longitudinal Studies · Male · Female · Peer Group · Social Networking · Friends


---


## 13. Toward life with a 19-amino acid alphabet through generative artificial intelligence design.

| 字段 | 内容 |
|---|---|
| **期刊** | Science |
| **发表日期** | 2026-Apr-30 |
| **作者** | Liu Liyuan, Rochereau Charlotte, Kozlov Simon, Urtecho Guillaume, Liu Xiaonan, Zhao Jiahui, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1126/science.aeb5171) |
...
因为所有已知的生命体至少由20种标准氨基酸构成，使用更简化氨基酸“字母表”的生命可行性仍不明确。在本研究中，我们利用计算设计和合成生物学探索用19种氨基酸的字母表构建一个细胞。初步分析表明异亮氨酸（Ile）可能并非必需，我们通过在大肠杆菌必需蛋白质中直接替换异亮氨酸残基验证了这一点。关键的是，在大多数情况下，蛋白质语言模型和基于结构的模型是重新设计功能性无Ile蛋白质所必需的。我们系统地替换了核糖体中全部382个异亮氨酸残基，并在天然基因组位点组合了21个重新设计的亚基，产生了一个可存活且进化稳定的细胞。这项工作为创造自早期进化以来的第一个19种氨基酸生物体提供了路线图。


### 核心发现


- 计算分析指出异亮氨酸可省略，经必需蛋白替换证实，19氨基酸字母表可能。

- 生成式蛋白质语言模型与结构模型是重新设计无Ile功能蛋白的必要工具。

- 系统替换核糖体全部382个Ile残基，装配21个重设计亚基，创建首个19氨基酸可存活进化稳定细胞。



**关键词**：Escherichia coli · Escherichia coli Proteins · Generative Artificial Intelligence · Genetic Code · Isoleucine · Ribosomes · Synthetic Biology · Codon · Codon Usage · Valine


---


## 14. Population dynamics of generalist and specialist strategies under feast-famine cycles.

| 字段 | 内容 |
|---|---|
| **期刊** | PLoS Comput Biol |
| **发表日期** | 2026-Apr-30 |
| **作者** | Niimi Rintaro, Furusawa Chikara, Himeoka Yusuke |
| **原文链接** | [查看原文](https://doi.org/10.1371/journal.pcbi.1014265) |
...
微生物种群展现出广泛的营养利用策略，从利用多种营养物的“泛化种”到高度适应特定营养物的“特化种”。识别营养利用策略多样化的条件，是生态学中的核心问题之一。先前的理论研究表明，不同资源利用功能之间的权衡——即细胞无法以近乎最佳的效率利用广泛的底物类型——对于多样化策略的出现至关重要。此外，在自然环境中，营养的可利用性常随时间波动，这给细胞施加了另一种权衡：在营养丰富条件下生长迅速的细胞，在营养贫瘠条件下往往具有更高的死亡率，导致生长-死亡权衡。这种额外的权衡可能促进多样化策略的产生。在此，我们引入了一个同时纳入资源利用权衡和生长-死亡权衡的数学模型。营养供应被建模为离散的随机事件，以模拟营养可利用性随时间的变化。我们表明，生长率与死亡率比值更高的表型将主导种群；即，生长-死亡权衡的强度在不同策略的出现中起着关键作用。我们还发现，稀疏且不确定的营养供应有利于特化种，提高了它们的时间平均丰度。我们的发现凸显了时间环境变化以及由此产生的生长-死亡权衡在驱动微生物营养利用策略多样化中的关键作用。


### 核心发现


- 生长率与死亡率比值决定优势表型，生长-死亡权衡强度是策略分化的关键。

- 稀疏且不确定的营养供应有利于特化种，增加其时间平均丰度。

- 同时纳入资源利用和生长-死亡权衡的模型揭示环境波动驱动策略多样化。
...
## 15. Learning the bistable cortical dynamics of the sleep-onset period.

| 字段 | 内容 |
|---|---|
| **期刊** | PLoS Comput Biol |
| **发表日期** | 2026-Apr-30 |
| **作者** | Hu Zhenxing, Aravind Manaoj, Lei Xu, Kutz J Nathan, Aucouturier Jean-Julien |
| **原文链接** | [查看原文](https://doi.org/10.1371/journal.pcbi.1014246) |
...
人类并不会像木头一样瞬间入睡，亦非阶跃函数式转变。相反，睡眠起始期（SOP）的脑电图（EEG）呈现动态和非单调变化，具有高度且至今仍不清楚的个体内与个体间变异性。睡眠调节网络的计算模型提出，睡眠过渡可视为一个由底层的控制信号或“睡眠驱动力”决定的鞍结分岔处的噪声分岔。然而，这些模型并未描述SOP中内部控制信号如何产生稳定的清醒与睡眠状态间的快速切换，也未描述这些状态空间变化如何转化为宏观EEG。在此，我们提出一个最小参数化的随机动态模型，其中由一个缓慢变化的控制参数驱动清醒到睡眠的过渡，同时表现出噪声驱动的双稳态。我们提供了一种根据实验性睡眠EEG数据的单次观测来估计模型参数的方法，并表明该模型能重现广泛的SOP现象学。利用模型分析已有的睡眠EEG数据集，我们发现估计的模型参数与主观困倦报告相关。这些结果表明，SOP的双稳态特征可作为生物标志物，用于追踪睡眠起始障碍的个体内和个体间变异性。


### 核心发现


- 提出最小参数化随机模型，揭示睡眠起始期清醒-睡眠切换由噪声驱动的双稳态。

- 该模型能从单次睡眠EEG观测估计参数，重现睡眠起始期的多种现象学特征。

- 模型参数与主观困倦相关，表明SOP双稳态可作为睡眠障碍变异性的生物标志物。
...
## 16. On-demand seizures facilitate rapid screening of therapeutics for epilepsy.

| 字段 | 内容 |
|---|---|
| **期刊** | Elife |
| **发表日期** | 2026-Apr-30 |
| **作者** | Chen Yuzhang, Litt Brian, Vitale Flavia, Takano Hajime |
| **原文链接** | [查看原文](https://doi.org/10.7554/eLife.101859) |
...
癫痫动物模型在药物开发和治疗性测试中至关重要。然而，评估癫痫治疗的主流方法面临高通量与病因相关性之间的权衡。筛选模型要么基于在野生型、初次使用动物中急性诱发的癫痫发作，要么基于慢性癫痫动物中的自发癫痫发作。两者各有缺点——急性致惊厥剂或点燃诱发的发作未考虑患病癫痫大脑中的众多神经病理学改变，而慢性癫痫模型中自发的行为性发作稀少，使得实验在统计学上需要耗费大量时间。在本研究中，我们开发了Opto-IHK（在海马内海人酸小鼠中光遗传学诱导癫痫发作）模型，这是一种在慢性癫痫小鼠中“按需”引发癫痫发作的机制性方法。我们短暂同步患病海马CA1区的主神经元，以可靠地诱导刻板的按需行为性发作。这些诱导的发作类似于癫痫动物中自然发生的自发性发作，并可以被常用处方抗癫痫药物如左乙拉西坦和地西泮终止。此外，我们显示慢性癫痫动物中诱导的发作与首次使用动物的发作不同，强调了在患病回路中评估治疗的重要性。总而言之，我们展望Opto-IHK模型将加速癫痫的药理学和闭环干预措施的评估。


### 核心发现


- 开发了Opto-IHK模型，在慢性癫痫小鼠中按需光遗传诱导与自发发作相似的刻板行为发作。

- 诱导的发作可被常用抗癫痫药物左乙拉西坦和地西泮有效终止。

- 慢性与初用动物诱发发作不同，凸显在患病回路中评估疗法的必要性。



**关键词**：Animals · Epilepsy · Mice · Disease Models, Animal · Seizures · Anticonvulsants · Optogenetics · Kainic Acid · Drug Evaluation, Preclinical · Diazepam


---


## 17. Complete biosynthesis of nicotine.

| 字段 | 内容 |
|---|---|
| **期刊** | Cell |
| **发表日期** | 2026-Apr-30 |
| **作者** | Chang Lijing, Xu Zhen, Deng Purong, Zhang Ning, He Tingrui, Liu Xinying, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.cell.2026.03.034) |
...
尼古丁是烟草中具有成瘾性和强效杀虫作用的生物碱，深刻影响了人类历史、农业及烟草植物本身。然而其生物合成中的酶促步骤和反应机制至今不明。本研究发现，最终偶联反应由尿苷二磷酸（UDP）-糖基转移酶通过糖基化稳定，经A622还原活化，再经立体选择性分子间曼尼希样反应缩合，随后依次被小檗碱桥酶样（BBL）氧化和β-葡萄糖苷酶去糖基化，最终生成尼古丁。一个由五种组分构成的代谢区室在液泡膜上组装，偶联尼古丁的生物合成与转运。我们在体外和异源体内重建了这一代谢区室，破坏任何组分都会消除尼古丁的积累。多药物和毒性化合物外排（MATE）转运蛋白对于在异源植物中高效工程化生产尼古丁至关重要，并能赋予植物抗虫性。该工作补全了尼古丁生物合成通路，并深刻揭示了分子间曼尼希样反应这一形成众多植物生物碱骨架的基本机制。


### 核心发现


- 揭示尼古丁合成的最终偶联反应需经糖基化稳定、A622还原、曼尼希缩合、BBL氧化和去糖基化五步完成。

- 发现由五种蛋白质在液泡膜上组成的代谢区室，实现了尼古丁生物合成与转运的协同。

- 在异源植物中重建该代谢区室并利用MATE转运蛋白可高效生产尼古丁并赋予抗虫性。



**关键词**：Nicotine · Nicotiana · Glycosylation · Plant Proteins · Glycosyltransferases


---


## 18. Deciphering mechanical determinants of morphological evolution.

| 字段 | 内容 |
|---|---|
| **期刊** | Cell |
| **发表日期** | 2026-Apr-30 |
| **作者** | Bailleul Richard, Cuny Nicolas, Khoromskaia Diana, Basu Soham, Bergamini Giulia, Cucurachi Paolo, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.cell.2026.02.010) |
...
形态多样性如何源于生物力学过程的变异仍是一个悬而未决的问题。尽管力塑造组织，但跨物种间力生成系统如何差异以创造多样形态尚不清楚。本研究结合比较形态发生和活性物质理论，跨越六种刺胞动物物种（涵盖5亿年的分化），鉴定了幼虫形态多样性的力学基础。我们定义了物种特异性的力学模块配置——称为力学类型（mechanotypes）——可定量预测各分类单元的幼虫形态。我们发现形态伸长在中观尺度上是一个简单性状，其变异取决于一个力学模块，而形态极性是一个复杂性状，依赖于多个模块。模拟种间调控差异的扰动重塑了这些模块，将幼虫形态重新编程为类似姊妹物种的形态。通过建立跨物种比较的中观力学框架，该工作揭示了有限的组织尺度参数变异如何产生形态多样性。


### 核心发现


- 力学类型（mechanotypes）可定量预测跨物种幼虫形态的演化差异。

- 形态伸长是简单性状，由单一力学模块决定；极性则是多模块决定的复杂性状。

- 模拟种间调控差异的扰动可重编程幼虫形态为类似姊妹物种的形态。



**关键词**：Animals · Biological Evolution · Larva · Biomechanical Phenomena · Morphogenesis · Species Specificity · Cnidaria


---


## 19. Translation-dependent degradation of cas12 mRNA triggered by an anti-CRISPR.

| 字段 | 内容 |
|---|---|
| **期刊** | Nature |
| **发表日期** | 2026-Apr-29 |
| **作者** | Marino Nicole D, Talaie Alexander, Gerovac Milan, Rodriguez Jorge L, Schmidt Andrew D, Astmann Theresa J, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41586-026-10440-8) |
...
细菌编码多种防御系统（包括CRISPR-Cas）以识别并切割噬菌体及其他可移动遗传元件的DNA。作为回应，噬菌体编码抗CRISPR（Acr）蛋白，通过阻断DNA结合或切割来抑制CRISPR-Cas活性。本文报道了抗CRISPR蛋白AcrVA2抑制Cas12a生物发生的一种意外机制。AcrVA2结合Cas12a N端附近保守且功能重要的氨基酸残基，并在cas12a mRNA翻译过程中触发其选择性降解。此外，AcrVA2 C端结构域中的保守残基使其能够与核糖体和多聚核糖体共沉降，这是实现靶向共翻译mRNA降解所必需的。AcrVA2的C端结构域在多种可移动遗传元件编码的同源物中广泛保守，且这些同源物通常存在于缺乏cas12a的宿主中，提示它们可能在其他细菌中识别并下调替代底物。这些发现揭示了细菌中分子冲突与基因调控的一种新机制。


### 核心发现


- AcrVA2结合Cas12a N端保守残基，在翻译过程中触发cas12a mRNA的选择性降解。

- AcrVA2的C端结构域与核糖体共沉降是实现共翻译mRNA降解的关键。

- AcrVA2的C端结构域在缺乏cas12a的宿主中广泛保守，暗示其可调控其他底物。
...
## 20. Stochastic intracellular calcium dynamics show preserved structures identified by deep learning classification.

| 字段 | 内容 |
|---|---|
| **期刊** | PLoS Comput Biol |
| **发表日期** | 2026-Apr-29 |
| **作者** | Choi Jaesung, Chanu Athokpam Langlen, Awasthi Shakul |
| **原文链接** | [查看原文](https://doi.org/10.1371/journal.pcbi.1014240) |
...
细胞内钙离子（Ca2+）展现出多样的动力学行为，这些行为与健康和疾病相关的细胞生理状态紧密相连。虽然确定性模型预测了生化参数如何产生不同的动力学区制——稳态、振荡、簇发、混沌和多周期性，但真实生物系统因分子群体的有限性而本质上是随机的。以往采用常规统计方法的研究表明，增强的内在噪声使这些动力学状态越来越难以区分，尤其是混沌和多周期性模式。这引出了一个问题：在现实噪声水平下，依赖于参数的组织原理是否依然存在，从而保持生物学意义和计算可检测性。我们通过设计一种大核卷积神经网络（LKCNN）来解决这一问题，该网络旨在跨越噪声水平捕捉全局动力学特征。利用化学朗之万方程生成具有真实内在噪声的合成训练数据，尽管噪声水平在视觉上模糊了区分，LKCNN仍在分类八种不同动力学状态中达到约90%的准确率。通过胰腺β细胞以及其他细胞（包括WT-HEK293、STIM-KO和ORAI TKO）的实验Ca2+数据进行验证，实现了96.8%的准确率，证实了其超越合成数据集的泛化能力，显著优于传统基线方法（支持向量机和随机森林），后者在相同实验数据上仅分别达到54.0%和51.6%的准确率。这些结果表明，确定性的组织特征能在现实的生物噪声中持续存在，提示参数依赖的动力学结构代表着支配细胞功能的稳健原理。我们的发现证实，精湛的模式识别能够桥接理论确定性动力学与嘈杂的生物现实，为从本质随机振荡的生物过程中提取有意义的动力学信息提供了一个框架。


### 核心发现


- 核心发现1：大核卷积神经网络能准确分类被噪声掩盖的八种细胞内钙离子动力学状态，准确率达~90%。

- 核心发现2：模型在多种细胞实验数据上泛化准确率达96.8%，远超支持向量机和随机森林等传统方法。

- 核心发现3：确定性的动力学子结构在生物噪声中持续存在，证明参数依赖的组织原理是细胞功能的稳健基础。
...
## 21. Individual and population level uncertainty interact to determine the performance of outbreak surveillance systems.

| 字段 | 内容 |
|---|---|
| **期刊** | PLoS Comput Biol |
| **发表日期** | 2026-Apr-29 |
| **作者** | Arnold Callum R K, Kong Alex C, Winter Amy K, Moss William J, Patenaude Bryan N, Ferrari Matthew J |
| **原文链接** | [查看原文](https://doi.org/10.1371/journal.pcbi.1013309) |
| **DOI** | `10.1371/journal.pcbi.1013309` |
...
背景：暴发检测常依赖于不完善的个体病例诊断。暴发和病例均为可能被错误分类的离散事件，个体层面的不确定性可能影响疫情预警和检测系统的表现。本文描述了暴发检测性能如何依赖于个体层面的诊断检测特征和群体层面的流行病学特征，并说明了在不完善个体检测的条件下，也能达到与“完美”诊断检测相当的稳定表现的情境。方法：我们构建了一个随机SEIR模型，模拟麻疹（真实信号）和非麻疹发热出疹性疾病（噪声）的每日发病数。将非麻疹来源建模为独立静态（泊松）噪声，或与独立的SEIR过程（如风疹）一致的动态噪声。将暴发警报定义为7天滚动平均观察检测阳性数超过阈值，我们优化了在不同噪声结构和强度、诊断检测准确度（对应完美检测或建议的快速诊断检测）及检测率组合下使暴发检测准确度最大化的阈值。结论：每种诊断检测的最优阈值通常随检测率单调递增。在静态噪声下，使用类RDT检测和完美检测的暴发检测准确度均可达90%，且暴发检测延迟相当。在动态噪声下，完美检测情景的准确度优于类RDT检测（约90% vs. ≤80%）。随着动态噪声增大，暴发检测准确度下降，噪声极高时类RDT检测将导致持续警报状态。暴发检测系统的性能对背景噪声的结构和强度高度敏感。依据流行病学背景，使用快速诊断检测进行暴发检测的表现可以与完美检测一样好。


### 核心发现


- 暴发检测系统性能高度依赖背景噪声的结构和强度，动态噪声导致RDT检测准确度显著下降。

- 静态噪声下，快速诊断检测（RDT）能达到与完美检测相当的90%准确度和相似延迟。

- 存在使不完美检测达到最优暴发检测的阈值策略，其阈值随检测率单调增加。
...
## 22. A Streamlined and High-Osmolarity Bacillus subtilis Cell-Free Platform for Rapid Characterization of Genetic Regulatory Elements.
...
基于裂解液的无细胞系统能在可控环境中模拟细胞功能，已成为从基础研究到蛋白质生产和合成回路原型设计的通用合成生物学工具。本研究通过系统重新评估传统提取制备和反应条件，建立了一种精简且稳健的枯草芽孢杆菌无细胞系统。我们发现常规所需的run-off孵育和费时的透析步骤可被省略而不影响性能。关键改进在于反应缓冲液的重新配制：将谷氨酸钾浓度提高至400 mM以增加离子强度，并添加渗透保护剂甜菜碱，显著增强了翻译能力，实现了2–3 μM超折叠绿色荧光蛋白（sfGFP）的产率。与已建立的体内启动子文库数据基准比较显示，启动子强度的相对层级得以保持，验证了该平台作为细菌生理学预测代理的可靠性。我们进一步在无细胞系统内通过磷酸化级联激活HPr蛋白，使与其形成复合物的分解代谢物阻遏蛋白CcpA功能性地抑制含cre盒的启动子，从而重构了复杂的遗传调控。最后，我们发现这种高谷氨酸盐配方对另一种革兰氏阳性菌加氏乳杆菌同样有利，表明其具有广泛的宿主兼容性。总之，简化的制备流程和优化的缓冲液组成为在多种非模式细菌物种中开发无细胞系统提供了多功能工具包。


### 核心发现


- 建立省略run-off和透析的简化枯草芽孢杆菌无细胞系统，通过高谷氨酸钾和甜菜碱缓冲液将sfGFP产量提升至2–3 μM。

- 该系统忠实保留体内启动子强度层级，并成功重构CcpA-HPr磷酸化级联对cre启动子的阻遏调控。

- 高谷氨酸盐框架同样提升加氏乳杆菌无细胞系统的性能，展现跨革兰氏阳性菌的广宿主兼容性。
...
## 23. Thermophoresis Induced by Photothermal Shock Alters Reversal Potential and Capacitance in Neurons.
...
激光诱导热冲击在神经科学与生物学中应用广泛，但其对细胞热力学的影响仍不甚明了。我们结合全细胞膜片钳记录与双光子激光刺激，揭示光热冲击可导致皮层神经元钾通道反转电位出现显著去极化偏移，偏离经典能斯特预测。我们提出，热泳驱动的细胞内外离子通量扰乱了传统热力学平衡，需修正能斯特反转电位公式。为验证该假设，我们将热泳动力学整合到瞬态热扩散耦合的泊松-能斯特-普朗克模型中。分析表明，虽然热梯度在亚毫秒级消散，但细胞弛豫持续更久，残留热记忆调控离子重分布——这是标准霍奇金-赫胥黎与戈德曼-霍奇金-卡茨模型所忽视的效应。该扩展模型准确预测了光热冲击后钾反转电位的去极化偏移。此外，我们证明膜电容增加——常被归因于热致膜形变——亦可由热泳介导的离子重排引起。这些发现共同建立了热梯度下离子输运的统一生物物理模型，并为下一代光热神经技术的原理性发展奠定了新理论基础。


### 核心发现


- 光热冲击使神经元钾反转电位产生去极化偏移，偏离经典能斯特方程。

- 提出热泳驱动离子通量破坏热力学平衡，需修正传统能斯特公式。

- 建立整合热泳与热扩散的模型，揭示残余热记忆与电容增加的新机制。
...
## 24. ErbB family receptor dimerization dynamics and dysregulation via long-term single-molecule imaging.

| 字段 | 内容 |
|---|---|
| **期刊** | Cell |
| **发表日期** | 2026-Apr-28 |
| **作者** | Ma Kaibo, Ma Xiaojie, Shida João F, Niu Zijian, Lin Yuzhu Karlie, Mandal Saptarshi, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.cell.2026.04.010) |
...
二聚化对ErbB家族受体的激活至关重要，但致癌突变对其实时动态及影响仍不清楚。本研究利用上转换纳米颗粒（UCNPs）在活细胞中对EGFR、HER2和HER3进行长期多色单颗粒追踪，UCNPs无光漂白特性。该技术可连续观测受体相互作用，揭示其二聚化动态细节。致癌性EGFR突变促进稳定的、不依赖配体的二聚化。出乎意料的是，HER2和HER3均表现出组成性同源二聚化，促使我们对它们的激活机制模型进行修正。与EGFR突变相比，HER2突变适度增强同源二聚体稳定性，而HER3突变则破坏同源二聚体稳定性，提示HER3同源二聚化可能螯合HER3并限制其与其他受体的异源二聚化。我们还鉴定出三种受体间均存在稳定的、不依赖配体的异源二聚体，配体刺激可进一步增强其稳定性。这些发现提供了一张全面的ErbB相互作用网络，阐明多样化的二聚化机制及其对致癌信号传导的影响。


### 核心发现


- 利用无光漂白的上转换纳米颗粒实现长期多色单分子追踪，揭示ErbB受体二聚化实时动态。

- 发现HER2和HER3均存在组成性同源二聚化，修正了其激活机制模型。

- 绘制了全面的ErbB互作网络，揭示配体非依赖的异源二聚体及突变对二聚体稳定性的差异化调控。
...
## 25. Single-molecule localization and diffusivity microscopy reveals dynamic biomolecular organization in living cells.
...
活细胞中的单分子跟踪能测量蛋白质扩散性，但需要稀疏成像，限制了高密度成像。本文介绍单分子定位与扩散系数显微镜（SMLDM），这是一种基于深度学习的方法，可直接从单帧快照准确估计单分子运动轨迹和扩散系数，无需轨迹连接。通过结合明亮光激活荧光团的移动光激活定位显微镜（MPALM）和基于U-Net的单分子分割实现，与传统跟踪方法相比，该方法将数据密度提高50至300倍，生成活人细胞中分子扩散性和组织的高密度空间超分辨图谱。我们将MPALM应用于多种动态细胞过程，揭示了核小体聚集成低移动性染色质域、通路偏好的μ-阿片受体动态聚集、黏着斑运动和非均匀分子扩散性，以及早期液滴融合过程中的微凝聚体组织。SMLDM为在活细胞中以单分子分辨率解析生物分子组织和动力学提供了有力工具。


### 核心发现


- 建立SMLDM/MPALM方法，从单帧快照直接估计分子扩散，数据密度提高50-300倍，无需轨迹连接。

- 揭示核小体聚集成低移动性染色质域，呈现空间超分辨的组织模式。

- 发现μ-阿片受体呈通路偏好动态聚集，早期液滴融合中微凝聚体组织非均匀。
...
## 26. Bifurcation of neural firing patterns driven by potassium dynamics and neuron-electrode geometry during high-frequency stimulation.
...
高频刺激是深部脑刺激的基础，但引发多样神经元反应的机制尚不清楚。传统的传导阻滞理论认为钠通道失活和轴突失效，但无法解释活体观察到的突然且可重复的放电转变。本研究结合大鼠CA1神经元的单单位记录和生物物理详尽的多房室模型，探究高频刺激如何塑造轴突兴奋性。结果显示，神经元反应受两个耦合因素支配：电极-轴突几何关系与轴突周围细胞外钾离子（[K⁺]o）动力学。任一参数的微小变化都能可靠地触发强直、簇状与低速率规则放电之间的分岔样转变。随着电极-轴突距离增大，传导阻滞先于发放起始失效出现；而[K⁺]o升高则使膜在兴奋与非兴奋状态间切换。这一统一分岔框架扩展了传导阻滞假说，将轴突重新定义为非线性元件，并为优化电极放置、刺激调谐和闭环神经调控策略提供了机制性洞见。


### 核心发现


- 神经元反应由电极-轴突几何和轴突周细胞外钾离子动力学两个耦合因素决定。

- 微小参数变化即可触发强直、簇状和低速率规则放电之间的分岔样转变。

- 距离增大时传导阻滞先于发放起始失效，钾升高导致膜兴奋态切换，形成统一分岔框架。
...
## 27. Interrogating the structure and function of the human voltage-gated proton channel (hHv1) with a fluorescent noncanonical amino acid.
...
人电压门控质子通道（hHv1）是由电压传感器域（VSDs）组成的二聚体，每个单体含有高选择性质子通透路径。除了电压，hHv1还受其他刺激调控，包括pH梯度、机械力和配体如Zn2+。除VSDs外，该膜蛋白还含有N端结构域和单体间形成的C端卷曲螺旋结构域（CC）。为满足直接测量hHv1构象重排的需求，我们开发了一种荧光共振能量转移（FRET）方法，用于测量从大肠杆菌中纯化的全长hHv1的构象重排。我们利用遗传密码扩展（GCE）生成了14个全长hHv1构建体文库，每个构建体在结构域的不同位点引入了荧光非天然氨基酸吖啶酮-2-基丙氨酸（Acd）。表达和纯化这些hHv1-Acd蛋白后，我们发现12个位点产生了稳定且有功能的质子通透通道。Acd在每个位点的荧光特性表现出微小的位点特异性差异。此外，我们测量了hHv1-Acd蛋白中色氨酸（Trp）和酪氨酸（Tyr）到Acd的位点特异性FRET效率，结果与去垢剂胶束中的正确折叠一致。最后，加入Zn2+引起可逆的FRET变化，受影响的残基聚集在通道的细胞内侧面。


### 核心发现


- 利用遗传密码扩展将荧光非天然氨基酸Acd引入hHv1的14个位点，其中12个位点保持质子通透功能。

- 通过FRET测量位点特异性能量转移，证实Acd标记的hHv1在去垢剂胶束中正确折叠。

- Zn2+引起可逆FRET变化，受影响的残基聚集在通道细胞内侧面，揭示变构调控区域。
...
## 28. Global genetic interaction network of a human cell maps conserved principles and informs functional interpretation of gene co-essentiality profiles.
...
破译人类细胞内基因如何相互作用对于理解其功能连接以及开发靶向治疗策略至关重要。在本研究中，我们基于CRISPR技术对约400万对基因进行扰动，呈现了人类单倍体细胞系HAP1中的基因组规模遗传相互作用图谱。由此产生的网络包含约89,000个高置信度的基因-基因相互作用，将基因组织成与蛋白质复合物和通路、生物过程以及细胞区室相对应的层次化模块，这反映了在酵母中观察到的原理，并突显了人类细胞的功能架构。这一大规模遗传网络通过捕获独特的功能信息、揭示先前未表征基因的作用，以及识别癌细胞系特异性遗传依赖性的分子决定因素，补充了DepMap基因共必需网络。本研究提出了一种通用的数据驱动策略，用于系统性地探索基因在人类细胞系中的作用及其功能连接。


### 核心发现


- 构建了人类HAP1细胞中约8.9万对高置信遗传相互作用的基因组尺度网络。

- 基因按蛋白质复合物、通路和细胞区室形成层次模块，揭示人类细胞功能架构。

- 网络补充DepMap共必需网络，揭示未知基因功能和癌症特异性依赖的分子决定因素。
...
## 29. Proteome-wide prediction of the functional impact of missense variants with ProteoCast.
...
解析基因突变的功能影响对于深化我们对基因型-表型关系的理解以及识别治疗靶点至关重要。尽管在测序和基因组编辑技术方面取得了进展，全蛋白质组范围内的突变效应预测仍然充满挑战。本研究表明，仅凭进化信息即可准确预测整个蛋白质组中的突变效应。ProteoCast 是一种可扩展且可解释的计算方法，它利用蛋白质序列保守性对遗传变异进行分类并识别功能上重要的蛋白质位点。我们将 ProteoCast 应用于完整的黑腹果蝇蛋白质组（包含22,000个亚型、3亿个突变），并利用近40万个自然和实验变异对其进行验证。该方法将85%的已知致死突变正确分类为具有功能影响，而群体变异的这一比例仅为13-18%。由 ProteoCast 指导的基因组编辑实验证实了这些预测。此外，ProteoCast 还成功识别了功能上重要的蛋白质修饰位点和结合基序。ProteoCast 为研究任何生物体的基因功能和突变提供了公开可用的资源及可部署的分析流程。


### 核心发现


- 仅依赖进化保守性信息即可在全蛋白质组尺度上准确预测错义突变的功能影响。

- ProteoCast 能正确识别85%的致死突变，且准确率远超对普通群体变异的判断。

- 该计算方法可精确定位功能关键的蛋白质修饰位点与结合基序。
...
## 30. Nonergodicity and Simpson's paradox in neurocognitive dynamics of cognitive control.
...
非遍历性与辛普森悖论在认知神经科学中构成显著但未获充分重视的挑战。利用逾四千名个体的脑成像与行为数据，以及认知动力学的贝叶斯计算模型，我们考察了认知控制背后的脑-行为关系，分别在受试间与受试内层面进行分析。引人注目的是，脑-行为关联在分析层面间出现反转，揭示了普遍的非遍历性。受试内分析揭示了反应性与主动性控制分离的神经表征，并显示适应性与适应不良地调节认知控制的个体展现出截然不同的脑-行为关联。我们的发现表明，受试间分析会从根本上误判个体内机制，因为群体层面模式不仅与个体层面模式不一致，甚至常常相反。本研究强调了在神经科学中区分受试间与受试内推断的必要性，对于理解认知机制和设计个性化干预措施具有深远意义。


### 核心发现


- 脑-行为关联在群体与个体分析层面发生反转，证实认知控制中普遍存在非遍历性。

- 受试内分析分离出反应性与主动性控制的神经表征，揭示两者独立的功能架构。

- 个体对认知控制的适应性与适应不良调节模式，对应截然不同的脑-行为关系。
...
## 31. Targeting insulo-frontal pathway to reduce stress-evoked cognitive rigidity.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-Apr-27 |
| **作者** | Ma Shaorong, Wang Kuan Hong, Zuo Yi |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-72221-1) |
...
认知僵化常伴随慢性应激出现，并普遍存在于应激相关的精神疾病中，但其背后的神经环路机制尚不清楚。利用小鼠的注意力定势转移任务（AST），我们发现前岛叶皮层向内侧前额叶皮层（aIC→mPFC）的投射是适应性决策的关键调控者。aIC→mPFC神经元在错误试验后，而非正确试验后，表现出增强的活动。这种增强的活动会延续到后续试验中，提供一个凸显信号，从而增强mPFC的结果依赖性更新，并促进跨试验神经活动模式的收敛。在决策前阶段对aIC→mPFC投射进行光遗传学操控，会干扰mPFC的更新并损害AST表现。此外，慢性应激破坏了aIC活动的结果依赖性，并损害了认知灵活性。关键的是，通过光遗传学选择性地在错误试验后增强aIC→mPFC活动，能够增强mPFC更新，改善跨试验神经活动收敛，并恢复应激小鼠的认知灵活性。这些发现揭示了aIC→mPFC环路在将试验结果与适应性决策联系起来中的先前未被认识的作用，并确定该通路是治疗应激诱发的认知僵化的一个有希望的靶点。


### 核心发现


- aIC→mPFC投射在错误后增强并延续，提供凸显信号促进mPFC更新和活动收敛。

- 慢性应激破坏该通路结果依赖性活动，损害认知灵活性。

- 光遗传增强错误后该通路活动可恢复认知灵活性，是治疗应激僵化的新靶点。
...
## 32. Polyphosphate modulates the stress-responsive formation of functional RNA-protein condensates in bacteria and mammalian cells.
...
揭示驱动特定生物分子在体内形成相分离凝聚体的原因并确定其生理意义具有根本重要性。本研究显示，氮饥饿的大肠杆菌产生长链多聚磷酸盐，这些多聚磷酸盐作为支架将RNA伴侣蛋白Hfq组装成高分子量复合物，最终与RNA翻译和加工机器组分一起发生相分离。多聚磷酸盐在这些凝聚体中的存在通过选择性稳定参与转录和蛋白质翻译的多腺苷酸化RNA，并促进与翻译和RNA代谢相关蛋白（参与从头蛋白质合成）的相互作用，从而控制Hfq的功能。多聚磷酸盐的缺乏显著损害凝聚体形成，增加细胞死亡，并阻碍从氮饥饿中的恢复。在功能类比中，我们证明多聚磷酸盐特异性地促进人细胞系中处理小体（P小体）的形成，揭示了一种单一、高度保守且祖先的多阴离子作为整个生命树中功能性相分离凝聚体形成的调控因子。


### 核心发现


- 氮饥饿大肠杆菌利用多聚磷酸盐支架Hfq和翻译组件形成相分离凝聚体

- 多聚磷酸盐稳定poly(A)RNA并促进翻译蛋白互作，调控Hfq功能，缺乏则损害应激恢复

- 多聚磷酸盐特异促进人P体形成，揭示其作为古老多阴离子调控相分离凝聚体的保守性
...
## 33. Diversity in emergent cell locomotion from the coupling cytosolic and cortical Marangoni flows with reaction-diffusion dynamics.
...
细胞迁移是单细胞与多细胞生物生存与功能的基础过程。真核细胞的爬行运动源于由细胞骨架驱动的周期性突起与回缩，其组织受Rho GTPases在胞质与皮层之间的反应-扩散动力学调控。这些动力学通过生化信号与力学反馈的耦合产生空间膜模式并建立前后极性。我们开发了一个跨尺度平均场框架，将反应-扩散信号与胞质和皮层流体动力学整合，以捕捉细胞形状演变与涌现的细胞运动。模型仅需微小参数变化即可重现实验观察到的多种形状与运动表型，表明这些行为对应于自组织极限环。相空间分析揭示，与胞质流动和空间变化表面张力的耦合对于恢复完整的运动模式谱系至关重要，为理解变形虫样迁移提供了理论基础。


### 核心发现


- 跨尺度框架整合反应-扩散信号与流体动力学，成功复现多种细胞运动表型。

- 微小参数变化即可驱动细胞形状与运动模式多样性的自组织极限环。

- 胞质流动与空间变化的皮层表面张力耦合是产生完整运动模式谱系的关键。
...
## 34. STARCall integrates image stitching, alignment, and read calling to enable scalable analysis of in situ sequencing data.
...
荧光原位测序是通过在完整细胞或组织内进行边合成边测序的成像方法，以揭示每个细胞内的目标核苷酸序列。这些目标序列通常是条形码，用于指示引入细胞的扰动（如CRISPR向导或遗传变异）。然而，处理原位测序数据面临巨大挑战，需要对数以万计的图像与数百万个细胞进行拼接和对齐，跨测序循环检测微小的扩增子克隆，并进行读段识别。为应对这些挑战，我们开发了STARCall（原位测序的图像拼接、对齐与读段识别）软件包，它分析原始原位测序图像，生成每个细胞的基因型-表型映射。STARCall将图像拼接和对齐合并为一个步骤，最大限度地减少了循环间和循环内的对齐误差，改进了先前方案。STARCall还通过加入过滤和归一化来抑制背景荧光信号，提升了测序读段的检测和提取。我们使用包含常见成像问题（如通道和循环间强度变化、高背景）的多样化图像集，将STARCall与其他方法进行了比较，具体包括约25万张来自~3,500个条形码标记的LMNA变体（在U2OS细胞中表达）以及~1,200个条形码标记的PTEN变体（在诱导多能干细胞iPSC和iPSC衍生神经元中表达）的混合筛选图像。总体而言，在所有九个图像集上，STARCall对超过50%的图块实现了小于1像素的残余未对齐，性能比其他软件包高出14-35%。由于改进的过滤和归一化方法有效应对了背景荧光，STARCall使基因分型细胞数量增加了8-40%。STARCall可以调用CellPose等工具进行细胞分割，并利用CellProfiler从表型图像中计算细胞特征。STARCall是开源且免费可用的，为原位测序数据分析提供了稳健的解决方案。


### 核心发现


- STARCall将图像拼接与对齐整合为单一步骤，最小化循环间和循环内的对齐误差。

- 通过改进的过滤和归一化方法，有效抑制背景荧光，基因分型细胞数提升8-40%。

- 在九组图像上，超50%区域实现对亚像素级对齐，性能比其他工具高14-35%。
...
## 35. Systems biology analysis of vasodynamics in mouse cerebral arterioles during resting state and functional hyperemia.
...
本研究采用系统生物学方法，探讨被动、肌源性、神经源性和星形细胞反应之间的相互作用如何决定小鼠脑小动脉的血管动力学。提出了一个神经血管耦合模型，其中神经元抑制并减弱肌源性反应以促进激活时的血管舒张，并通过释放神经肽Y促进激活后快速血管收缩。还引入了胶质血管耦合模型，星形细胞通过其终足与小动脉的相互作用，在检测到肌源性活动增强时周期性抑制该反应。模拟显示，静息状态下肌源性自动调节的延迟可内生地产生低频（约0.1赫兹）血管直径振荡（血管运动），无需外部神经或节律性输入；而在激活状态下，这些振荡被神经源性和星形细胞反应所干扰。该小动脉血管动力学的生物物理模型为脑血管健康诊断和血流动力学神经成像的定量分析奠定了基础。


### 核心发现


- 提出神经血管耦合模型：神经元抑制肌源性反应以促血管舒张，并通过神经肽Y促激活后收缩。

- 提出胶质血管耦合模型：星形细胞通过终足感知肌源性活动增强来周期性抑制该反应。

- 静息状态肌源性延迟可内生低频血管振荡，而功能性充血时该振荡被神经和星形细胞反应干扰。
