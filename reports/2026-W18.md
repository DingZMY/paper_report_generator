...
> **收录论文**：49 篇（已翻译 48 篇）
> **生成时间**：2026-04-27 09:03 UTC

---



## 1. Engineered conductive pili enable high-efficiency photosynthetic electron extraction in biophotovoltaics.
...
## 2. Dango: Predicting higher-order genetic interactions.
...
## 3. Engineered commensals for metabolic modulation of the gut-liver-brain axis.
...
## 4. Condensation-independent intramodular translocation mechanism of the trans-AT polyketide synthase assembly line.
...
## 5. PinkyCaMP: an mScarlet-based calcium sensor with enhanced brightness, photostability and multiplexing capabilities.
...
## 6. A combinatorial domain screening platform reveals epigenetic effector interactions for transcriptional perturbation.
...
## 8. CRISPRi-based functional genomic screening identifies genes essential for CH4-dependent growth in a methanotrophic bacterium.
...
## 9. Automating population construction and parallel simulation of biophysical models for neuromuscular cells: An inverse approach.
...
## 10. The dual molecular identity of vestibular kinocilia bridges structural and functional traits of primary and motile cilia.
...
## 11. Construction and Application of a Multicolor Bactosensor for Detecting Bioavailable Antibiotic Mixtures in the Environment.
...
## 12. Biomimetic metamaterial-based interface for decoding heterogeneous mechanodermal activity.
...
## 13. Piezo1-mediated mechanohydraulic control of cell volume drives cardiac morphogenesis.
...
## 14. Dual role of ZIC2 during neural induction: from priming transcription factor to enhancer activator.
...
## 15. De novo chemo-optogenetics through the rational design of photoresponsive molecules and selection of their artificial protein binding pairs.
...
## 16. Genome editing of phylogenetically distinct bacteria using cross-species retron-mediated recombineering.
...
## 17. Spatiotemporal asymmetries on brain energy landscape uncover system entrapment related to depression severity.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-Apr-23 |
| **作者** | Kilic B Ülgen, Jubeir Jenna, Balchandani Priti, Murrough James W, Morris Laurel S, Jacob Yael |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-71961-4) |
| **DOI** | `10.1038/s41467-026-71961-4` |
...
静息态大规模脑活动被认为在由潜在结构连接塑造的离散类吸引子功能构型间游走，但重度抑郁症（major depressive disorder, MDD）中状态转换如何受到扰动仍不清楚。本研究整合静息态功能成像与弥散加权结构连接，在患有和未患有重度抑郁症的个体中识别稳定且重复出现的全脑共激活模式。更频繁且短暂地访问以显著性网络（salience network）为主导的构型与抑郁症中的快感缺失（anhedonia）相关。此外，视觉-注意与边缘-默认模式构型间的状态转换减少与更严重的抑郁症状相关，表明认知僵化，并表现为尽管存在结构便利的替代路径，但仍偏好能量成本较高的轨迹。我们的发现表明，动态特性、转换频率和能量不对称性的改变定义了一个失衡的状态转换景观，其中抑郁大脑被困在源型和汇型吸引子（source- and sink-like attractors）之间，为抑郁症中异常状态轨迹提供了机制性解释。


### 核心发现


- 抑郁症患者更频繁且短暂地访问显著性主导脑功能构型，与快感缺失症状密切相关

- 视觉-注意与边缘-默认模式网络间状态转换减少与症状严重程度相关，反映认知僵化特征

- 抑郁大脑在能量景观中陷入源-汇型吸引子间的俘获状态，表现为高能耗轨迹偏好
...
## 18. Yeast MoClo Secretion and Surface Display Toolkit 2.0: Improvements and Applications for Analysis of protein-protein Interactions and Whole-Cell Biocatalysis.

| 字段 | 内容 |
|---|---|
| **期刊** | ACS Synth Biol |
| **发表日期** | 2026-Apr-23 |
| **作者** | Jurić Vanja, Erwin Leah G, O'Riordan Nicola M, Maher Eamonn, Holmes Justin D, Young Paul W |
| **原文链接** | [查看原文](https://doi.org/10.1021/acssynbio.6c00085) |
...
酿酒酵母是基础生物学研究和生物技术应用（包括重组蛋白生产以及蛋白和代谢工程）中极其宝贵的模式生物。我们此前开发了一种基于模块化克隆（MoClo）的酿酒酵母工具包，可促进信号肽和锚定蛋白的快速优化，以高效分泌和/或表面展示目标异源蛋白。本文描述了该MoClo酵母分泌与展示（MoClo YSD）工具包的进一步改进和应用。我们介绍了编码新型锚定蛋白的部件：基于N端与截短Aga1融合以及C端与Aga2融合的锚定蛋白，每种均提供三种可选的表位标签。我们还添加了可通过GFP荧光互补实现目标分泌蛋白高通量检测的部件，以及编码"分泌增强"酵母蛋白的部件——此前报道称过表达这些蛋白可增强异源蛋白分泌。此外，本文展示了该工具包在表面展示方面的两项应用。我们证明，酵母表面展示抗GFP纳米抗体可通过流式细胞术或基于酵母的免疫共沉淀，对GFP标记的目标蛋白相互作用进行成本效益评估。此外，以酵母细胞作为全细胞催化剂，我们展示了聚对苯二甲酸乙二醇酯（PET）降解酶——叶枝堆肥角质酶与疏水蛋白1的共展示可增强PET塑料的分解，而这两种蛋白与MHETase的三重共展示则可将中间产物单羟基乙基对苯二甲酸酯（MHET）完全转化为对苯二甲酸。本文所述的多样化应用展示了升级版MoClo YSD工具包2.0在合成生物学及其他研究领域的广泛应用前景。


### 核心发现


- 开发新型锚定蛋白模块（N端截短Aga1/C端Aga2）及分泌增强元件，拓展工具包功能

- 建立基于抗GFP纳米抗体表面展示的低成本蛋白互作分析平台，支持流式细胞术与免疫共沉淀

- 构建多酶共展示全细胞催化剂，实现PET塑料高效降解及中间产物MHET完全转化为对苯二甲酸
...
## 19. Mechanical load inhibits cancer growth in mouse and human hearts.
...
## 20. TBL1X/TBL1XR1 govern β-cell identity through a PAX6-containing gene regulatory network.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-Apr-23 |
| **作者** | Walth-Hummel Alina A, Jouffe Celine, Weber Peter, Motzler Karsten, Geppert Julia, Sterr Michael, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-72077-5) |
...
糖尿病中β细胞功能障碍的主要机制是身份丧失，该过程由诱导身份基因表达并抑制disallowed基因的转录因子所调控。转录因子如何同时促进表达与抑制仍是糖尿病研究中的知识空白。本研究鉴定出转录辅因子转导素β样蛋白1 X连锁(TBL1X)及其同源物TBL1X相关蛋白(TBL1XR1，合称TBL/R1)是维持β细胞身份及决定糖尿病发生发展的关键因子。小鼠β细胞特异性敲除TBL/R1导致进行性低胰岛素血症和高血糖。单细胞RNA测序(scRNA-seq)显示敲除后出现β细胞丢失、多激素细胞涌现及成熟度下降。互作组筛选和染色质免疫共沉淀(ChIP)证实TBL/R1通过PAX6-HDAC3基因调控网络直接调控胰岛素启动子活性，且该机制在人类模型中得到验证。鉴于TBL/R1与人类糖尿病相关，本研究揭示了维持β细胞身份的关键调控新层级。


### 核心发现


- 鉴定TBL1X/TBL1XR1为维持β细胞身份的关键转录辅因子，决定糖尿病发生发展

- 敲除TBL/R1导致β细胞丢失、多激素细胞出现及成熟度下降，引发进行性高血糖

- 揭示TBL/R1通过PAX6-HDAC3网络直接调控胰岛素启动子，在人类模型中得到验证



**关键词**：Insulin-Secreting Cells · Animals · Mice · Mice, Knockout · PAX6 Transcription Factor · Gene Regulatory Networks · Humans · Histone Deacetylases · Insulin · Receptors, Cytoplasmic and Nuclear


---


## 21. Recurrent neural chemical reaction networks that approximate arbitrary dynamics.
...
## 22. Synthetic multicolor antigen-stabilizable nanobody platform for intersectional labeling and functional imaging.
...
## 23. A pro-carcinogenic bacterial toxin binds claudin-4 to cleave E-cadherin.

| 字段 | 内容 |
|---|---|
| **期刊** | Nature |
| **发表日期** | 2026-Apr-22 |
| **作者** | White Maxwell T, Wang Kang, Zhang Hailong, Eckhard Ulrich, Hullahalli Karthik, Chen Jason, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41586-026-10375-0) |
...
人体结肠定植着数万亿细菌，在人类健康和疾病中发挥重要作用。流行病学和实验研究表明，某些结肠细菌可促进结直肠癌的发生发展。其中，产肠毒素脆弱拟杆菌（enterotoxigenic Bacteroides fragilis）通过其单一毒素——脆弱拟杆菌毒素（BFT）驱动结肠肿瘤形成。BFT是一种金属蛋白酶，可结合结肠上皮细胞受体并导致E-钙粘蛋白（E-cadherin）胞外域切割，引起上皮屏障破坏、炎症和细胞增殖增加。然而，BFT受体身份未知，且BFT启动E-cadherin切割的分子机制尚不清楚。本研究通过全基因组CRISPR（成簇规律间隔短回文重复序列）筛选鉴定claudin-4为BFT受体，并证明claudin-4结合促进BFT介导的细胞表面E-cadherin切割。该工作不仅阐明了BFT的作用机制，也为开发抗BFT疗法开辟了新途径，可能对结直肠癌预防和急性产肠毒素脆弱拟杆菌感染治疗具有重要价值。


### 核心发现


- 通过全基因组CRISPR筛选鉴定claudin-4是BFT毒素的受体蛋白

- 阐明claudin-4结合促进BFT介导细胞表面E-cadherin切割的分子机制

- 为结直肠癌预防和急性感染治疗提供抗BFT疗法开发新策略
...
## 24. An in vitro approach for simulating divergent Golgi O-glycosylation of tumor-associated MUC1 from normal MUC1.
...
## 25. Integrating theory and machine learning to reveal determinants of plasmid copy number.
...
## 26. Spatio-temporal modelling of in vitro influenza A virus infection: The impact of defective interfering particles on the type I interferon response.
...
## 27. Using experimental results of protein design to guide biomolecular energy-function development.
...
## 28. A quantitative in vivo CRISPR-imaging platform identifies regulators of hyperplastic and hypertrophic adipose morphology in zebrafish.
...
## 29. Designing soft materials through synthetic morphogenesis.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-Apr-21 |
| **作者** | Bortnikov Evgeniy O, Paikar Arpita, Zhigileva Ekaterina A, Fomitskaya Polina, Ariel Noam, Diskin-Posner Yael, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-71999-4) |
| **DOI** | `10.1038/s41467-026-71999-4` |
...
尽管图灵型形态发生（Turing-type morphogenesis）的原理是理解生物图案形成的核心，但将其理性应用于合成材料设计仍面临重大挑战。为填补这一空白，我们利用含巯基（thiol）的小分子构建化学反应网络（chemical reaction network, CRN），理性设计定态反应-扩散图案——巯基是材料化学中普遍存在的功能基团。该CRN具有自催化特性，并与快速直接抑制及负反馈回路相耦合。借助数值模拟，通过调节反应物进料速率和浓度，我们获得了点状、线状和网状图案。利用二硫键交联的聚丙烯酰胺水凝胶（disulfide-crosslinked polyacrylamide hydrogels）调控巯基扩散，并将固定化的巯基衍生化为染料、酶和交联剂，从而制备出软材料。从非平衡自组织到图案化软材料的整个过程，在概念上类似于产生皮肤图案的生物学过程。总之，本研究建立了将图灵型自组织应用于合成物质结构化的途径。


### 核心发现


- 核心发现1：构建含巯基小分子的化学反应网络（CRN），耦合自催化与双重抑制机制，实现定态反应-扩散图案

- 核心发现2：通过调节反应物进料参数，在数值模拟辅助下可编程生成点、线、网状等多样定态图案

- 核心发现3：利用二硫键交联水凝胶固定图案并功能化，建立从非平衡自组织到图案化软材料的合成途径
...
## 30. Toward sustainable food preservatives: high-level production of sorbic acid in engineered Saccharomyces cerevisiae.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-Apr-21 |
| **作者** | Xiao Jianbin, Lin Wei, Chen Xingtong, Yu Haoyu, Chen Chao, Li Qin, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-72163-8) |
...
山梨酸（SA）及其盐类是全球最广泛使用且最安全的食物防腐剂之一，但其工业生产仍依赖化石燃料衍生的原料通过化学合成实现。本研究通过解析SA生物合成途径并对酿酒酵母（Saccharomyces cerevisiae）进行代谢工程改造，实现了SA的微生物发酵生产。研究从拟帚霉属（Myrothecium sp.）FJNU6中鉴定出SA及其酰胺衍生物山梨酰胺（SN），首次发现微生物来源的SA。基因组测序和异源表达揭示了SA/SN生物合成基因簇，包含高还原型聚酮合酶（SoaA）、水解酶（SoaB）和酰胺转移酶（SoaC）。为实现可持续高产，研究通过多层次工程策略在酿酒酵母中重构并优化SoaA-SoaB途径，包括动态启动子调控、乙酰辅酶A/丙二酰辅酶A途径增强、过氧化物酶体区室化以及两阶段补料分批发酵。这些策略在50 L生物反应器中实现了1.84 g/L的SA产量。本研究揭示了SA/SN的微生物生物合成途径，建立了SA生产的微生物平台，为开发化石基制造的可持续替代方案奠定基础。


### 核心发现


- 从拟帚霉属真菌中首次鉴定山梨酸生物合成基因簇及其聚酮合酶SoaA。

- 在酿酒酵母中重构SoaA-SoaB途径，50L发酵罐产量达1.84 g/L。

- 整合动态启动子调控、前体供应增强与过氧化物酶体区室化实现高效合成。
...
## 31. Effects of sensorimotor delays and muscle force capacity limits on the performance of feedforward and feedback control in animals of different sizes.

| 字段 | 内容 |
|---|---|
| **期刊** | PLoS Comput Biol |
| **发表日期** | 2026-Apr-21 |
| **作者** | Mohamed Thangal Sayed Naseel, More Heather L, Remy C David, Donelan J Maxwell |
| **原文链接** | [查看原文](https://doi.org/10.1371/journal.pcbi.1012502) |
...
动物依赖前馈和反馈控制应对扰动。比较不同体型陆生哺乳动物时发现，影响扰动响应的多个特征随体型变化——体型越大，感觉运动时间延迟越长、身体节段越重、肌肉相对越弱。本研究利用简单计算模型，探究快速扰动响应的控制如何受感觉运动延迟和肌肉力量容量这两种限制的约束。我们构建了两种任务模拟动物运动中的典型扰动响应场景：分布式质量摆模拟摆动肢体重定位（摆动任务），倒立摆模拟全身姿势恢复（姿势任务）。首先，通过标准化反馈控制系统证明，反馈响应时间或受肌肉力量生成容量限制（力量受限），或受感觉运动延迟限制——后者制约了产生稳定响应的最大反馈增益（延迟受限）。随后，使用涵盖陆生哺乳动物完整体型范围的详细缩放模型，根据文献中的缩放关系参数化感觉运动延迟、最大肌肉力量及惯性特性。结果显示，跨越所有体型和两种任务，反馈控制主要受延迟限制，最快响应仅利用部分肌肉力量容量。与最快前馈控制策略比较发现，前馈响应时间在小动物中比反馈控制快约4倍，在大动物中快约2倍。对于快速扰动响应，反馈控制对所有体型陆生哺乳动物均无效，因其模拟的最快响应时间超过了可用运动时间。因此，前馈控制策略——包括预期调整、弹道运动程序及利用内在肌肉骨骼动力学——对陆生哺乳动物快速应对突发大幅度扰动可能至关重要。


### 核心发现


- 反馈控制主要受感觉运动延迟限制，最快响应仅利用部分可用肌肉力量容量

- 前馈控制响应速度显著快于反馈控制，小动物快4倍，大动物快约2倍

- 快速扰动下反馈控制对所有体型动物均无效，前馈控制策略至关重要
...
## 32. Orderly mitosis shapes interphase genome architecture.

| 字段 | 内容 |
|---|---|
| **期刊** | Elife |
| **发表日期** | 2026-Apr-21 |
| **作者** | Guin Krishnendu, Keikhosravi Adib, Chari Raj, Pegoraro Gianluca, Misteli Tom |
| **原文链接** | [查看原文](https://doi.org/10.7554/eLife.108410) |
| **DOI** | `10.7554/eLife.108410` |
...
基因组在间期细胞核中呈现复杂的三维结构。然而，决定全局基因组结构的分子机制目前仍知之甚少。为鉴定高阶基因组组织的机制，我们在多种人类细胞系中开展了基于高通量成像的CRISPR敲除筛选，靶向1064个编码核蛋白的基因。我们以单细胞分辨率评估了着丝粒分布的变化，将其作为全局基因组组织的替代标志物。筛选发现了着丝粒空间分布的多个主要调控因子，包括核仁、动粒（kinetochore）、黏连蛋白（cohesins）、凝缩蛋白（condensins）和核孔复合体的组分。着丝粒分布的改变需要细胞周期进程，急性耗竭具有不同功能的有丝分裂因子会改变随后间期中的着丝粒分布。这些结果鉴定了着丝粒空间组织的分子决定因素，并表明有序的有丝分裂进程塑造了间期基因组结构。


### 核心发现


- 通过CRISPR筛选鉴定了核仁、动粒、黏连蛋白等着丝粒空间分布的关键调控因子

- 有丝分裂因子的急性耗竭会导致随后间期着丝粒分布的显著改变

- 有序的有丝分裂进程是塑造间期基因组三维架构的重要机制



**关键词**：Humans · Mitosis · Interphase · Centromere · Genome, Human · Cell Line


---


## 33. Protein Scaffold-Driven Enzyme Colocalization Combined with Glucose Derepression in Candida tropicalis Enables High-Level 1,2,4-Butanetriol Production.

| 字段 | 内容 |
|---|---|
| **期刊** | ACS Synth Biol |
| **发表日期** | 2026-Apr-21 |
| **作者** | Wei Bo, Ma Xiaohan, Shen Wei, Yang Haiquan, Zhou Li, Xia Yuanyuan, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1021/acssynbio.6c00134) |
| **DOI** | `10.1021/acssynbio.6c00134` |
...
## 34. Astral architecture can enhance mechanical strength of cytoskeletal networks by modulating percolation thresholds.

| 字段 | 内容 |
|---|---|
| **期刊** | Biophys J |
| **发表日期** | 2026-Apr-21 |
| **作者** | Berg Brady, Allard Jun |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.bpj.2026.03.028) |
| **DOI** | `10.1016/j.bpj.2026.03.028` |
...
细胞骨架结构中的一种重复模式是星状体（aster），其中多条F-肌动蛋白（F-actin）丝从中心节点呈星形发出。基于星状体的结构存在于细胞质肌动蛋白、酵母细胞分裂环的早期阶段以及仿生材料工程背景下。在本研究中，我们利用计算模拟证明，即使F-肌动蛋白密度固定，每个星状体仍存在最优丝束数可使刚性最大化。这种非线性依赖关系同时适用于剪切模量和延伸模量。在生理参数条件下，该最大值与近期超分辨率成像中观察到的皮质F-肌动蛋白每个星状体的丝束数大致相符。此外，我们发现增加每个星状体的丝束数会导致网络刚性样本间变异显著增大。我们利用渗流（percolation）理论解释这两种效应，其中给定网络有效连通的概率对参数表现出锐化依赖。网络刚性对这种纳米尺度结构特征的依赖性可能提示细胞通过局部快速调节（无需组装新的F-肌动蛋白）其肌动蛋白网络物理性质的机制，并可为创建受肌动蛋白网络启发的自适应合成超材料提供参考。


### 核心发现


- 星状体结构存在最优丝束数使刚性最大化，该最优值与生理条件下皮质F-actin超分辨率观测结果相符

- 剪切模量和延伸模量均表现出对丝束数的非线性依赖，且不随F-actin密度固定而消失

- 增加丝束数会显著增大网络刚性的样本间变异，此现象可通过渗流理论中连通概率的锐化依赖解释



**关键词**：Actins · Biomechanical Phenomena · Cytoskeleton · Actin Cytoskeleton · Mechanical Phenomena · Models, Biological


---


## 35. Impact of red blood cell rigidity on in vivo flow dynamics and lingering in bifurcations.

| 字段 | 内容 |
|---|---|
| **期刊** | Biophys J |
| **发表日期** | 2026-Apr-21 |
| **作者** | Rashidi Yazdan, Maurer Felix, Wrublewsky Selina, Larhrissi Khadija, John Thomas, Gidley Frances B, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.bpj.2026.03.023) |
| **DOI** | `10.1016/j.bpj.2026.03.023` |
...
分叉是哺乳动物微血管系统的关键组成部分，构成了血液与组织间的界面。红细胞（red blood cells, RBCs）作为血液的主要细胞成分，其柔韧性被认为强烈影响细胞在分叉处的分配，但定量体内测量迄今难以实现。本研究通过比较人工刚性化红细胞与健康红细胞在体内的运动，探讨细胞刚性对小动脉分叉后横向运动及滞留（lingering）现象的影响。滞留是指红细胞在进入任一分支前暂居于两分支间分叉区域的近期发现的现象。结果表明，红细胞刚性增加会减少滞留；与预期相反，在某些微循环区域，健康红细胞的速度反而低于刚性化红细胞。我们得出结论：刚性红细胞表现出不同的流动行为，导致血管壁接触减少（可能改变内皮信号传导和营养输送），并在分叉处呈现不同的分配模式。


### 核心发现


- 红细胞刚性增加显著减少其在血管分叉处的滞留现象。

- 与预期相反，健康红细胞在部分微循环区域的速度低于刚性化红细胞。

- 刚性红细胞具有不同的流动行为和分配模式，可能改变内皮信号传导和营养输送。



**关键词**：Erythrocytes · Animals · Humans · Microcirculation


---


## 36. Protein-free membrane fusion: A refined view of the delicate fusogenic properties of calcium.

| 字段 | 内容 |
|---|---|
| **期刊** | Biophys J |
| **发表日期** | 2026-Apr-21 |
| **作者** | Accorsi Matilde, Ní Earchaí Rachel, Yandrapalli Naresh, Pramanik Shreya, Dimova Rumiana |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.bpj.2026.03.040) |
...
20世纪末，当发现钙离子可诱导阴离子大单层囊泡（large unilamellar vesicles, LUVs）融合时，钙被赋予了独立促融合剂（fusogen）的身份，但其驱动细胞尺度膜融合的能力仍知之甚少。本研究利用微流控捕获平台结合共聚焦显微镜，直接定量测定钙离子介导的巨型单层囊泡（giant unilamellar vesicles, GUVs）融合，实现了对数百个单囊泡的脂质混合、内容物混合及融合结局的同步测量。我们系统绘制了融合效率随钙离子浓度、膜组成及机械张力变化的图谱。研究发现，无蛋白质条件下钙离子诱导的 GUVs 融合具有显著的易变性和组成敏感性：囊泡需足够不稳定以允许融合孔开放，同时又需足够稳定以防止破裂和塌陷。含高比例 DOPE（二油酰磷脂酰乙醇胺）的负电荷 GUVs 表现出最强的促融合响应性，而其他组成仅发生广泛脂质混合而无孔形成。增加膜张力可改变这一平衡并促进完全融合，揭示了钙离子作为细胞尺度膜有效无蛋白质促融合剂的狭窄参数空间。这些发现澄清了基于 LUV 和 GUV 的钙融合测定之间长期存在的差异，并为在合成生物学和膜重组研究中应用钙作为促融合剂提供了定量设计规则——在这些研究中，受控膜生长、囊泡-囊泡融合及模块整合是构建和维持人工细胞的核心。


### 核心发现


- 利用微流控平台定量测定钙离子介导的巨型单层囊泡（GUVs）融合效率，同步监测脂质混合、内容物混合及融合结局。

- 钙离子诱导的蛋白质无关融合高度敏感且依赖膜组成：高 DOPE 含量的负电荷囊泡最易形成融合孔，其他组成仅发生脂质混合。

- 增加膜张力可促进完全融合，揭示了钙离子作为有效无蛋白质促融合剂（fusogen）的狭窄参数空间及定量设计规则。



**关键词**：Calcium · Membrane Fusion · Unilamellar Liposomes


---


## 37. Computational rheometry for modeling viscoelasticity and mechanical responses of biomolecular condensates.

| 字段 | 内容 |
|---|---|
| **期刊** | Biophys J |
| **发表日期** | 2026-Apr-21 |
| **作者** | Zhang Ruoyao, Mitra Gaurav, Ghosh Souradeep, Pappu Rohit V |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.bpj.2026.03.037) |
| **DOI** | `10.1016/j.bpj.2026.03.037` |
...
生物分子凝聚体是一类粘弹性材料，表现出组分特异性的流变学特性及对外部机械力的响应。对于由内在无序蛋白（intrinsically disordered proteins）和多价核酸形成的凝聚体，研究者们曾利用粗粒化模拟（coarse-grained simulations）得到的结构，通过基于图的介观内部结构描述，结合广义Rouse模型提取粘弹性模数。该模型依赖于从计算获得的凝聚体特异性图中导出图拉普拉斯（graph Laplacian）的特征值。本文引入了"计算流变学"（computational rheometry）的理论框架，通过桥接分子尺度与连续介质尺度，实现对材料特性的机制性理解。在计算流变学中，凝聚体被表征为网络图，其中每个节点代表一个与Stokes流体耦合的Maxwell单元，从而构建出Stokes-Maxwell单元网络。本文阐述了该理论框架及其在模拟粗粒化模拟所得凝聚体图的力学响应中的应用。通过小振幅振荡剪切测试，我们成功重现了被动微流变学（passive microrheology）探测报道的凝聚体频率依赖性响应全谱。计算流变学还可用于模拟不同形式的蠕变测试（creep tests）。我们调整并实施了这些蠕变测试，其结果在近期实验背景下进行了讨论。总体而言，本研究确立了计算流变学作为桥接不同长度与时间尺度的方法，用以评估分子尺度相互作用与动力学如何引发介观尺度的粘弹性响应。


### 核心发现


- 建立了计算流变学框架，将凝聚体建模为Stokes-Maxwell单元网络，桥接分子与连续介质尺度

- 通过小振幅振荡剪切测试重现了凝聚体全频谱粘弹性响应，与被动微流变实验结果一致

- 实现了多种蠕变测试模拟，揭示分子相互作用如何产生介观尺度粘弹性响应的计算方法



**关键词**：Viscosity · Elasticity · Rheology · Biomolecular Condensates · Biomechanical Phenomena


---


## 38. Disruption of nuclear-cytoskeletal connection impairs epithelial cell mechanosensing and collective migration.

| 字段 | 内容 |
|---|---|
| **期刊** | Biophys J |
| **发表日期** | 2026-Apr-21 |
| **作者** | Shaji Akash T, Pathak Amit |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.bpj.2026.03.030) |
| **DOI** | `10.1016/j.bpj.2026.03.030` |
...
核骨架与细胞骨架连接物（LINC）复合物介导细胞骨架与细胞核之间的物理通讯，从而影响细胞核定位、转录反应和细胞迁移。尽管先前研究已探讨LINC在单细胞迁移中的作用，但其对上皮细胞在不同硬度表面上集体迁移的影响仍不清楚。本研究利用可诱导的显性负性SUN1构建体（SUN1L）隔离nesprins，从而破坏MCF10A乳腺上皮细胞中的LINC功能。结果发现，在涂覆胶原蛋白的0.1、5和50 kPa聚丙烯酰胺水凝胶上，表达SUN1L的MCF10A细胞比野生型对照组迁移更慢。虽然对照组和SUN1L细胞的迁移速度在较硬基质上均有所增加，但SUN1L细胞表现出较低的硬度敏感性，表明LINC破坏后机械感应受损。LINC破坏还改变了细胞和组织尺度的力学特性：SUN1L细胞更小、更软，并形成更致密的上皮片层。伴随这些物理变化，LINC破坏的细胞显示出更强的细胞-基质粘附和降低的肌动蛋白-肌球蛋白（actomyosin）收缩性。细胞骨架扰动进一步揭示SUN1L细胞对actomyosin活性的依赖性降低。这些发现强调了核-细胞骨架解耦如何触发多尺度物理变化——细胞软化、拥挤和粘附动力学改变——从而共同损害机械敏感性迁移。鉴于LINC与层蛋白病（laminopathies）和癌症的关联，本研究为其功能障碍如何影响疾病进展和集体细胞行为提供了新见解。


### 核心发现


- LINC复合物功能破坏显著降低乳腺上皮细胞在不同硬度基质上的迁移速度并削弱其机械敏感性。

- 核-细胞骨架解耦导致细胞软化、体积缩小、上皮片层致密化及细胞-基质粘附增强等多尺度物理改变。

- LINC破坏降低细胞对肌动蛋白-肌球蛋白收缩力的依赖性，改变细胞迁移的力学调控机制。



**关键词**：Humans · Cell Movement · Epithelial Cells · Cytoskeleton · Cell Nucleus · Mechanotransduction, Cellular · Cell Line · Actomyosin · Microtubule-Associated Proteins · Nuclear Proteins


---


## 39. Domain interactions in neuronal nitric oxide synthase identified by time-resolved fluorescence.

| 字段 | 内容 |
|---|---|
| **期刊** | Biophys J |
| **发表日期** | 2026-Apr-21 |
| **作者** | Snyder Alexa A, McCoy Christian L, Gambill Alexandria K, Guadalupe Karina, Bailey Sheila K, Catlin Audrey E, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.bpj.2026.03.022) |
| **DOI** | `10.1016/j.bpj.2026.03.022` |
...
神经元型一氧化氮合酶（NOS）通过还原酶域中FAD向FMN的电子传递，以及随后FMN向同源二聚体伙伴酶加氧酶域血红素的电子传递，催化一氧化氮生成。钙信号蛋白钙调蛋白（CaM）结合于还原酶域与加氧酶域之间的CaM结合域以激活NOS。NOS的定向电子传递通过形成使电子供受体域依次靠近的构象状态实现，表明该过程受构象门控。本研究利用荧光标记的CaM和时间分辨荧光检测，揭示了NOS存在多种构象状态。连接于CaM的荧光团向酶血红素基团的FRET（荧光共振能量转移）效率取决于酶的构象状态。通过对NOS和CaM定点突变体的荧光衰减分析，我们将荧光猝灭状态归因于特定构象状态。结果表明存在FAD与FMN亚域靠近的构象、FMN亚域与伙伴酶加氧酶域互作的构象，以及CaM与加氧酶域靠近的多种状态。CaM与加氧酶域的对接可促进FMN域与其二聚体伙伴的有效互作，从而实现从FMN向伙伴酶血红素的电子传递。


### 核心发现


- 利用时间分辨荧光FRET技术揭示了神经元型一氧化氮合酶（nNOS）存在多种构象状态

- 鉴定出FAD-FMN亚域靠近及FMN与伙伴酶加氧酶域互作的特定功能构象

- 发现钙调蛋白（CaM）与加氧酶域对接可促进FMN域向二聚体伙伴血红素的电子传递



**关键词**：Calmodulin · Nitric Oxide Synthase Type I · Protein Domains · Fluorescence Resonance Energy Transfer · Protein Binding · Time Factors · Animals · Models, Molecular


---


## 40. Asymmetry-induced transient gel formation in fluid lipid membranes.

| 字段 | 内容 |
|---|---|
| **期刊** | Biophys J |
| **发表日期** | 2026-Apr-21 |
| **作者** | Pirhadi Emad, Yong Xin |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.bpj.2026.03.025) |
| **DOI** | `10.1016/j.bpj.2026.03.025` |
...
组成不对称性是细胞膜的决定性特征，控制着膜渗透性、蛋白质活性、胆固醇动态及形状重塑。这种不对称性可产生应力失衡，使两个单层（leaflets）经历相反张力，尽管单层应力的直接实验测量仍具挑战性。这种应力失衡可压缩某一单层并触发流体-凝胶相变，从而降低膜流动性并显著增加弯曲刚性。这些现象引出一个关键问题：在跨越转变阈值之前膜如何机械响应，这一状态范围仍与生物功能密切相关。在此，我们结合全面的全原子（all-atom）和粗粒化（coarse-grained）分子动力学模拟，研究应力不对称如何调节接近转变点的膜结构和力学。以POPE和DLPC双层作为模型系统，我们发现适度不对称诱导瞬态凝胶样结构域持续形成和溶解，放大波动并降低双层刚性。超过凝胶化阈值后，趋势逆转且双层变硬，导致刚性对不对称性呈非单调依赖。此外，我们的结果揭示流体相和凝胶相具有不同的曲率偏好。将此分析扩展到多组分细菌外膜，我们证明应力不对称即使在复杂脂质混合物中也能触发瞬态凝胶样结构域形成。这为差异应力通过诱导软化或硬化来调节膜力学提供了原理验证，补充了分子组成的影响。我们的发现阐明了细胞如何利用应力-曲率-相耦合在近生理条件下调节膜刚性。


### 核心发现


- 适度不对称诱导瞬态凝胶样结构域持续形成与溶解，降低膜弯曲刚性并放大波动

- 膜刚性随不对称性呈非单调依赖：超过凝胶化阈值后由软化转为硬化

- 在多组分细菌外膜中证实应力不对称可触发瞬态凝胶域，验证该机制的普适性



**关键词**：Gels · Lipid Bilayers · Molecular Dynamics Simulation · Membrane Fluidity · Phosphatidylcholines · Phosphatidylethanolamines · Cell Membrane


---


## 41. Nonuniform filament turnover, contractility, and bundle formation in disordered actomyosin networks.
...
| **原文链接** | [查看原文](https://doi.org/10.1016/j.bpj.2026.02.032) |
...
我们利用数学建模与模拟研究了丝状体周转（filament turnover）及力学特性如何影响无序肌动蛋白-肌球蛋白（actomyosin）网络中的收缩性（contractility）与束形成（bundle formation）。通过建立二维基于智能体（agent-based）的肌动蛋白-肌球蛋白网络模型，我们探究了四种简化的丝状体周转模式：均匀周转、偏置周转、分支（branching）和踏车（treadmilling）。在无周转条件下，随着时间推移，网络收缩性降低而束形成增加，最终形成无法收缩的静止模式。引入周转机制可使收缩性较无周转情形维持更久。均匀周转（新丝状体具有随机位置和方向）破坏束形成并实现持续收缩。在偏置周转、分支和踏车模型中，新丝状体的位置和方向依赖于现有网络结构。这些非均匀周转模型较均匀周转增加束形成，同时仍允许长期收缩。70°分支破坏束形成以实现长期收缩，而丝状体踏车运动打破束形成与收缩性之间的权衡。偏置周转将新丝状体置于现有丝状体附近，促进束形成但在维持收缩性方面效果较差。模拟显示，更易弯曲的丝状体加速束形成，而更高的有效交联摩擦（cross-linking friction）增强收缩性。我们的结果表明，细胞可通过调节肌动蛋白周转途径来调控无序肌动蛋白-肌球蛋白网络中的收缩性与束形成。


### 核心发现


- 非均匀周转模式在维持网络收缩性的同时促进束形成，打破束形成与收缩性的传统权衡。

- 70°分支通过破坏束形成延长收缩时间，踏车运动打破束形成与收缩性之间的权衡。

- 丝状体可弯曲性加速束形成，而交联摩擦增强收缩性，为细胞力学调控提供独立物理参数。



**关键词**：Actomyosin · Models, Biological · Actin Cytoskeleton · Biomechanical Phenomena · Muscle Contraction


---


## 42. Phased fragility and stability of non-genetic B cell states in the germinal center accelerate the genetic evolution of antibodies.
...
疫苗反应依赖于B细胞的达尔文式遗传演化以产生高亲和力抗体。然而，B细胞在搜寻抗原和T辅助细胞（T helper cells）过程中获得非遗传异质性，但在增殖性克隆扩增（clonal bursts）期间其非遗传细胞状态保持稳定。我们通过整合大量免疫学知识构建数学模型，探索了这种非遗传变异性动态调控的功能意义。研究发现，B细胞命运决定的变异性非但不损害，反而通过允许高亲和力离群细胞（outliers）逃逸浆细胞分化并启动新一轮达尔文演化，从而加速亲和力成熟。在克隆扩增期间，非遗传细胞状态的稳定性进一步促进这些细胞的扩增。所得模型正确预测了B细胞命运决定特征改变的小鼠品系中涌现的疫苗反应特性。本研究调和了经典B细胞克隆选择理论与实验观察到的非遗传变异性，并为支持个性化疫苗策略提供了可解释的知识驱动建模框架。


### 核心发现


- B细胞命运决定的变异性非但不损害，反而通过允许高亲和力离群细胞逃逸浆细胞分化以启动新一轮达尔文演化，从而加速亲和力成熟

- 克隆扩增期间非遗传细胞状态的稳定性促进高亲和力离群细胞的进一步放大

- 该知识驱动的数学模型成功预测不同B细胞命运决定特征的小鼠品系中的疫苗反应特性，为个性化疫苗策略提供可解释框架
...
## 43. Systematic discovery of pro- and anti-HIV host factors in primary human CD4+ T cells.
...
促进或限制人类免疫缺陷病毒（HIV）感染人类CD4+ T细胞的宿主因子尚未被全面鉴定。本研究利用正交全基因组CRISPR激活（CRISPRa）和CRISPR敲除筛选，在原代CD4+ T细胞中系统发现促HIV和抗HIV宿主因子。二次筛选和单独扰动验证高置信度靶点并揭示多种作用机制。CRISPRa发现多个强效抗病毒因子，包括PI16、PPID、SHISA3和ITM2A。PI16与参与HIV融合的宿主因子相互作用并抑制病毒进入，而PPID（Cyp40，促病毒亲环蛋白CypA的旁系同源物）结合衣壳并减少HIV核心核输入。结构建模、进化分析和靶向诱变揭示了PPID介导HIV限制所需的结构域和残基，包括增强抗病毒活性的非人灵长类直系同源替换。这些数据共同定义了原代人T细胞中的功能性HIV-宿主相互作用图谱，并揭示调控感染的新机制。


### 核心发现


- 利用正交全基因组CRISPRa和CRISPR敲除筛选在原发性CD4+ T细胞中系统鉴定促/抗HIV宿主因子

- 发现PI16抑制病毒进入，PPID（Cyp40）结合衣壳并阻断HIV核心核输入

- 揭示PPID限制HIV的结构域、关键残基及非人灵长类增强型变异
...
## 44. Photoactivatable CRISPR/Cas13d via upconversion nanoparticles for deep tissue RNA engineering and orthopedic therapy.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-Apr-20 |
| **作者** | Zhao Jie, Zhang Jingyu, Gao Miaomiao, Miao Zukang, Zhang Yang, Guo Yue, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-72181-6) |
| **DOI** | `10.1038/s41467-026-72181-6` |
...
时空控制RNA治疗药物仍是限制临床转化的根本性挑战。本研究开发了光激活CRISPR/Cas13d（paCas13d）系统，可在深部组织中实现无创光控RNA操控。通过结构引导工程，我们在RfxCas13d中确定了最佳分裂位点，并利用CRY2PHR/CIBN光遗传二聚化创建光开关片段。为克服蓝光组织穿透性差的限制，我们设计了聚乙烯亚胺功能化的上转换纳米颗粒（UCNPs-PEI），其兼具基因载体和光子转换器功能，可将具有组织穿透性的近红外光（NIR）转换为蓝光。UCNPs-PEI@paCas13d系统在活体骨组织中实现了RNA靶向的精准时空控制。在类固醇相关骨坏死小鼠模型中，NIR激活的paCas13d有效敲低TET3，阻断了驱动糖皮质激素诱导骨细胞凋亡的TET3-5hmC-PTEN轴。该靶向干预阻止了骨退化，治疗组小鼠显示出保留的骨小梁结构、增强的骨体积及骨转换标志物的有利转变，同时维持系统性糖皮质激素疗效。本研究平台结合CRISPR/Cas13d的可编程性与无创光学控制，为需要局部RNA调控且最小化全身效应的疾病治疗提供了通用策略。


### 核心发现


- 开发光激活CRISPR/Cas13d系统，利用CRY2PHR/CIBN光遗传二聚化实现RNA靶向时空精准控制

- 设计上转换纳米颗粒(UCNPs-PEI)作为基因载体，将近红外光转换为蓝光以克服组织穿透限制

- 在骨坏死模型中实现TET3精准敲低，阻断TET3-5hmC-PTEN凋亡轴并恢复骨小梁结构与骨量
...
## 45. Multi-omic mapping of Drosophila protein secretomes reveals tissue-specific origins and inter-organ trafficking.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-Apr-20 |
| **作者** | Bosch Justin A, Beltran Pierre Michel Jean, Cavers Cooper, LaGraff James Thai, Melanson Randy, Singh Ankita, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-71763-8) |
| **DOI** | `10.1038/s41467-026-71763-8` |
...
分泌蛋白调控动物生物学的诸多方面，是极具吸引力的生物标志物和治疗靶点。然而，全面识别“分泌组”及其组织来源仍极具挑战性。为此，我们采用多种组学方法，绘制了黑腹果蝇（Drosophila melanogaster）535个血浆蛋白的组织-分泌组图谱，这些蛋白来源于特定细胞类型和器官。该图谱的建立得益于方法学改进，包括用于标记10种主要组织类型内源分泌蛋白的转基因果蝇品系集合、大规模血液分离、全动物单核RNA测序（snRNA-seq）以及40个CRISPR敲入品系。利用该图谱，我们鉴定了循环蛋白的特征：大多数源自特定组织，包括非常规来源（如胶质细胞）；许多尚未被表征；部分为跨膜蛋白脱落的胞外域。此外，体内实验揭示了具有组织特异性表达的循环蛋白，以及在其合成组织以外的组织中沉积的蛋白，提示潜在的器官间功能。我们的分泌组图谱将作为研究血液蛋白功能、发现候选组织-组织通讯信号以及挖掘人类生物标志物同源物的资源。


### 核心发现


- 整合多组学方法绘制了535个血浆蛋白的果蝇组织-分泌组图谱，涵盖10种主要组织类型的特异性来源

- 揭示多数循环蛋白源自特定组织（包括胶质细胞等非常规来源），部分为跨膜蛋白脱落的胞外域

- 通过体内实验发现蛋白存在器官间转运现象，即某些蛋白在合成组织以外的部位沉积，提示潜在的器官间通讯功能
...
## 46. SPNS2 exports sphingosine-1-phosphate and imports glucose.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-Apr-20 |
| **作者** | Weigel Cynthia, Hossen Md Lokman, Brown Ryan D R, Senkal Can E, Green Christopher D, Newton Jason, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-71659-7) |
...
Spinster同源蛋白2（SPNS2）将具有生物活性的鞘脂代谢物鞘氨醇-1-磷酸（S1P）输出细胞，以调控与健康和疾病相关的重要生理过程。然而，SPNS2转运功能的分子机制及其精确的生理作用尚未完全阐明。本研究通过小鼠模型、细胞实验以及特别是体外无细胞结合与转运实验等一系列互补方法，证明SPNS2具有反向转运蛋白（antiporter）样活性，可将S1P输出细胞并输入葡萄糖。我们证实SPNS2直接结合并转运葡萄糖，并鉴定了SPNS2中参与葡萄糖结合和导入的关键氨基酸残基。研究数据表明，从SPNS2细胞质侧进入的S1P促进构象变化，使细胞外葡萄糖能够通过中央腔向内移动。因此，我们揭示了一种动态响应代谢和鞘脂信号、参与葡萄糖稳态调控的机制，具有临床和病理生理学意义。


### 核心发现


- 揭示SPNS2作为反向转运蛋白同时输出S1P并输入葡萄糖的双重功能

- 鉴定SPNS2参与葡萄糖结合与转运的关键氨基酸残基

- 阐明S1P结合诱导构象变化促进葡萄糖跨膜转运的分子机制
...
## 47. Disruption of the SAGA CORE triggers collateral degradation of KAT2A.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-Apr-20 |
| **作者** | Batty Paul, Beneder Hannah, Schätz Caroline, Onea Gabriel, Zaczek Maciej, Kutschat Ana P, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-71613-7) |
| **DOI** | `10.1038/s41467-026-71613-7` |
...
Spt-Ada-Gcn5乙酰转移酶（SAGA）复合体通过其组蛋白乙酰转移酶（HAT）KAT2A介导的启动子组蛋白乙酰化来调控基因表达。尽管SAGA的结构和功能已被充分阐明，但调控各亚基（包括KAT2A）稳定性的机制仍不清楚。本研究利用基于荧光的KAT2A稳定性报告系统，系统解析了调控KAT2A蛋白丰度的分子依赖性，发现非酶促的SAGA CORE模块亚基——TADA1、TAF5L和TAF6L——对KAT2A的稳定性至关重要。这些亚基的缺失会破坏SAGA复合体完整性，导致KAT2A脱离染色质并被蛋白酶体降解，进而降低H3K9乙酰化水平。蛋白质组学分析显示，急性破坏SAGA CORE会导致CORE和HAT模块组分逐步丢失，表明完整的CORE对众多SAGA组分的稳定性是必需的。最后，针对泛素-蛋白酶体系统基因的CRISPR筛选鉴定出E3连接酶UBR5（已知孤儿蛋白降解调控因子）和去泛素化酶OTUD5，在SAGA CORE受损时调控KAT2A的降解。这些发现共同揭示了KAT2A蛋白稳定性依赖于SAGA CORE的完整性，并定义了针对未组装KAT2A的孤儿蛋白质量控制机制，为SAGA驱动的恶性肿瘤提供了潜在的治疗靶点。


### 核心发现


- 非酶促SAGA CORE亚基TADA1、TAF5L和TAF6L是维持KAT2A稳定性的关键因子

- CORE模块破坏导致SAGA复合体完整性丧失及游离KAT2A的蛋白酶体降解

- E3连接酶UBR5与去泛素化酶OTUD5协同调控孤儿KAT2A的降解过程



**关键词**：Histone Acetyltransferases · Humans · Proteolysis · Proteasome Endopeptidase Complex · Acetylation · Histones · Ubiquitin-Protein Ligases · HEK293 Cells · Proteomics · Trans-Activators


---


## 48. A standardized workflow for kinetic metabolic model curation and dissemination.

| 字段 | 内容 |
|---|---|
| **期刊** | PLoS Comput Biol |
| **发表日期** | 2026-Apr-20 |
| **作者** | Cook Margaret, Anastasakis Stella, Heydarabadipour Adel, Shin Janis, Burbano Diego Alba, Carothers James M, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1371/journal.pcbi.1014227) |
...
动力学代谢模型为细胞代谢提供了宝贵的见解，支持合成生物学、代谢工程和系统生物学等领域的应用。然而，这些模型的可重复性和实用性依赖于清晰严谨的文档记录、标准化的注释以及可及的可视化。本文提出了一个用于构建、注释、可视化和共享动力学代谢模型的标准化流程。该方法整合社区标准与开源工具，确保可重复性、互操作性和用户可及性。该流程使研究人员能够产出可重用且文档完善的动力学模型，推动其成为代谢研究中的有力工具。


### 核心发现


- 建立了涵盖构建、注释、可视化及共享的动力学代谢模型标准化整理流程。

- 整合社区标准与开源工具，系统性地保障模型的可重复性与互操作性。

- 显著提升了动力学代谢模型的可重用性与文档质量，强化其在代谢研究中的应用价值。
...
## 49. How individual vigor shapes human-human physical interaction.

| 字段 | 内容 |
|---|---|
| **期刊** | Elife |
| **发表日期** | 2026-Apr-20 |
| **作者** | Verdel Dorian, Berret Bastien, Burdet Etienne |
| **原文链接** | [查看原文](https://doi.org/10.7554/eLife.109781) |
| **DOI** | `10.7554/eLife.109781` |
...
自主运动的速度存在系统性差异，某些个体在不同动作中始终比他人更快。这些被概念化为活力（vigor）的变异反映了运动规划中的时间-努力-准确性权衡。两个具有不同个体活力的机械耦合伙伴如何协作，例如共同移动桌子？本研究表明，此类二人组以最小交互力协调目标导向动作，表现出具有个体活力相似特征的二元活力（dyadic vigor）。涌现的二元运动计划受较慢伙伴的强烈影响，其活力可预测二元活力，且该效应持续超越练习阶段。基于随机最优控制（stochastic optimal control）的计算建模揭示了伙伴动作时间不确定性与活力在塑造协调中的关键作用，使我们能够跨多种条件从个体行为预测二元动作。这些发现阐明了人类协作的潜在机制，可应用于物理训练、康复及制造业协作机器人等领域。


### 核心发现


- 机械耦合的二人组以最小交互力协调目标导向动作，涌现的二元活力具有个体活力相似特征

- 较慢伙伴的活力主导二元运动计划形成，其预测效应持久存在且不因练习而减弱

- 随机最优控制模型揭示伙伴动作时间不确定性与活力对协调的塑造机制，实现跨条件预测



**关键词**：Humans · Male · Female · Adult · Movement · Young Adult · Psychomotor Performance · Interpersonal Relations · Cooperative Behavior
