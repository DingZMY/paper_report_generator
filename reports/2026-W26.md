# 系统/合成生物学文献周报 2026-W26

> **时间范围**：2026/06/15 ~ 2026/06/22
> **来源期刊**：Nature / Cell / Science 系列
> **收录论文**：55 篇（已翻译 55 篇）
> **生成时间**：2026-06-22 00:51 UTC

---



## 1. Conformational Gating of an Active-Site Loop Governs Phenolic and Aromatic Amine Substrate Selectivity in Fungal Laccase.

| 字段 | 内容 |
|---|---|
| **期刊** | Biophys J |
| **发表日期** | 2026-Jun-19 |
| **作者** | Biswas Anushka, Singh Deepshikha, Datta Bhaskar, Radhakrishna Mithun |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.bpj.2026.06.021) |
| **DOI** | `10.1016/j.bpj.2026.06.021` |
...
漆酶可氧化多种芳香族化合物，但构象动力学如何调控底物识别与功能特异性仍不完全清楚。本研究结合长时间尺度分子动力学模拟、马尔可夫状态模型、过渡路径分析、自由能计算及等温滴定量热法，探究了一株真菌漆酶底物类别选择性的分子机制。以代表性酚类和芳香胺底物为例，发现进入T1铜中心的途径受一个柔性活性位点环调控，该环差异性调节结合路径。酚类配体需要此环打开才能获得空间可及的结合构型，并经历多个亚稳态中间体，符合构象选择-诱导契合偶联机制。而芳香胺无需环重排便可通过以构象选择为主的路径进入活性位点。计算机模拟中使该环刚性化会选择性地破坏酚类结合但保留胺结合，印证了蛋白质动力学所编码的机制分叉。质子化状态进一步调节结合能，与等温滴定量热法在酸性条件下不同酚类异构体间亲和力差异的结果一致。总之，这些发现确立了构象门控是一种将蛋白质动力学与漆酶中底物类别依赖性识别关联的结构机制，并为理解柔性活性位点元件如何调控氧化还原酶的功能特异性提供了分子层面的见解。


### 核心发现


- 活性位点柔性环通过构象门控区分酚类与芳香胺底物的结合路径。

- 酚类需环打开且经历多步亚稳态，遵循构象选择-诱导契合偶联机制。

- 环的刚性化选择性破坏酚类结合，质子化状态调节亲和力，揭示动力学驱动的特异性。
...
| **期刊** | Nat Biotechnol |
| **发表日期** | 2026-Jun-19 |
| **作者** | Ali Zahir, Butt Haroon, Alghamdi Raghad, Moreno Ramirez Jose Luis, Mahfouz Magdy |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41587-026-03181-6) |
...
精准整合数千碱基的DNA片段在植物中仍是一个主要技术障碍。本文引入非长末端重复（non-LTR）R2反转录转座子作为植物靶向基因整合的多功能系统。我们在本氏烟草中重建了R2活性，并利用基于烟草花叶病毒（TMV）的游离型报告系统评估其插入效率和保真度。结果表明，绿色荧光蛋白（GFP，2.2 kb）和重组酶兼容着陆垫（0.6 kb）可位点特异性整合到28S rDNA阵列中，完整表达盒插入频率分别高达75%和53%。为在时间上限制供体可用性并避免DNA中间体，我们将植物体内效应子表达与重组RNA病毒介导的供体递送相结合。进一步将R2反转录转座子用于水稻愈伤组织rDNA内抗性表达盒的靶向插入，整合效率达17%。这些结果确立了R2反转录转座子作为无DNA双链断裂、RNA模板化在rDNA位点插入数千碱基基因表达盒的系统，可用于植物安全港性状叠加，在作物改良和合成生物学中具有潜在应用价值。


### 核心发现


- 利用R2反转录转座子在烟草28S rDNA位点实现完整GFP表达盒75%的位点特异性整合。

- 通过结合效应子表达和重组RNA病毒递送供体，避免DNA中间体并实现时间控制。

- 在水稻愈伤组织rDNA位点靶向插入抗性表达盒效率达17%，展示作物改良潜力。
...
## 3. FKBP8 connects the Hsp70-Hsp90 chaperone machinery to the folding of membrane proteins.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-Jun-19 |
| **作者** | Ge Man-Xi, Wu Ming-Zhi, Ji Jia, Li Zhao-Peng, Bai Zhongjian, He Jieyan, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-74519-6) |
...
内质网表面膜蛋白胞质结构域的折叠以及它们与跨膜区、外质区的协调机制仍知之甚少。通过全基因组CRISPR-Cas9筛选，我们发现内质网锚定的FK506结合蛋白8（FKBP8）是膜蛋白折叠和组装所必需的分子伴侣。以ABC转运体为模型底物，我们证明FKBP8与Hsp70-Hsp90分子机器协同，将新生或错误折叠的胞质域重塑为天然构象。冷冻电镜分析显示，FKBP8利用保守的疏水簇ϕ94ϕ96ϕ97/ϕ158ϕ162，在FKBP8-Hsp90复合物中帮助形成一个巨大的底物结合腔，捕获折叠中间体。FKBP8缺失、该疏水簇破坏或疾病相关的FKBP8突变都会阻碍底物成熟，导致内质网滞留和降解。利用纯化组分进行重建实验表明，FKBP8和Hsp40-Hsp70-HOP-Hsp90构成了足以恢复错误折叠ABC转运体天然结构的最小分子机器。这些发现揭示了内质网-胞质界面上一套专一的折叠模块，将胞质分子伴侣与膜蛋白质量控制联系起来，提示FKBP8在复杂膜蛋白生物合成中具有广泛保障作用。


### 核心发现


- 全基因组筛选鉴定FKBP8为内质网膜蛋白折叠必需的锚定分子伴侣。

- FKBP8利用保守疏水簇与Hsp90形成捕获折叠中间体的大型底物结合腔。

- FKBP8与Hsp40-Hsp70-HOP-Hsp90构成恢复膜蛋白天然构象的最小折叠机器。
...
## 4. Notch coordinates self-organization of germ layers and axial polarity in sea anemone gastruloids.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-Jun-19 |
| **作者** | Narayanaswamy Sanjay, Haas Franziska, Haillot Emmanuel, Cole Alison G, Tanaka Elly M, Technau Ulrich |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-74441-x) |
...
海葵（Nematostella vectensis）早期原肠胚细胞再聚集形成的“胃状体”能够在数天内再生为完整的水螅体。然而，体轴和胚层重建背后的细胞与分子机制仍大部分未知。本研究中，我们发现中胚层细胞分选至胃状体外周，并在那里与搭车的内胚层细胞共同形成细胞簇。其中一个细胞簇在一极内迁，形成未来的咽部和内层。这一使组织者组织发生对称性破缺的形态发生行为，需要Wnt-Notch信号反馈环路。这揭示了Notch信号在自组织胃状体中一种迄今未知的作用。Notch介导的胚层间边界形成在两侧对称动物中存在保守性，反映出类似机制。这表明，祖先调控网络的应用使得形态空间收敛至相似的体型，从而促进了进化稳健性。


### 核心发现


- 中胚层细胞分选至外周并形成细胞簇，携带内胚层细胞，一簇在一极内迁触发对称破缺。

- 该形态发生过程依赖Wnt-Notch反馈环路，揭示Notch在自组织胃状体中的新功能。

- Notch介导的胚层边界形成机制在两侧对称动物中保守，体现祖先调控网络促进进化稳健。
...
## 5. Human macrophages encode stimulus-specific information of prior exposures through trained immunity.

| 字段 | 内容 |
|---|---|
| **期刊** | Cell Syst |
| **发表日期** | 2026-Jun-19 |
| **作者** | O'Farrell Aoife, Niu Zijian, Li Jingxin, Van Eyndhoven Laura C, Sarma Kavitha, Raj Arjun |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.cels.2026.101647) |
| **DOI** | `10.1016/j.cels.2026.101647` |
...
训练免疫（一种固有免疫记忆形式）部分定义为对病原体再刺激的增强反应，可由多种不同刺激产生。然而，不同刺激产生的训练状态在定量上的差异及其下游后果仍不清楚。本研究使用原代人单核细胞来源的巨噬细胞，展示了初始暴露6天后训练免疫的表型和分子刺激特异性。通过单分子RNA成像定量细胞因子产生，揭示了再刺激反应的特异性模式：训练后细胞对与初始刺激更相似的二次刺激表现出更强反应。训练后6天，炎症转录因子的差异性授权与染色质中特异性的编码有关，而体外训练后11天，部分（而非全部）训练刺激的记忆已丢失。总之，我们的发现表明，不同训练刺激可赋予特异性记忆，产生不同的训练表型。


### 核心发现


- 不同训练刺激诱导的训练免疫在表型和分子层面具有刺激特异性，而非通用反应。

- 训练巨噬细胞对与初始刺激更相似的二次刺激表现出更强的细胞因子反应。

- 染色质编码的转录因子差异性授权是记忆特异性的分子基础，且部分记忆在11天后消退。
...
## 6. Dynamic population coding of kinematic structure across executed and observed actions in primate premotor cortex.

| 字段 | 内容 |
|---|---|
| **期刊** | Sci Adv |
| **发表日期** | 2026-Jun-19 |
| **作者** | Chatzimichail Konstantinos, Paschalidis Christos, Tzamali Eleftheria, Papadourakis Vassilis, Raos Vassilis |
| **原文链接** | [查看原文](https://doi.org/10.1126/sciadv.aed9309) |
...
在动作执行和观察过程中均活跃的神经元[镜像神经元（MirNs）]是动作理解理论的核心，然而它们所表征的内容仍存在争议：是抽象目标、静态抓握类型，还是运动学特征？我们记录了猕猴前运动皮层在执行和观察伸手抓握动作时的433个神经元。群体分析揭示了两种条件下均存在抓握特异性信息，这些信息广泛分布于各神经元中，并随时间动态重构。跨任务阶段的泛化有限，表明群体编码是时间特异性和动态演化的，而非静态表征。执行与观察通过共享的、部分重叠的群体几何结构联系起来，支持可靠的跨条件分类，且最强对齐出现在运动和保持阶段。神经活动与多维手部运动学特征存在系统性的关系，并且这种关系可跨神经群体和跨主体泛化。总之，这些发现支持对镜像神经元功能的动态群体层面解释：前运动回路整合视觉与运动信号，以表征并预测他人动作结构的展开过程。


### 核心发现


- 抓握信息由神经元群体动态编码，随时间重构，跨阶段泛化有限，表明是时间特异性动态表征而非静态编码。

- 执行与观察通过部分重叠的群体几何结构联系，在运动与保持期对齐最强，支持可靠的跨条件分类。

- 神经活动系统性地编码多维手部运动学特征，且这种运动学表征可跨神经元群体和跨主体泛化。



**关键词**：Animals · Motor Cortex · Biomechanical Phenomena · Neurons · Movement · Macaca mulatta · Hand Strength · Male


---


## 7. Machine learning potentials for modeling alloys across compositions.

| 字段 | 内容 |
|---|---|
| **期刊** | Sci Adv |
| **发表日期** | 2026-Jun-19 |
| **作者** | Sheriff Killian, Xiao Daniel Z, Cao Yifan, Owen Lewis R, Freitas Rodrigo |
| **原文链接** | [查看原文](https://doi.org/10.1126/sciadv.aea9951) |
...
材料性质强烈依赖于化学组成，即各化学元素的相对含量。组成变化导致完全不同的化学排布，其复杂度从完全有序（即化学计量化合物）到完全无序（即固溶体）不等。准确捕捉这一系列化学排布仍是一项重大挑战，限制了机器学习势在材料建模中的预测精度。在此，我们结合信息论与机器学习，优化化学基序的采样，并设计出能够有效捕捉金属合金在整个组成和结构范围内行为的机器学习势。通过预测 AuPt 和 CuAu 二元合金、CrCoNi 三元合金以及 TiTaVW 高熵合金的各种材料性质（包括层错能、短程有序、热容和相图）对组成的依赖性，证明了该方法的有效性。与实验数据的广泛对比表明，该方法具有高度的物理保真度，能够实现稳健的材料建模。


### 核心发现


- 结合信息论与机器学习，优化化学基序采样，设计出覆盖合金全组成-结构空间的机器学习势。

- 该方法成功预测了多种合金的层错能、短程有序、热容和相图等性质对组成的依赖关系。

- 通过大量实验对比验证了方法的稳健性，实现了高物理保真度的材料建模。
...
## 8. Tardigrade-Derived Strategy for Low-Cost Storage of Cell-Free Expression Lysates.
...
无细胞表达系统（CFESs）日益与传统生物技术方法并行使用，以加速早期原型构建，并在现场使用环境中尤其有价值。然而，耗时且成本高昂的制备过程以及严格的低温储存要求限制了其更广泛采用。为解决这一问题，已有研究探索在保护性添加剂存在下进行冻干处理，以生成稳定的固态CFES，但需在保护效果与添加剂导致的活性损失之间取得平衡。本研究展示了一种含有缓步动物来源的胞质丰富热溶性（CAHS）蛋白的CFES，该蛋白可保护裂解液中的生物合成装置在干燥过程中免受损伤。我们发现，CAHS蛋白单独使用即可在低成本室温干燥后保持蛋白质合成活性，而未受保护的裂解液在mRNA合成动力学和翻译产量方面受到明显影响。缓步动物来源的保护蛋白多样性为无细胞合成生物学提供了宝贵资源，尤其有助于使CFES更易获取和便携。


### 核心发现


- 含有CAHS蛋白的无细胞表达系统在室温干燥后仍能保持蛋白质合成活性。

- CAHS蛋白无需任何其他添加剂即可保护裂解液免受干燥损伤。

- 未受保护的裂解液在干燥后mRNA合成动力学和翻译产量均受损。
...
## 9. SARGE: Serine Integrase-Assisted Rapid Genome Integration through Cassette Exchange in Escherichia coli.
...
丝氨酸整合酶能实现精确的基因组工程，但在迭代基因组整合中仍未被充分利用，因为现有流程往往劳动密集且难以扩展。本文报道了SARGE，一种丝氨酸整合酶辅助的快速基因组整合平台，通过盒式交换将大型DNA片段迭代插入大肠杆菌基因组。通过结合正交的PhiC31与Bxb1丝氨酸整合酶及合理的供体质粒设计，SARGE支持快速、可编程的整合循环，且无需在各轮次间切除抗性标记。为简化正确重组体的鉴定，我们整合了基于sfGFP和mKate的双荧光报告系统，并建立了绿色-红色筛选策略，可直接肉眼挑选菌落。该可视化筛选在所测菌落中准确鉴定了所需重组体，准确率达100%。SARGE的盒式交换效率最高可达95%，对10 kb的大片段负载仍保持约90%的效率。这些特性大幅简化了迭代基因组整合，使SARGE成为大肠杆菌基因组工程与合成生物学中稳健、易用的平台，并有潜力扩展到其他遗传易操作的微生物宿主。


### 核心发现


- 开发SARGE平台，利用正交丝氨酸整合酶实现迭代DNA整合，免除抗性标记切除步骤。

- 通过双荧光报告系统（sfGFP/mKate）与绿红筛选策略，肉眼即可100%准确识别正确重组体。

- SARGE盒式交换效率最高达95%，对10 kb大片段负载效率仍保持约90%。
...
## 10. Characterization of the biosynthetic enzymes of the Morus alba benzofuran natural products.

| 字段 | 内容 |
|---|---|
| **期刊** | Sci Adv |
| **发表日期** | 2026-Jun-19 |
| **作者** | Yu Xinshui, Guo Nianxin, Long Teng, Zhu Yuling, Ke Han, Ding Qi, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1126/sciadv.aec4824) |
...
摩拉辛是一类独特的苯并呋喃天然产物，主要存在于桑科植物中。尽管它们具有多样的生物活性和有趣的化学结构，但对其生物合成知之甚少。在此，我们阐明了摩拉辛 M 的生物合成途径，它是摩拉辛生物合成中的一个共同中间体。利用热蛋白质组分析，我们从桑树中鉴定出一种非经典的漆酶（MaLAC），它催化氧白藜芦醇发生前所未有的氧化环化反应生成摩拉辛 M。与其他植物漆酶不同，MaLAC 是首个被证明在天然产物生物合成中催化分子内氧化环化反应的植物漆酶。机制研究揭示了铜介导的自由基环化，为工程化漆酶以催化具有合成价值的分子内氧化环化提供了指导。我们在本氏烟草中异源构建了摩拉辛 M 的生物合成途径。这项工作不仅为摩拉辛生物合成提供了重要见解，为通过代谢工程策略生产具有生物活性的摩拉辛奠定了基础，而且证明了无标记化学蛋白质组学在阐明植物次生代谢物未知生物合成途径中的威力。


### 核心发现


- 发现桑树非经典漆酶MaLAC，首次催化分子内氧化环化反应，将氧化白藜芦醇转化为摩拉辛M。

- 机制研究揭示铜介导的自由基环化，为工程化漆酶实现合成有用的分子内氧化环化提供指导。

- 在烟草中异源重构摩拉辛M途径，奠定代谢工程基础，并展现无标记化学蛋白质组学解析植物次生代谢途径的能力。



**关键词**：Morus · Benzofurans · Biological Products · Biosynthetic Pathways · Laccase · Nicotiana · Plant Proteins · Cyclization


---


## 11. Evaluating DNA Function Understanding in Genomic Language Models Using Evolutionarily Implausible Sequences.
...
基因组语言模型（gLMs）有望为合成生物学生成全新的功能性DNA序列。关键挑战在于判断gLM是理解序列功能，还是仅记忆源自天然基因组的训练模式。我们提出Nullsettes评估框架，用于衡量模型预测合成表达盒中缺乏进化先例的计算机模拟功能丧失（LOF）突变的准确性。针对当前领先的gLM，我们发现它们普遍无法检测到强LOF突变。当原始非突变序列具有较低模型似然时，预测准确性急剧下降，表明模型依赖进化模式匹配而非对基因表达的机制性理解。这些结果暴露了gLM在泛化至工程化遗传构建体时的核心局限，并强调需要发展能明确检验功能理解的评估与建模策略。


### 核心发现


- gLM普遍无法识别合成表达盒中的强功能丧失突变。

- 低似然原始序列的预测准确性骤降，揭示gLM依赖进化模式匹配而非机制理解。

- 结果凸显gLM在工程化构建上的泛化局限，需建立测试功能理解的新评估范式。



**关键词**：Synthetic Biology · Models, Genetic · DNA · Mutation · Genomics · Evolution, Molecular


---


## 12. Elucidating the Bell-Shaped Dependence of Protein Translation Activity on EF-Tu Concentration in a Reconstituted Cell-Free System Using a Mechanistic Model.

| 字段 | 内容 |
|---|---|
| **期刊** | ACS Synth Biol |
| **发表日期** | 2026-Jun-19 |
| **作者** | Ban Shunnosuke, Himeoka Yusuke, Kagawa Ako, Shimizu Yoshihiro, Matsuura Tomoaki, Furusawa Chikara |
| **原文链接** | [查看原文](https://doi.org/10.1021/acssynbio.6c00333) |
...
无细胞蛋白质合成系统中的蛋白质生产常表现出非直观的输入-输出关系。在PURE（利用重组元件进行的蛋白质合成）这一重构无细胞系统中，蛋白质产量在低延伸因子Tu（EF-Tu）浓度时达到峰值，而在较高浓度下反而下降，呈现出特征性的钟形曲线。本文利用PURE系统中翻译过程的详细机制模型（名为ePURE）探究了这一现象的成因，该模型描述了数百种分子物种和反应过程的动态。计算分析表明，过量的EF-Tu会将起始tRNA（tRNAfMet）隔离在非生产性的EF-Tu·GTP·Met-tRNAfMet复合物中，从而耗竭可用于翻译起始的起始tRNA池。这种抑制源于对有限分子资源的竞争，而非直接抑制作用。基于这一机制，我们预测提高tRNAfMet和甲硫氨酰-tRNA甲酰转移酶的浓度能够消除钟形依赖关系，并通过实验验证了这一预测。在这些修饰条件下，钟形响应消失，蛋白质产量得到提升。这些发现展示了机制计算模型如何揭示复杂生化网络中非直观输入-输出关系背后的隐藏约束，并为无细胞蛋白质合成系统的理性优化提供指导。


### 核心发现


- 过量EF-Tu通过将起始tRNA隔离在非生产性复合物中，抑制翻译起始。

- 该钟形依赖源于分子资源竞争，而非EF-Tu的直接抑制效应。

- 提高tRNAfMet和甲酰转移酶浓度可消除钟形响应并增强蛋白质产量。



**关键词**：Peptide Elongation Factor Tu · Cell-Free System · Protein Biosynthesis · RNA, Transfer, Met · Guanosine Triphosphate · Models, Biological


---


## 13. Characterization of a Two-Sided Orthogonal Translation System and Its Application to Violacein Biosynthesis.

| 字段 | 内容 |
|---|---|
| **期刊** | ACS Synth Biol |
| **发表日期** | 2026-Jun-19 |
| **作者** | Kusumi Riko, Kosaka Yuishin, Sugita Takashi, Kobayashi Keisuke, Motone Keisuke, Miyazaki Kentaro, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1021/acssynbio.6c00143) |
| **DOI** | `10.1021/acssynbio.6c00143` |
...
微生物生产高价值化合物面临的主要挑战是细胞生长与生物合成之间的权衡。正交翻译系统利用源自工程化rRNA的正交核糖体翻译特定mRNA，可将翻译资源从细胞生长重新分配到生物合成。然而，正交翻译系统的体内正交性和定量有效性尚未得到充分评估。本研究以sfGFP（超级折叠绿色荧光蛋白）为报告基因，在大肠杆菌中系统测试正交rRNA-mRNA对，鉴定出一对具有稳健双向正交性和高翻译效率的组合。随后将此正交翻译系统应用于紫罗兰素生物合成途径。与内源翻译系统相比，正交翻译系统使紫罗兰素产量提高6.30至7.77倍，并在进入稳定期后仍能持续生产。这些结果表明，经过良好表征的双向正交翻译系统可通过体内重分配翻译能力，成功提高多酶途径的生产力。


### 核心发现


- 鉴定出一对在大肠杆菌内具有强双向正交性和高翻译效率的rRNA-mRNA组合。

- 该正交翻译系统使紫罗兰素产量比内源系统提高6.30至7.77倍，并在稳定期持续生产。

- 证明重分配体内翻译能力的策略可有效提升多酶生物合成途径的生产力。



**关键词**：Escherichia coli · Indoles · Protein Biosynthesis · Ribosomes · RNA, Messenger · Metabolic Engineering · Green Fluorescent Proteins


---


## 14. Tunable Transcription-Level CRISPR Interference in Vibrio natriegens Using Engineered Single Guide RNAs.

| 字段 | 内容 |
|---|---|
| **期刊** | ACS Synth Biol |
| **发表日期** | 2026-Jun-19 |
| **作者** | Choi Mincheol, Byun Gibyuck, Kim Giho, Yang Jina, Seo Sang Woo |
| **原文链接** | [查看原文](https://doi.org/10.1021/acssynbio.6c00057) |
...
需钠弧菌是一种快速生长的细菌，已成为微生物学和生物工程领域有前景的下一代微生物平台。尽管已建立了不断扩展的遗传元件和基因组工程方法工具箱，但精确且可预测地控制基因表达的策略仍然有限。在此，我们报道了一种基于dCas9的可调CRISPR干扰（CRISPRi）系统，能够在需钠弧菌中实现多级转录调控。通过工程化改造单导RNA（sgRNA）的四环和侧翼区域，我们构建了一个合成的sgRNA库，可调节sgRNA与dCas9的结合亲和力。所得的sgRNA变体在多个原间隔序列靶点上表现出模块化阻抑行为。我们进一步通过重定向胞内碳通量，展示了该可调CRISPRi系统在代谢工程应用中的实用性。对内源基因的可调阻导致3-羟基丙酸（3-HP）产量提高2.2倍，番茄红素产量提高1.5倍。综上，本工作为需钠弧菌中可调基因调控提供了一种简单有效的策略，并拓展了其作为通用平台生产高附加值化学品的潜力。


### 核心发现


- 构建了sgRNA四环与侧翼区工程化改造的合成库，可通过调节dCas9结合亲和力实现多级转录阻抑。

- 该可调CRISPRi系统在多个靶点上表现出模块化阻抑行为，实现了需钠弧菌中基因表达的精确控制。

- 重定向碳通量使3-羟基丙酸产量提高2.2倍、番茄红素产量提高1.5倍，证明其在代谢工程中的应用价值。



**关键词**：Vibrio · RNA, Guide, CRISPR-Cas Systems · CRISPR-Cas Systems · Lycopene · Metabolic Engineering · Transcription, Genetic · Clustered Regularly Interspaced Short Palindromic Repeats · Gene Expression Regulation, Bacterial


---


## 15. Closed-Loop Optogenetic Control in a Microplate Reader.

| 字段 | 内容 |
|---|---|
| **期刊** | ACS Synth Biol |
| **发表日期** | 2026-Jun-19 |
| **作者** | Namboothiri Hari R, Pochana Krishna, Jaiswal Bhavya, Emami Azita, Hu Chelsea Y |
| **原文链接** | [查看原文](https://doi.org/10.1021/acssynbio.6c00003) |
| **DOI** | `10.1021/acssynbio.6c00003` |
...
光遗传学将活细胞与电子器件整合为强大的细胞-硅系统，但其动力学原型设计仍然充满挑战。现有工具要么需要机器人液体转移到流式细胞仪，要么依赖动态范围狭窄的定制传感器，从而限制了控制器性能。此外，目前成功的光遗传反馈控制器仅在恒化器或微流控装置中运行，这些装置强制恒定生长，因为缺乏用于分批培养中生长感知控制器设计的模型。在此，我们展示LEMOS，一种嵌入LED的低成本微孔板，在商用微孔板读数仪内运行。结合用于控制器调谐的生长感知多尺度基因表达模型，该平台为细胞-硅系统实现了快速的设计-构建-测试-学习循环。我们在标准微孔板读数仪中演示了分批培养中基因表达的闭环设定点跟踪，并展示了生长动态如何使控制器选择和调谐复杂化。总之，该平台减少了设置开销并加快了迭代速度，实现了精确的实时光遗传反馈控制。


### 核心发现


- 开发LEMOS平台，在商用微孔板读数仪内实现低成本光遗传闭环控制。

- 引入生长感知多尺度模型，解决分批培养中生长动态对控制器设计的挑战。

- 展示分批培养闭环基因表达设定点跟踪，揭示生长动态对控制器选择的影响。
...
## 16. Receptor-Guided AAV Tropism Engineering via MATCH.

| 字段 | 内容 |
|---|---|
| **期刊** | ACS Synth Biol |
| **发表日期** | 2026-Jun-19 |
| **作者** | Graham Nolan, Kumar Satheesh, Rainaldi Joseph, Yang Seoin, Portell Andrew, Santoso Billy, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1021/acssynbio.5c00910) |
| **DOI** | `10.1021/acssynbio.5c00910` |
...
在基因递送技术开发中，精确控制病毒嗜性仍然是一项重大挑战。我们提出MATCH（通过偶联归巢蛋白调节AAV嗜性），这是一种模块化生化方法，能够通过位点特异性共价蛋白偶联，实现对腺相关病毒（AAV）的可编程、受体引导的再靶向。通过将SpyTag肽基序整合到选定的AAV衣壳环中，MATCH允许将重组的SpyCatcher连接的靶向蛋白以一步、化学计量定义的方式附着到病毒表面。使用具有受控SpyTag整合的嵌合AAV-DJ和AAV9衣壳，我们实现了高效组装和可调配体展示。与抗CD3单链抗体偶联的MATCH-AAV能够在体外混合PBMC群体中高效激活并转导静息人T细胞，转导水平高达总PBMC的约58%。与转铁蛋白受体（TfR1）结合蛋白的偶联在体内实现了增强的脑转导，鼠TfR1靶向的MATCH-AAV9相对于野生型AAV9，脑部表达增加了高达88倍。人TfR1靶向载体同样在体外和人源化小鼠模型中实现了强大的、受体依赖的转导。两种TfR1靶向载体都能实现对实质的广泛转导，这与TfR1介导的血脑屏障穿越一致。最后，我们建立了一种简化的“一锅式”Mix-and-MATCH生产策略，即在载体生成过程中共表达衣壳和靶向配体，产生功能性靶向AAV，其滴度与传统生产相当。这种简单且可推广的合成生物学方法为理性AAV嗜性工程提供了多功能工具箱，为研究和治疗应用中的定制载体设计提供了一条可扩展的途径。


### 核心发现


- MATCH策略通过SpyTag/SpyCatcher共价偶联，实现了对AAV载体的受体引导、可编程再靶向。

- 抗CD3-MATCH-AAV可在混合PBMC中高效激活并转导静息人T细胞，转导率高达58%。

- TfR1靶向MATCH-AAV9显著增强脑部基因递送，在体内脑表达水平较野生型提高88倍。



**关键词**：Dependovirus · Humans · Animals · Receptors, Transferrin · Viral Tropism · Mice · Genetic Vectors · Gene Transfer Techniques · Capsid Proteins · Capsid


---


## 17. Using Domain Insertion to Create Sulfite Reductases That Present Chemical-Dependent Activities.

| 字段 | 内容 |
|---|---|
| **期刊** | ACS Synth Biol |
| **发表日期** | 2026-Jun-19 |
| **作者** | Windham Elizabeth, Myerscough Dru, Schwartz Samuel K, Carpenter Matthew D, Ajo-Franklin Caroline M, Silberg Jonathan J |
| **原文链接** | [查看原文](https://doi.org/10.1021/acssynbio.6c00209) |
| **DOI** | `10.1021/acssynbio.6c00209` |
...
结构域插入可用于创建其活性依赖于分析物结合的氧化还原酶。迄今为止，大多数结构域插入研究都针对结构已知的相对较小的氧化还原酶，因此如何将这种蛋白质工程方法应用于需要动态构象变化进行催化的大型异源寡聚蛋白仍不清楚。为了解决这个问题，我们研究了肽和结构域插入对来自大肠杆菌的NADPH依赖性亚硫酸盐还原酶（SiR）活性的影响，这是一种十二聚体氧化还原酶，含有四个血红素蛋白亚基和八个黄素蛋白亚基。首先通过系统性八肽插入和细胞选择评估了SiR的突变耐受性，这识别了血红素蛋白结构中在插入后仍保留类亲本活性的区域。当在耐受肽插入的骨架位置（包括靠近和远离亚基间界面的位点）插入配体结合结构域时，约90%保留了催化活性，且超过50%表现出受内分泌干扰物调节的活性。利用一种结构域插入变体，可以使用生物电化学反应器通过电化学方法从细胞中监测硫化物的条件性生产。这些结果展示了如何利用系统性肽插入来指导大型异源寡聚蛋白复合物中的结构域插入，并说明了如何将SiR工程化改造，以将环境中的化学信息转化为可跨细胞膜扩散的氧化还原活性代谢物。


### 核心发现


- 通过八肽插入筛选出大肠杆菌SiR血红素蛋白中保留活性的耐受区域。

- 在耐受位点插入配体结合域后，>50%变体活性受内分泌干扰物调控。

- 利用域插入变体实现条件性硫化物生产及电化学监测，展示环境信号转化能力。



**关键词**：Escherichia coli · Protein Engineering · Protein Domains · Sulfite Reductase (NADPH)


---


## 18. Bottom-Up Creation of Virus-like Particles via Post-Insertion of Protein-Lipid Conjugates.

| 字段 | 内容 |
|---|---|
| **期刊** | ACS Synth Biol |
| **发表日期** | 2026-Jun-19 |
| **作者** | Kobayashi Iori, Yoshimoto Shogo, Yokoyama Takeshi, Tanaka Yoshikazu, Kiyonaka Shigeki, Matsuura Tomoaki, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1021/acssynbio.6c00172) |
...
病毒样颗粒（VLPs）被广泛用作疫苗、药物递送和合成生物学中的非感染性平台。本文报道了一种自下而上的方法，通过将蛋白质-脂质偶联物后插入预制的脂质体中，产生脂质体VLPs。蛋白质通过生物正交且不可逆的SNAP-tag/BG反应共价偶联到苯甲鸟嘌呤（BG）脂质上，所得蛋白质-脂质偶联物能够无去污剂地插入脂质体脂双层。使用SNAP-tag标记的GFP偶联物，我们首先通过脂质体共沉降确认了膜关联，并通过流式细胞术检测到单个脂质体上GFP荧光增强。接着，我们将该方法应用于SARS-CoV-2刺突蛋白外域。在Expi293F细胞中表达了Spike-SNAP重组蛋白，并通过荧光BG底物的结合抑制试验验证了其与BG脂质的偶联能力。将病毒大小的脂质体与Spike-脂质偶联物孵育，结果在脂质体组分中，Spike的回收依赖于脂质锚，流式细胞术显示Spike相关荧光增强，且未检测到粒径变化。负染透射电子显微镜显示，与未修饰脂质体相比，Spike修饰的脂质体呈现出病毒样纹理。在ACE2平板结合试验中，Spike修饰脂质体的结合能力高于未修饰脂质体，且结合被已报道的Spike-ACE2相互作用抑制剂刚果红以及一种中和抗Spike抗体所减弱。通过将偶联物的设计从脂质体的设计中解耦，这种后插入方法可促进VLPs的快速系统优化，用于机制研究和应用导向的研究。


### 核心发现


- 建立后插入蛋白-脂质偶联物生成脂质体VLPs的模块化方法，无需去污剂。

- 成功将SARS-CoV-2 Spike蛋白锚定于脂质体，赋予病毒样特征和ACE2结合能力。

- 通过解耦设计实现VLPs快速系统优化，适用于疫苗和递送系统开发。



**关键词**：Liposomes · Spike Glycoprotein, Coronavirus · SARS-CoV-2 · Humans · Green Fluorescent Proteins · Lipids · Vaccines, Virus-Like Particle


---


## 19. CRISPR/Cas9 Recombineering with Bacteriophage Recombinases Enables Efficient and Scarless Genome Editing in Cupriavidus necator H16.

| 字段 | 内容 |
|---|---|
| **期刊** | ACS Synth Biol |
| **发表日期** | 2026-Jun-19 |
| **作者** | Liu Yuzhong, Wang Caidie, Tian Yiyuan, Li Junlin, Wu Zeyu, Wang Pengxia, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1021/acssynbio.6c00055) |
...
钩虫贪铜菌H16因其能以氢气衍生的还原力同化CO₂，是单碳（C1）生物制造的有前景底盘，但该菌的基因组工程仍受限于DNA递送效率低和同源重组效率低下。在此，我们建立了一个模块化双质粒CRISPR/Cas9重组工程系统，将Cas9表达与sgRNA周转解耦，引入自剪接内含子核糖开关以严格控制Cas9反筛，并导入一对噬菌体重组酶Che9c60和Che9c61，显著提高同源重组效率。使用Che9c60和Che9c61后，基因敲除效率从不可检测水平提升至58.3%，并在某些位点达到100%。重要的是，引入这两种重组酶使无痕染色体敲入成为可能，插入效率从无法检测提升至近40%，并支持长达4.0 kb的DNA片段定向整合。为便于快速多轮工程改造，我们在sgRNA质粒中整合了基于tdk的反筛标记，在5-氟-2'-脱氧尿苷（FUDR）筛选下，24小时内将质粒滞留细胞比例降至1.1%。总体而言，这一精简的工具箱可在钩虫贪铜菌中实现高效、迭代、无痕的基因组编辑，加速用于C1代谢工程和工业应用的钩虫贪铜菌菌株开发。


### 核心发现


- 引入噬菌体重组酶Che9c60和Che9c61，将C. necator的基因敲除效率从无法检测提升至58.3%，部分位点达100%。

- 两种重组酶实现了无痕染色体敲入，插入效率提升至近40%，并支持4.0 kb片段整合。

- 构建的tdk反筛系统可在24小时内清除含质粒细胞至1.1%，实现高效迭代编辑。



**关键词**：CRISPR-Cas Systems · Cupriavidus necator · Gene Editing · Bacteriophages · Recombinases · Plasmids · Gene Knockout Techniques


---


## 20. Temporal Programming of Cell-Free Transcription Using Orthogonal Enzyme-Responsive DNA Blockers.

| 字段 | 内容 |
|---|---|
| **期刊** | ACS Synth Biol |
| **发表日期** | 2026-Jun-19 |
| **作者** | Lerma-Escalera Jordy Alexis, Bucci Juliette, Urošević Ana, Amador Romain, Ercolani Gianfranco, Ricci Francesco |
| **原文链接** | [查看原文](https://doi.org/10.1021/acssynbio.6c00147) |
...
本文报道了正交的、酶驱动的DNA转录计时器的设计，能够在无细胞体外转录中实现时间延迟的精确编程。这些计时器利用阻断链瞬时结合启动子区域来阻止转录起始。通过选择性酶切割阻断链使其移除，允许输入DNA链结合并启动转录。通过调节酶或阻断链浓度来调谐酶促阻断链降解动力学，我们实现了对转录半衰期（t1/2）的精细时间控制，范围从0.48 ± 0.02小时到8.4 ± 0.1小时。使用三种不同的阻断链降解酶（RNase H、尿嘧啶DNA糖基化酶（UDG）和甲酰胺嘧啶DNA糖基化酶（Fpg）），我们还展示了在单一溶液中对多个转录模板的正交时间控制。最后，我们展示了通过此类转录计时器实现Cas12a酶促侧支切割活性的编程终止控制和下游调控。总体而言，这些正交DNA转录计时器为无细胞合成生物学中的时间分辨转录和基因表达编程建立了一个通用且直接的框架。


### 核心发现


- 设计酶响应DNA阻断链，实现无细胞转录起始的可编程时间延迟，半衰期调控范围达0.48至8.4小时。

- 利用三种正交酶（RNase H、UDG、Fpg）实现同一溶液中多个转录反应的独立时间控制。

- 应用转录计时器编程控制Cas12a侧支切割活性的终止，展示对下游基因调控的时间分辨能力。



**关键词**：Transcription, Genetic · Cell-Free System · DNA · Promoter Regions, Genetic · Uracil-DNA Glycosidase · Ribonuclease H · Kinetics


---


## 21. Engineered Escherichia coli Strains for Flavoprotein Research: From Production of Apoproteins to Incorporation of Flavin Derivatives.

| 字段 | 内容 |
|---|---|
| **期刊** | ACS Synth Biol |
| **发表日期** | 2026-Jun-19 |
| **作者** | Fan Xiaoman, Widodo Wahyu S, Rozeboom Henriette J, Fraaije Marco W |
| **原文链接** | [查看原文](https://doi.org/10.1021/acssynbio.6c00269) |
...
黄素蛋白参与广泛的生物过程，含有一个或多个黄素辅因子，以非共价或共价方式结合，催化多种氧化还原反应。常用表达菌株通常产全黄素蛋白。获得其脱辅基形式将便利结构与机理研究，并允许引入非天然的类黄素辅因子。为此，我们利用CRISPR/Cas9系统构建了一株因ribB基因缺失而无法合成经典FMN与FAD的大肠杆菌核黄素营养缺陷型菌株，该菌株可产生FMN和FAD依赖型黄素蛋白的脱辅基蛋白，并能在培养基中添加核黄素衍生物以掺入黄素衍生物。同时，通过基因组整合构建了表达T7启动子驱动的FAD合成酶和lac启动子驱动的黄素转移酶的菌株，便于应用黄素标记与固定技术使目标蛋白共价连接FMN，无需多重质粒。进一步组合了核黄素营养缺陷与黄素转移酶活性，构建了适于生成共价锚定黄素衍生物黄素蛋白的菌株。这些E. coli BL21-AI衍生工程菌株是黄素蛋白生产、研究与应用的有力工具。


### 核心发现


- 构建核黄素营养缺陷型菌株，可生产脱辅基黄素蛋白并掺入非天然黄素衍生物。

- 基因组整合构建共表达FAD合成酶和黄素转移酶的菌株，实现一步共价标记FMN。

- 组合营养缺陷与转移酶功能，实现共价锚定黄素衍生物的黄素蛋白生产。



**关键词**：Escherichia coli · Flavoproteins · Flavins · Flavin Mononucleotide · Apoproteins · Flavin-Adenine Dinucleotide · Riboflavin · CRISPR-Cas Systems · Escherichia coli Proteins


---


## 22. A Linear Mixed Effects Model for Evaluating Synthetic Gene Circuits.

| 字段 | 内容 |
|---|---|
| **期刊** | ACS Synth Biol |
| **发表日期** | 2026-Jun-19 |
| **作者** | Partipilo Gina, Coleman Sarah M, Holwerda Alexis J, Gao Yang, Mahfoud Ismar E Miniel, Wilke Claus O, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1021/acssynbio.6c00171) |
...
合成生物学的一个重大进展是开发了具有可预测布尔逻辑的合成基因电路，但缺乏通用的统计检验来分析其性能。许多基本检验忽略了预测逻辑，许多研究甚至忽略统计分析。随着这些电路向计算、生物传感和人类健康等先进应用转变，标准化统计方法至关重要。本文提出应用线性混合效应模型分析和量化遗传布尔逻辑门性能。首先，用无监督机器学习（k-means聚类）分析了144个已发表的布尔逻辑门以发现趋势；接着模拟各聚类中心的数据；再利用线性混合效应模型估算电路的布尔特性，其中固定效应β表示ON与OFF状态的差异，随机效应为输入分组。在模拟数据上验证模型，并通过蒙特卡洛模拟推荐评估所需的样本量。进一步使用一系列性能故意下降的嵌套阻遏物OR门，评估固定效应点估计β̂作为电路性能的整体指标，发现β̂与预测的翻译速率相关，并利用β̂指导设计3输入门，突显了该方法对新布尔门正向设计的潜力。最后将分析扩展至多输入（>2）和多输出门，强调该模型可应用于高阶逻辑。总之，线性混合效应模型可有效评估合成基因电路，固定效应点估计β̂是门行为的合适描述符，可用于统计评价性能。


### 核心发现


- 提出线性混合效应模型，以固定效应β量化ON/OFF状态差异，标准化评估基因电路性能。

- 基于144个逻辑门聚类与模拟数据，用蒙特卡洛方法验证模型并推荐了合适的样本量。

- 点估计β̂可综合表征门行为，成功指导3输入门设计，并适用于多输入多输出高阶逻辑电路。



**关键词**：Synthetic Biology · Gene Regulatory Networks · Linear Models · Genes, Synthetic · Models, Genetic · Computer Simulation · Monte Carlo Method · Algorithms


---


## 23. Repurposing the Diatom Periplastidial Compartment for Heterologous Terpenoid Production.
...
硅藻是利用CO₂进行光合萜类生产的潜力微生物，但其区室化工程的潜力仍未被充分探索。本研究系统分析了三角褐指藻在胞质、叶绿体和周质体区室（PPC）中表达代表性合酶以合成半萜、单萜、倍半萜和四萜的能力。结果表明，二甲基烯丙基焦磷酸（DMAPP）、牻牛儿基焦磷酸（GPP）、法尼基焦磷酸（FPP）和牻牛儿基牻牛儿基焦磷酸（GGPP）等所有主要异戊二烯基磷酸前体在包括PPC在内的各区室均可获取，并能支持异源代谢通量而不造成显著生理负担，尽管生产效率和产物类别因区室而异。基于前体可用性的确认，我们提出硅藻PPC可作为与真核复杂叶绿体直接整合的可工程化细胞内空间，为区室化萜类生物合成工程策略奠定基础。此外，我们强调其作为探究甲羟戊酸（MVA）途径与甲基赤藓醇磷酸（MEP）途径间代谢交换的独特界面的价值。这些发现为硅藻区室特异性萜类工程提供了系统框架，并为光合真核生物中的模块化途径组装和合成生物学开辟了新机遇。


### 核心发现


- 所有主要异戊二烯基磷酸前体在三角褐指藻的胞质、叶绿体和周质体区室中均可获得，支持异源萜类生产。

- 硅藻周质体区室可作为与真核叶绿体直接整合的可工程化空间，用于区室化萜类生物合成。

- 该区室为研究MVA和MEP途径间的代谢交换提供独特界面，开辟光合真核生物合成生物学新机会。



**关键词**：Diatoms · Terpenes · Chloroplasts · Metabolic Engineering · Photosynthesis · Biosynthetic Pathways · Cytosol · Polyisoprenyl Phosphates


---


## 24. A functional map of phosphoprotein phosphatase regulation identifies an evolutionarily conserved reductase for the catalytic metal ions.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-Jun-18 |
| **作者** | Meeusen Bob, Ambjørn Sara M, Veis Jiri, Riley Rachel C, Vit Gianmatteo, Brauer Brooke L, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-74262-y) |
| **DOI** | `10.1038/s41467-026-74262-y` |
...
丝氨酸/苏氨酸磷酸蛋白磷酸酶（PPP）是保守的金属酶，也是细胞内信号传导的关键调节因子。本研究利用全基因组CRISPR敲除和聚焦碱基编辑筛选，系统绘制了PP2A类磷酸酶（PP2A/PP4/PP6）功能所需的组分和残基图谱，并作为综合资源提供。我们发现还原酶CYB5R4是PP4和PP6的进化保守激活因子，但不作用于PP2A。我们证明PP4和PP6具有氧化还原敏感性，需要CYB5R4还原其活性位点金属离子以实现激活。其机制涉及CYB5R4 N端尾部中的结合元件对PP4和PP6的选择性结合，以及CYB5R4结合的血红素作为活性位点的电子供体。我们进一步表明，这一激活过程对调控DNA损伤应答至关重要。本工作提示活性位点金属离子的氧化还原调控介导了细胞氧化状态与特定磷酸酶活性之间的交互作用。


### 核心发现


- 系统绘制PP2A类磷酸酶功能组分与残基图谱，提供全面资源。

- 发现CYB5R4是PP4/PP6的保守氧化还原激活因子，通过还原活性位点金属离子实现选择性激活。

- 揭示CYB5R4通过N端结合和血红素电子传递激活PP4/PP6，对DNA损伤应答至关重要。
...
## 25. Deep learning-guided engineering of SpuFz1 and rational miniaturization of ωRNA enables efficient genome editing.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-Jun-18 |
| **作者** | Chen Shuanghong, Hsiao Shenlin, Xie Tian, Chen Danni, Chen Naixin, Jiang Jing, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-74624-6) |
| **DOI** | `10.1038/s41467-026-74624-6` |
...
提高可编程基因组编辑核酸酶的性能仍然是拓展其研究与治疗应用的关键挑战。在此，我们提出一种可扩展的深度学习引导的蛋白质工程框架，无需依赖实验训练数据即可提升核酸酶活性。作为演示，我们将该策略应用于真核生物来源的紧凑型Fanzor核酸酶SpuFz1，识别并验证了有益突变，获得一个多突变变体，编辑效率提升11.6倍。同时，我们通过比较序列分析设计并实验验证了一个75-nt的超短ωRNA支架，将引导RNA长度缩短79%而保持活性。整合这些优化组分得到enFanzor，一种紧凑型基因组编辑系统，在哺乳动物细胞中编辑效率高达81.9%，在人类造血干细胞与祖细胞（HSPCs）和小鼠胚胎中均表现出强大的编辑性能。通过该策略开发的优异变体还支持强大的胞嘧啶碱基编辑器（CBE）和腺嘌呤碱基编辑器（ABE）活性。值得注意的是，缩短的ωRNA不仅提高了核酸酶的编辑特异性，还显著提升了碱基编辑效率。总之，这项工作展示了人工智能指导的蛋白质优化与理性RNA设计相结合的力量，并建立了一种可推广的下一代基因组编辑工具工程化策略。


### 核心发现


- 开发了无需实验训练数据的深度学习框架，成功改造SpuFz1核酸酶，编辑效率提升11.6倍。

- 通过序列分析设计仅75-nt的超短ωRNA，缩短引导RNA长度79%且保持活性，同时提升编辑特异性与碱基编辑效率。

- 整合优化组分构建的enFanzor系统在哺乳动物细胞、HSPCs及小鼠胚胎中实现高达81.9%的编辑效率，并支持CBE和ABE。
...
## 26. Viola seed pod architecture shapes sequential, force-augmented pinching.

| 字段 | 内容 |
|---|---|
| **期刊** | Science |
| **发表日期** | 2026-Jun-18 |
| **作者** | Kim Cheongsan, Won Jihyun, Kim Donghyeon, Jung Sohyun, Kim Ho-Young, Hyun Youbong |
| **原文链接** | [查看原文](https://doi.org/10.1126/science.aed2953) |
...
许多植物通过爆发性方式弹射种子，但这些天然弹射器在种子喷射中常常表现出低效、不可预测的能量传递。堇菜（Viola spp.）通过从单个蒴果中连续以一致的推进力依次弹射种子，解决了这一问题，这一策略需要精密的能量释放。在本研究中，我们展示了堇菜通过一种简单而紧凑的结构实现了这一壮举，该结构通过顺序捏合产生自适应力增强。我们的生物学和数学分析表明，蒴果瓣的形态几何优化了捏合过程，在有限的材料成本下具有足够的种子弹射强度，并形成了一个移动的力放大热点，从而实现连续的种子弹射。我们利用这一设计原理创建了自主拉链式致动器，适用于一系列应用，包括生物医学软体机器。


### 核心发现


- 堇菜蒴果通过顺序捏合机制实现连续且力增强的种子弹射。

- 蒴果瓣形态几何以有限材料成本优化捏合强度，形成移动力放大热点实现连续弹射。

- 该原理被用于设计自主拉链致动器，适用于生物医学软机器等。



**关键词**：Seeds · Biomechanical Phenomena


---


## 27. Decoupling of global metabolic flux and proteome partitioning in bacteria.

| 字段 | 内容 |
|---|---|
| **期刊** | Science |
| **发表日期** | 2026-Jun-18 |
| **作者** | Thiermann Ryan, Yang Jin, Zodage Aniket, She Fukang, Fung Danny K, Rytlewski Taylor, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1126/science.aeb6410) |
...
细菌通过调整蛋白质组组成来调节稳态生长。在大肠杆菌中，这种协调由鸟苷四磷酸和鸟苷五磷酸（统称(p)ppGpp）介导，将氨基酸供应与核糖体生产偶联。我们在枯草芽孢杆菌中发现了不同的调控架构：鸟苷三磷酸（GTP）而非(p)ppGpp控制蛋白质组分配。翻译抑制导致GTP耗竭，通过反馈抑制抑制氨基酸生物合成，但不改变核糖体丰度，从而在总氨基酸通量与蛋白质组组成之间建立了受调控的解耦，使得通量偏离基于蛋白质组的预测。通过人工调节GTP浓度，我们重新偶联了通量与蛋白质组，恢复了最大生长。这种受调控的次优性实现了生长与胁迫耐受之间的权衡。类似基于GTP的策略存在于其他厚壁菌门物种中，表明可能的进化保守性。在一些细菌中，蛋白质组组成与代谢通量由不同的调控层级控制。


### 核心发现


- 枯草芽孢杆菌中GTP（而非(p)ppGpp）控制蛋白质组分配，实现代谢通量与蛋白质组的解耦。

- 翻译抑制引发GTP耗竭并反馈抑制氨基酸合成，使通量偏离蛋白质组预测而不改变核糖体丰度。

- 人工调节GTP浓度可重偶联通量与蛋白质组恢复最大生长，该策略在厚壁菌门中保守，平衡生长与胁迫耐受。



**关键词**：Proteome · Bacillus subtilis · Guanosine Triphosphate · Bacterial Proteins · Amino Acids · Protein Biosynthesis · Guanosine Pentaphosphate · Ribosomes · Feedback, Physiological


---


## 28. scMagnifier: Resolving fine-grained cell subtypes via GRN-informed perturbations and consensus clustering.

| 字段 | 内容 |
|---|---|
| **期刊** | PLoS Comput Biol |
| **发表日期** | 2026-Jun-18 |
| **作者** | He Zhenhui, Kangning Dong |
| **原文链接** | [查看原文](https://doi.org/10.1371/journal.pcbi.1014167) |
...
在单细胞RNA测序（scRNA-seq）数据中，解析细粒度细胞亚型仍具挑战性，因为其细微的转录差异常被技术噪声和数据稀疏所掩盖。本文提出scMagnifier，一种一致性聚类框架，利用基因调控网络（GRN）指导的计算机模拟扰动放大细微转录差异，揭示潜在细胞亚群。scMagnifier扰动候选转录因子（TF），通过簇特异性GRN传播扰动效应以模拟扰动后表达谱，并将多种扰动下的聚类结果整合为稳定的亚型分配。此外，scMagnifier引入调控扰动一致性UMAP（rpcUMAP），一种扰动感知的可视化方法，能在细胞亚型间提供更清晰的分离并指导最优簇数的选择。在单批次和多批次基准测试中，scMagnifier持续提升了细粒度细胞类型鉴定的分辨率和准确性。值得注意的是，当与空间聚类方法如STAGATE整合时，scMagnifier兼容空间转录组学工作流程，并有效揭示卵巢癌中肿瘤细胞亚型及其空间组织。


### 核心发现


- scMagnifier通过GRN指导的计算机扰动放大转录差异，提高细胞亚型分辨率。

- 引入rpcUMAP可视化方法改善亚型分离并辅助确定最佳簇数。

- 与空间转录组学结合，有效揭示卵巢癌肿瘤细胞亚型的空间组织。
...
## 29. Mechanisms underlying spontaneous and evoked calcium responses in oligodendrocyte precursor cells: A modeling investigation.

| 字段 | 内容 |
|---|---|
| **期刊** | PLoS Comput Biol |
| **发表日期** | 2026-Jun-18 |
| **作者** | Lardy Martin, Wang Leqi, Guerrier Claire, Cheli Veronica T, Paez Pablo M, Khadra Anmar |
| **原文链接** | [查看原文](https://doi.org/10.1371/journal.pcbi.1013430) |
| **DOI** | `10.1371/journal.pcbi.1013430` |
...
钙信号已成为少突胶质细胞活动依赖性髓鞘形成的中心调控者。这些钙信号包括以非电压依赖方式内源产生或受膜电压促进的刺激无关的自发性局部钙瞬变（SCaLTs），以及由 ATP 和谷氨酸释放触发的诱发响应。为了研究这种组合峰活动背后的调控机制，我们开发了少突胶质前体细胞（OPCs）钙瞬变的随机时空通量平衡模型。该模型纳入了这些细胞中所有相关通量，并利用拟合钙荧光记录的参数将膜电压动力学与钙诱导的钙释放（CICR）机制相整合。模型再现了在缺乏嘌呤能和谷氨酸能受体情况下 OPCs 的内源性和电压促进的 SCaLTs，并捕捉到了由机器分类器识别的延长 ATP 和谷氨酸刺激所诱导的三种不同模式的诱发钙响应。模型突出了 ATP 和谷氨酸在产生这些簇中的作用，并表明 CICR 的快速动力学是产生这些诱发响应的关键。对模型的进一步分析还揭示，电压门控 L 型和 T 型钙通道稍微增加 SCaLTs 的频率，而使用模拟体内条件的随机分布脉冲进行 ATP 和谷氨酸刺激，导致钙峰（即 SCaLTs 和诱发响应的组合）幅度增加以及宽峰的出现率上升，尤其是在谷氨酸刺激时。对模型在无扩散情况下的确定性版本进行分岔分析表明，ATP和谷氨酸刺激可将系统转入振荡状态，从而增强 SCaLT 动力学的确定性成分。这项研究因此提供了 OPC 钙瞬变的全面表征，将记录的离体行为与体内动力学联系起来。


### 核心发现


- 建立的模型成功再现OPC自发钙瞬变及ATP/谷氨酸诱发的三种响应模式，揭示CICR快速动力学的核心作用。

- 电压门控钙通道仅轻微提高SCaLTs频率，体内随机刺激（尤其谷氨酸）大幅增加钙峰幅度与宽峰比例。

- 分岔分析显示刺激可将系统推入振荡状态，从而增强自发钙动力学中的确定性成分。
...
## 30. Single-step in vitro reconstitution of the Escherichia coli ribosome mediated by two GTPase factors, EngA and ObgE.

| 字段 | 内容 |
|---|---|
| **期刊** | Elife |
| **发表日期** | 2026-Jun-18 |
| **作者** | Sato Aya, Lai Weng Yu, Sakai Yusuke, Masuda Keiko, Shimizu Yoshihiro |
| **原文链接** | [查看原文](https://doi.org/10.7554/eLife.109916) |
| **DOI** | `10.7554/eLife.109916` |

### 摘要（英文）

When Escherichia coli ribosomes are assembled in vitro, manipulation of incubation temperature and magnesium ion concentration has been an essential procedure, which is a crucial step for the assembly of active large subunits. The present study tackles this issue to develop a single-step procedure, which can be performed in near-physiological conditions, where cell-free protein synthesis is active. We found that GTPase factors EngA and ObgE can complement the changes in temperature and magnesium ion concentrations. In the presence of these factors, both the ribosome assembly and translation processes were successfully integrated in the reconstituted cell-free protein synthesis system. Furthermore, we found that these GTPase factors can reassemble the ribosomes to an active state, whose structure was disrupted by EDTA chelation of magnesium ions, indicating that these two factors can reversibly induce the ribosome structure to an intact state. The findings are essential for the bottom-up construction of synthetic cells.

### 摘要（中文）

当在体外组装大肠杆菌核糖体时，调节孵育温度和镁离子浓度一直是关键步骤，这对于获得有活性的大亚基至关重要。本研究致力于开发一种可在近生理条件下进行的单步程序，此时无细胞蛋白质合成处于活跃状态。我们发现GTP酶因子EngA和ObgE能够补偿温度和镁离子浓度的变化。在这些因子存在下，核糖体组装和翻译过程成功整合到了重组的无细胞蛋白质合成系统中。此外，我们发现这些GTP酶因子可以将因EDTA螯合镁离子而结构遭到破坏的核糖体重新组装至活性状态，表明这两种因子能够可逆地诱导核糖体结构恢复完整。这些发现对于自下而上构建合成细胞至关重要。


### 核心发现


- EngA和ObgE两种GTP酶可补偿温度与镁离子变化，实现核糖体单步体外组装。

- 在EngA和ObgE辅助下，核糖体组装与翻译成功整合入无细胞蛋白合成系统。

- EngA和ObgE能可逆重组装被EDTA破坏的核糖体，恢复其完整活性结构。



**关键词**：Escherichia coli · Ribosomes · Escherichia coli Proteins · Magnesium · Protein Biosynthesis · GTP Phosphohydrolases · Cell-Free System · Temperature · Monomeric GTP-Binding Proteins · Edetic Acid


---


## 31. Phosphoserine as an Alternative Energy Source for E. coli Cell-Free Protein Synthesis with Increased Yield and Prolonged Activity.
...
能量供应分子对生物过程至关重要，尤其是转录和翻译。无细胞蛋白合成（CFPS）系统是体外蛋白生产的有力工具，灵活应用于高通量筛选到治疗性蛋白生产。然而，CFPS中的能量再生仍具挑战，尤其在大规模或资源受限环境。本研究引入磷酸丝氨酸（PS）作为简单、经济的替代次级能源，可部分或完全取代常用但供应有限的三磷酸甘油酸（PGA）。在基于大肠杆菌裂解液的CFPS中补充PS，蛋白产量提高2倍，成本效率显著提升，且增强效果在不同裂解液批次和蛋白靶标中保持一致。我们还提供了低成本的CFPS配方，保留蛋白合成能力，使系统更适用于资源有限场景。更高的PS浓度虽降低最终蛋白产量，却可将反应持续时间延长超过2倍。因此，PS作为替代能量供体可实现蛋白产量、反应寿命与成本的可调平衡。数据支持PS通过丝氨酸生物合成途径调节通量，平衡丝氨酸生成与糖酵解以支持ATP再生，从而增强CFPS。最后，我们在合成细胞中验证了优化配方的应用。本研究确立了PS作为大肠杆菌裂解液CFPS系统有前景的能源，为面向多样临床、生物技术和工业需求的增效经济蛋白合成平台奠定基础。


### 核心发现


- 磷酸丝氨酸可完全或部分替代PGA作为能源，使CFPS蛋白产量提高2倍。

- PS增强效果跨裂解液批次和蛋白靶标一致，并给出低成本配方适用资源受限环境。

- 高浓度PS虽降低产量但延长反应时间2倍以上，实现产量、时长和成本可调。
...
## 32. An extracellular, optogenetic antibody platform for stimulus-gated antigen recognition and modulation of cell behavior.
...
本文提出了extrabody，一种可激活的模块化抗体平台，可通过光遗传学或化学手段重新组装分割抗体片段，实现可诱导的胞外抗原识别。我们展示了该平台对GFP、mCherry以及肿瘤相关抗原EGFR和HER2等多种靶标的兼容性，并证实纳米抗体和scFv来源的片段均支持光依赖性重构。Extrabody能够实现输入门控的细胞间相互作用与抗原转移，从而提供对细胞间通讯的外部控制。通过整合synNotch受体与嵌合抗原受体（CARs），进一步实现了对下游反应的双输入调控，包括基因表达、细胞因子释放和细胞毒性。总之，这些结果确立了extrabody作为一个多功能且可推广的接口，用于外部控制的细胞通讯与合成信号传导。


### 核心发现


- 光或化学信号触发分割抗体片段重装，实现胞外抗原识别的可诱导控制。

- 平台兼容纳米抗体和scFv，对GFP、EGFR、HER2等多种靶标均有效。

- 结合synNotch和CAR系统，实现双输入调控基因表达、细胞因子释放和细胞毒性。



**关键词**：Optogenetics · Humans · Erb-b2 Receptor Tyrosine Kinases · ErbB Receptors · Cell Communication · Animals · Single-Chain Antibodies · Single-Domain Antibodies · Antigens


---


## 33. Engineering a robust and autonomous biological funneling for efficient valorization of lignin-related aromatics.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-Jun-17 |
| **作者** | Xu Jianping, Zhang Jian, Wang Xiaotong, Zhao Tengfei, Qi Qingsheng, Wang Qian |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-74424-y) |
| **DOI** | `10.1038/s41467-026-74424-y` |
...
木质素相关芳香族化合物是前景广阔的可再生原料，但其微生物转化为有用化合物受到底物毒性和代谢流不平衡的根本限制。在此，我们在谷氨酸棒杆菌中构建了一个自主微生物系统，该系统整合了进化工程与多重动态调控，以克服这些限制。该系统具备一个稳健的、进化的底盘，增强了细胞防御机制和升级了芳香族外排装置。我们工程设计了一个分层、多输入调控网络，能够实时、响应底物并精确控制代谢途径。该系统实现了99.89%的多种木质素相关芳香族化合物向原儿茶酸的转化，随后原儿茶酸滴度达到53.40 g L⁻¹。这进一步支持了下游高附加值化学品的高滴度生产，包括顺,顺-粘康酸和β-酮己二酸，滴度分别达到51.90 g L⁻¹和38.33 g L⁻¹。本工作为木质素原料的自主生物加工成可持续高附加值产品建立了一种通用且可规模化的范式。


### 核心发现


- 构建整合进化工程与多重动态调控的自主系统，克服底物毒性和代谢流不平衡。

- 实现99.89%转化率，高产原儿茶酸(53.40 g/L)及下游化学品(粘康酸、酮己二酸)。

- 设计分层多输入调控网络，实现实时底物响应与精准代谢控制，建立可规模化范式。
...
## 34. Guide RNA reprogramming facilitates minimized tracrRNA-dependent off-target and versatile CRISPR/Cas9 engineering.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-Jun-17 |
| **作者** | Yu Wenxia, Chen Jun, Guo Junfan, Yu Fang, Wang Ge, Lin Jianxiang, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-74520-z) |
| **DOI** | `10.1038/s41467-026-74520-z` |
...
当前CRISPR-Cas9系统虽具创新性，但面临序列非依赖的非经典脱靶等安全与实用挑战。我们证明指导RNA（gRNA）中的crRNA:tracrRNA双链体既可拆分又可重编程，但这一特性使得含crRNA样序列的内源RNA能劫持任何gRNA，引发低频却普遍的依赖tracrRNA的脱靶（TDO）效应。利用高通量gRNA变体筛选训练的机器学习，我们推导出最优gRNA设计规则并工程化与人类/小鼠转录组错配的crRNA变体，从而将TDO最小化。借助可拆分与重编程性，我们开发出用于CRISPRa介导mRNA检测的可重编程tracrRNA，并重新设计支架以遏制无PAM的Cas9“自编辑”。我们还创建了独立表达gRNA（segRNA）平台，其拆分tracrRNA与非重复串联crRNA可实现干细胞中多达六个基因的多重编辑及功能增强子注释。这项发现揭示了一个此前被忽视的脱靶机制，并提供了提升CRISPR系统安全性与实用性的多种策略。


### 核心发现


- 揭示gRNA中crRNA:tracrRNA双链体可拆分重编程，并由此引发依赖tracrRNA的非经典脱靶效应（TDO）。

- 通过机器学习优化gRNA设计规则，工程化crRNA变体显著降低TDO。

- 开发可重编程tracrRNA、无PAM自编辑支架及segRNA平台，实现mRNA检测、多重基因编辑和增强子注释。
...
## 35. Dango: Predicting higher-order genetic interactions.
...
高阶遗传相互作用对理解表型变异的分子机制具有深远影响，但至今仍缺乏充分表征。大多数研究仅关注成对相互作用，因为在庞大的组合空间中进行高通量筛选极具挑战性。本文开发了 Dango，一种基于自注意力超图神经网络的计算方法，可预测基因群组之间的高阶遗传相互作用。作为概念验证，我们提供了酿酒酵母中超过 4 亿个三基因相互作用的预测，极大地扩展了其定量景观。Dango 能准确预测三基因相互作用，并揭示与细胞生长相关的生物学功能。我们进一步整合蛋白质嵌入和不确定性估计，以提高生物学相关性和可解释性。此外，预测的相互作用可作为不同条件下生长响应的遗传标记。总之，Dango 使得更完整地绘制塑造表型多样性的复杂遗传相互作用图谱成为可能。本论文透明的同行评审过程记录见补充信息。


### 核心发现


- 开发了基于自注意力超图神经网络的 Dango 方法，可预测高阶遗传相互作用。

- 在酵母中预测了超过 4 亿个三基因相互作用，并揭示了细胞生长相关功能。

- 整合蛋白质嵌入和不确定性估计，提升了模型的生物学相关性和可解释性。



**关键词**：Saccharomyces cerevisiae · Epistasis, Genetic · Phenotype · Neural Networks, Computer · Computational Biology · Models, Genetic · Gene Regulatory Networks


---


## 36. Fusing imaging and metabolic modeling via multimodal deep learning in ovarian cancer.

| 字段 | 内容 |
|---|---|
| **期刊** | Cell Syst |
| **发表日期** | 2026-Jun-17 |
| **作者** | Eftekhari Noushin, Verma Suraj, Saha Aninda, Zampieri Guido, Sawan Saladin, Occhipinti Annalisa, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.cels.2026.101594) |
...
整合基因型（如转录组学）、表型（如影像）和肿瘤微环境（如代谢组学）对阐明卵巢癌的分子基础至关重要。然而，当仅有少量共同样本时，缺乏稳健的多模态整合方法。本研究从转录组数据出发构建患者特异性代谢模型，并将其与影像数据整合。我们证明，这种前所未有的多模态整合可改善生存预测，并使预测具有机制可解释性。利用转录组学、通量组学和三维计算机断层扫描（CT）影像数据的不同组合，评估了该方法的稳健性，并基于风险对患者进行了正确分层。与广泛采用的转录组-影像方法相比，融合代谢建模与影像和转录组学显著提高了模型准确性，并揭示了关键的代谢反应。本方法具有普适性，可应用于其他拥有配对影像-转录组数据的癌症类型。本文的透明同行评审过程记录见补充信息。


### 核心发现


- 首次将代谢模型与影像数据多模态整合，改善了卵巢癌生存预测并提供机制解释。

- 转录组、通量组与CT影像的组合分析能够稳健地进行患者风险分层。

- 融合代谢建模显著优于传统转录组-影像方法，揭示了关键代谢反应。



**关键词**：Ovarian Neoplasms · Female · Humans · Deep Learning · Metabolomics · Tomography, X-Ray Computed · Tumor Microenvironment · Transcriptome · Gene Expression Profiling · Multiomics


---


## 37. Phased fragility and stability of non-genetic B cell states in the germinal center accelerate the genetic evolution of antibodies.

| 字段 | 内容 |
|---|---|
| **期刊** | Cell Syst |
| **发表日期** | 2026-Jun-17 |
| **作者** | Xiang Mark Y, Vaidehi Narayanan Haripriya, Kesarwani Vaibhava, Vanheusden Rohan, Wang Tiffany, Hoffmann Alexander |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.cels.2026.101590) |
| **DOI** | `10.1016/j.cels.2026.101590` |
...
疫苗反应依赖于B细胞的达尔文式遗传进化以产生高亲和力抗体。然而，B细胞在搜寻抗原和T辅助细胞的过程中获得非遗传异质性，但随后其非遗传细胞状态在增殖性克隆爆发中保持稳定。我们通过开发一个整合了大量免疫学知识的数学模型，探索了这种非遗传变异动态控制的功能性后果。我们发现，B细胞命运决定的变异性并不会削弱，反而会加速亲和力成熟，因为它允许高亲和力异常细胞逃避浆细胞分化，并启动进一步的达尔文进化轮次。在克隆爆发期间，非遗传细胞状态的稳定性进一步促进了它们的扩增。由此产生的模型正确预测了具有改变B细胞命运决定特征的小鼠品系中涌现的疫苗反应特性。我们的工作调和了经典的B细胞克隆选择理论与实验观察到的非遗传变异性，并提供了一个可解释的基于知识的建模框架，以支持个性化疫苗接种策略。


### 核心发现


- B细胞命运决定的变异性使高亲和力细胞逃避浆细胞分化，从而加速亲和力成熟。

- 克隆爆发期间非遗传细胞状态稳定，促进高亲和力B细胞的扩增。

- 模型准确预测小鼠疫苗反应，调和了克隆选择理论与非遗传变异性。



**关键词**：Germinal Center · Animals · B-Lymphocytes · Mice · Antibodies · Cell Differentiation · Evolution, Molecular


---


## 38. Pancreatic islet oscillation rhythmicity arises from δ and α cell interactions.
...
葡萄糖刺激的胰岛激素分泌与胞质 Ca²⁺ 振荡密切相关，后者源自复杂的细胞内和细胞间信号传导。与外围 α 细胞交错分布的 δ 细胞是重要的旁分泌调节因子，但其在塑造 Ca²⁺ 振荡中的作用尚不清楚。本研究显示，δ-α 细胞相互作用导致了葡萄糖诱导的 Ca²⁺ 振荡模式的变异性。δ 细胞释放的生长抑素以 α 细胞数量依赖的方式延长振荡周期。药理学和光遗传学扰动 δ-α 相互作用可触发振荡模式转变。连续调节 δ-α 耦合强度使快振荡胰岛转变为混合振荡和慢振荡。数学模型表明这一快-混合-慢转变是霍普夫分岔。在体内，血糖与振荡模式相关：高血糖伴随慢振荡，而正常血糖伴随快振荡。这些发现解释了 δ 和 α 细胞如何塑造胰岛 Ca²⁺ 动力学，该现象由胰岛多样的细胞构筑所决定。本文的透明同行评审过程记录见补充信息。


### 核心发现


- δ 细胞释放的生长抑素以 α 细胞数量依赖的方式延长 Ca²⁺ 振荡周期。

- 连续调节 δ-α 耦合强度可驱动快振荡胰岛向混合和慢振荡模式转变。

- 数学建模揭示振荡模式转变为霍普夫分岔，且体内高血糖对应慢振荡、正常血糖对应快振荡。



**关键词**：Animals · Glucagon-Secreting Cells · Calcium Signaling · Calcium · Islets of Langerhans · Somatostatin-Secreting Cells · Glucose · Cell Communication · Somatostatin · Mice


---


## 39. A community reconstruction of Chinese hamster metabolism and structural systems biology elucidate metabolic rewiring in lactate-free CHO cells.
...
基因组规模代谢模型（GEMs）是研究细胞代谢不可或缺的工具。我们提出了iCHO3K，一个基于社区共识、经人工审编的中国仓鼠代谢网络重建。iCHO3K涵盖11,004个反应，关联3,597个基因，并通过纳入3,489个蛋白质结构及对超过70%的7,377种代谢物进行物理化学描述，实现了结构感知分析。我们应用iCHO3K来关联一个工程化消除乳酸分泌的补料分批CHO细胞系的转录组和代谢组数据。模型显示，糖酵解通量降低，三羧酸循环（TCA）活性增强，胞内NADH和磷酸烯醇式丙酮酸（PEP）升高，与实验测量一致。利用iCHO3K的结构注释，我们评估了NADH和PEP在早期糖酵解酶中的潜在脱靶结合，并发现PEP与磷酸果糖激酶之间存在一个推定的别构相互作用，揭示了葡萄糖摄取和糖酵解通量下降的结构机制。总之，iCHO3K为系统多组学整合、改进通量预测和结构引导的机理洞察提供了框架，推动了CHO细胞工程和生物制造的发展。本文的透明同行评审记录包含在补充信息中。


### 核心发现


- 构建了整合蛋白结构和代谢物属性、支持结构感知分析的社区共识代谢模型iCHO3K。

- 揭示无乳酸CHO细胞中糖酵解降低、TCA循环增强及NADH和PEP积累的代谢重编程特征。

- 预测PEP可能通过别构抑制磷酸果糖激酶，为糖酵解通量下降提供了结构层面的机理解释。



**关键词**：Animals · CHO Cells · Cricetulus · Systems Biology · Cricetinae · Glycolysis · Metabolic Networks and Pathways · Lactic Acid · Metabolomics · NAD


---


## 40. Modular Scalable Synthetic Gene Circuits for Complex Functions Within Minimal Computational Layers in Human Cells.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-Jun-16 |
| **作者** | Roas Keren, Kovalski Ilanit, Mouhadeb Odelia, Aminov Tamar, Weinstein-Marom Hadas, Nissim Lior |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-74408-y) |
...
在合成生物学中，工程化哺乳动物细胞以执行复杂的遗传程序仍然是一项重大挑战。合成基因电路通常通过级联的计算层来实现复杂的程序。然而，这些架构需要大量的正交部件，增加了遗传负载，并消耗细胞资源，从而限制了功能和可扩展性。我们提出了一个模块化设计框架，用于设计可扩展的基因电路，能够在更少的计算层内执行复杂功能。该平台整合了基于正交反式剪接的AND门、用于可调调控的天然-合成混合启动子，以及实现抑制逻辑的人工合成microRNA。利用这种方法，我们构建了复杂的电路，包括一个三输入组合逻辑门、一个半加器、一个全加器，以及一个动态的3-to-1多路复用器，该复用器带有一个专用的选择器过载状态输出，仅当两个选择器输入同时激活时才会产生。通过最小化计算层数同时保持功能，该策略解决了基因电路工程中的可扩展性障碍，并扩展了在生物医学、生物技术和基础生物学中的适用性。


### 核心发现


- 提出整合正交反式剪接AND门、混合启动子和合成microRNA的模块化框架，在更少计算层实现复杂功能。

- 成功构建三输入组合逻辑门、半加器、全加器及带有选择器过载状态输出的3-to-1多路复用器。

- 通过最小化计算层数，突破基因电路可扩展性瓶颈，拓展生物医学与生物技术应用。
...
## 41. Emission dipole orientation reveals dynamic single-molecule interactions with 2D crystals at solvent interfaces.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-Jun-16 |
| **作者** | Guo Wei, Chen Tzu-Heng, Ronceray Nathan, Mayner Eveline, Watanabe Kenji, Taniguchi Takashi, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-74191-w) |
...
直接观察单个荧光发射体对于研究量子材料、化学反应和生物系统至关重要。然而，当前的单分子追踪方法仅关注分子的定位，忽略了分子构型和取向。在本工作中，我们引入了一种高通量偏振单分子定位显微镜，能够以纳米级精度同时解析单个荧光发射体的位置和发射偶极取向。利用六方氮化硼（h-BN）与有机溶剂之间的界面作为具有挑战性的平台，我们在室温下捕获了超过10⁵个荧光事件，并揭示了不同的分子相互作用动力学。测量的偶极取向与h-BN晶格的三重（C₃）旋转对称性一致，并且液体环境中的分子动力学可以通过电化学方式进行调控，这为按需控制量子发射体提供了一种途径。我们还发现固液界面处的横向扩散比固态发射体的扩散更为动态。这种同时跟踪分子构象和光物理的方法推进了对单分子相互作用的理解，并使得通过二维材料进行实时传感成为可能。


### 核心发现


- 高通量偏振单分子定位显微镜实现位置与偶极取向纳米级同时解析。

- h-BN界面分子偶极取向遵循C3对称性，可通过电化学动态调控量子发射体。

- 固液界面横向扩散远强于固态，推进单分子相互作用理解和实时传感。
...
## 42. A CRISPR knockout mouse library for functional genomics in influenza research.
...
在整体动物模型中对宿主因子进行功能验证是病毒学领域的一个主要瓶颈；它阻碍了将体外研究数据转化为对病毒生命周期和发病机制更深入的理解。为应对这一挑战，我们开发了一个针对甲型流感病毒的系统性体内筛选平台。该平台包含一个由84个CRISPR-Cas9基因编辑小鼠系组成的文库，这些小鼠系靶向通过文献和体外小干扰RNA（siRNA）筛选研究优先选取的宿主因子。利用这一资源，我们鉴定出17个宿主因子，其基因缺失可赋予对甲型流感病毒感染的抵抗力。对其中两个因子Arhgef28和Lasp1的进一步研究揭示了针对甲型流感病毒的不同保护机制。我们将此小鼠文库作为在生理相关条件下研究病毒-宿主相互作用的强大平台提供给研究界。


### 核心发现


- 开发了包含84个CRISPR基因编辑小鼠系的系统性体内筛选平台，用于鉴定甲型流感病毒宿主因子。

- 通过该平台发现17个宿主因子缺失可抵抗甲型流感病毒感染，其中Arhgef28和Lasp1具有不同保护机制。

- 该小鼠文库为在生理背景下研究病毒-宿主相互作用提供了强大且可共享的资源。
...
## 43. Rapid Affinity Characterization of Cell-Free Expressed Nanobodies Directly in Lysate with Biolayer Interferometry via Antigen-Immobilized Biosensors.
...
近年来，已有少数研究展示了将无细胞蛋白质表达（CFE）与生物层干涉术（BLI）相结合的优势，用于快速动力学表征新型捕获蛋白（如纳米抗体），以构建序列-功能数据集。此类数据可为迭代的、机器学习引导的模型提供信息，从而优化与目标抗原的结合。在这项工作中，我们通过提出一种新的 CFE-BLI 工作流程来扩展这些方法，该流程翻转了 BLI 的结合方向，使目标蛋白固定在生物传感器表面，而 CFE 表达的纳米抗体直接在裂解液中测量。这扩大了 CFE-BLI 筛选的适用范围，使其能够涵盖更广泛的纳米抗体-抗原对，例如多聚体靶蛋白。我们的工作流程利用一种荧光 HaloTag 融合蛋白，能够精确进行蛋白质定量，同时放大较小分子量结合物（如纳米抗体）的 BLI 信号。我们在此使用两种先前已表征的纳米抗体（I3 和 2Rs15d）测试了我们的工作流程，并表明我们的 CFE-BLI 筛选方法可以计算出与先前报道相似的平衡解离常数（KD）。我们进一步将工作流程与现代组合诱变工作流程相结合，筛选了 12 种先前未表征的 I3 纳米抗体突变体以提高亲和力，展示了其用于快速、高迭代、主动学习引导的蛋白质工程活动的潜力，尤其是在复杂背景中呈现的多聚体靶标上。


### 核心发现


- 提出翻转 BLI 方向的 CFE-BLI 新方法，将靶蛋白固定于传感器，直接在裂解液检测纳米抗体。

- 利用荧光 HaloTag 融合蛋白实现精确定量与信号放大，使流程适用于多聚体靶标。

- 成功筛选 12 种 I3 纳米抗体突变体，证明其可用于主动学习引导的快速蛋白质工程。
...
## 44. Centromeres and telomeres as rheological probes of the human cell nucleus.

| 字段 | 内容 |
|---|---|
| **期刊** | Biophys J |
| **发表日期** | 2026-Jun-16 |
| **作者** | Clavijo Alexis S, Ionov Steven, Zidovska Alexandra |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.bpj.2026.02.023) |
| **DOI** | `10.1016/j.bpj.2026.02.023` |
...
人类基因组包含生命必需的遗传信息，通过生物学中心法则控制所有细胞过程。它是活性聚合物的一个典型实例，但其在细胞核内动态自组织的物理原理仍未知。本研究通过追踪线性染色体聚合物的中心（着丝粒）和自由端（端粒）的运动，以及其核环境的流变学，探究活人细胞中基因组的聚合物性质。我们的发现表明，端粒的位移比着丝粒大10倍，远超聚合物理论的预测。我们发现这种出人意料的大差异源于着丝粒和端粒定位在独特的核环境中，二者在生物活性和材料流变学上均截然不同。前者位于基因组的沉默区域，后者则定位于转录活跃区域。流变学分析显示，着丝粒嵌入弹性环境，而端粒周围为粘性环境，这直接影响各自运动的时间尺度和长度尺度。我们的结果提示核异质性在基因组动力学中起关键作用，并通过生化扰动核结构（如异染色质和核斑点）加以证实。最后，通过低渗冲击使核环境均匀化后，我们观察到着丝粒和端粒运动趋于一致，验证了我们的假说。我们的观察表明，核环境异质性直接影响局部基因组运动的时间尺度和长度尺度，进而可能影响整个细胞核内时空基因调控。


### 核心发现


- 端粒的核内位移比着丝粒大10倍，远超经典聚合物理论预测。

- 着丝粒定位于弹性环境，端粒定位于粘性环境，导致运动尺度与时间尺度的差异。

- 核环境异质性是基因组运动差异的主因，低渗均质化实验使两者运动趋于一致。



**关键词**：Humans · Telomere · Centromere · Rheology · Cell Nucleus · Viscosity · HeLa Cells


---


## 45. Single-cell parameter inference reveals kinetic heterogeneity in synthetic mammalian gene expression.

| 字段 | 内容 |
|---|---|
| **期刊** | Biophys J |
| **发表日期** | 2026-Jun-16 |
| **作者** | Azukas Joshua, Krzysztoń Rafał, Helenek Christopher, Carter Simon, Li Stan Xiaogang, Balázsi Gábor, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.bpj.2026.02.017) |
| **DOI** | `10.1016/j.bpj.2026.02.017` |

### 摘要（英文）

Natural fluctuations in gene expression ("noise") impact many cell processes such as differentiation, proliferation, and apoptosis. Negative feedback gene circuits, a gene self-repressing its own transcription, e.g., through protein binding, are often used to reduce the single-gene expression noise and control its impact on investigated phenomena. However, the detailed, quantitative understanding of how temporal and population-level noise is affected by natural mammalian gene expression processes and how this dependence is modified by transcriptional negative feedback is lacking. Here, we present a comprehensive framework combining live-cell imaging of single-cell arrays with likelihood-free inference methods to characterize stochastic gene expression in the time domain and at single-cell resolution. We investigate the temporal and population-level variability in protein levels driven by a negative feedback circuit and compare the results with constitutive expression where the feedback mechanism is broken. We use inferred kinetic rate parameters to validate several hybrid stochastic/deterministic models at their predictive capacity regarding coefficient of variation (CV) versus mean protein-level relation. Our analysis shows significant differences between temporal and population-level noise profiles. We show how feedback strength and cooperativity quantitatively control not only mean protein levels per cell but also their variation range within cell population (CVp) and across time in individual cells (CVt).

### 摘要（中文）

基因表达的自然波动（“噪声”）影响许多细胞过程，如分化、增殖和凋亡。负反馈基因回路，即基因通过蛋白质结合等方式自我抑制其自身的转录，常被用来降低单基因表达噪声并控制其对所研究现象的影响。然而，对于哺乳动物天然基因表达过程如何影响时序和群体水平噪声，以及这种依赖性如何被转录负反馈所改变，目前仍缺乏详细的定量理解。在此，我们提出了一个综合框架，将单细胞阵列的活细胞成像与无似然推断方法相结合，在时间域和单细胞分辨率下表征随机基因表达。我们研究了由负反馈回路驱动的蛋白质水平在时序和群体上的变异性，并将结果与反馈机制被破坏的组成性表达进行了比较。我们利用推断的动力学速率参数，验证了几种混合随机/确定性模型在变异系数（CV）与平均蛋白质水平关系方面的预测能力。我们的分析显示时序噪声谱和群体噪声谱之间存在显著差异。我们展示了反馈强度和协同性如何定量地控制每个细胞的平均蛋白质水平，以及它们在细胞群体内的变异范围（CVp）和单个细胞随时间的变异（CVt）。


### 核心发现


- 开发了结合活细胞成像与无似然推断的框架，实现单细胞分辨率下随机基因表达的时序表征。

- 揭示时序噪声与群体噪声谱显著不同，负反馈回路导致蛋白质水平的时序和群体变异性差异。

- 反馈强度和协同性定量控制平均蛋白水平、细胞群体变异范围（CVp）和单个细胞时序变异（CVt）。



**关键词**：Kinetics · Animals · Single-Cell Analysis · Stochastic Processes · Single-Cell Gene Expression Analysis · Gene Expression Regulation · Feedback, Physiological


---


## 46. Interplay between Rac1/RhoA and actin waves in giant epithelial cells: Experiment and theory.

| 字段 | 内容 |
|---|---|
| **期刊** | Biophys J |
| **发表日期** | 2026-Jun-16 |
| **作者** | Berthoz Rémi, Li He, André Marie, Lieb Michèle, Hinderling Lucien, Grädel Benjamin, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.bpj.2025.10.031) |
| **DOI** | `10.1016/j.bpj.2025.10.031` |
...
肌动蛋白-肌球蛋白细胞骨架是体内外细胞形状变化的关键驱动力，其组织依赖于肌动蛋白装配及与肌球蛋白的相互作用，并受Rac1和RhoA等小Rho GTP酶调控。为揭示骨架组织原理，我们在增大的扁平皮质层中结合活细胞显微术与理论分析肌动蛋白模式，从而表征其在远离侧边界和细胞连接的连续皮质面上的演化。在巨型MDCK上皮细胞中观察到肌动蛋白-肌球蛋白应力纤维与传播波，类似结构在REF52成纤维细胞中也存在。应力纤维呈静止态，与Rac和RhoA活性均匀分布相关；波以约1微米/分钟的速度传播，伴随肌动蛋白、Rac和活性RhoA的密度变化，部分波可运输细胞组分或在边缘产生突起。波传播的核心特征可由肌动蛋白与Rac的极性反应-扩散系统描述，且两波碰撞会互相湮灭。细胞内肌球蛋白活性并非波产生所必需，反而倾向于抑制波；相应地，局部激活RhoA会减慢或停止并破碎波。这些结果突显了肌动蛋白-肌球蛋白与Rho GTP酶耦合如何生成多样的骨架结构与动力学。


### 核心发现


- 在巨型上皮细胞中，肌动蛋白传播波伴随Rac与活性RhoA密度波动，应力纤维与均匀的Rac/RhoA活性分布相关。

- 波传播的核心特征被肌动蛋白和Rac的极性反应-扩散系统捕获，且两波碰撞时发生湮灭。

- 肌球蛋白活性对波的出现非必需且具抑制作用，局部激活RhoA可减慢、停止并破碎波。



**关键词**：Animals · Dogs · Actins · Epithelial Cells · rhoA GTP-Binding Protein · rac1 GTP-Binding Protein · Madin Darby Canine Kidney Cells · Models, Biological · Stress Fibers · Myosins


---


## 47. Characterization of transient elasticity in neutrophils.

| 字段 | 内容 |
|---|---|
| **期刊** | Biophys J |
| **发表日期** | 2026-Jun-16 |
| **作者** | Waugh Richard E, Lomakina Elena |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.bpj.2025.10.041) |
| **DOI** | `10.1016/j.bpj.2025.10.041` |
...
白细胞的可变形性对其功能至关重要，因为它们在通过血管系统、黏附并变形于内皮上，特别是在微血管中时必须经历变形。现有的细胞力学描述无法完全解释细胞行为，即使细胞处于被动状态也是如此。我们提供了新的证据，表明在细胞持续变形进入圆柱形微管的过程中，细胞内部存在持续的弹性应力贡献。我们建立了一个基于瞬态弹性的细胞行为模型，并通过实验测量以材料系数的形式表征了细胞特性。我们采用一个近似的细胞变形模型，从实验测量中估算出每个系数。细胞响应的弹性部分表现出应变硬化，并被建模为不可压缩的Gent材料，该材料包含两个材料常数：一个弹性模量（约28 ± 7 Pa）和一个应变硬化参数，对应约80%的最大弹性伸长。细胞稳定流入微管的过程反映了弹性应力的力依赖性松弛以及潜在的细胞质粘度（32.1 ± 0.2 Pa·s）。应力松弛模型包含两个系数：一个网络稳定性系数，对应于零应力下网络自发性松弛的速率（0.62 ± 0.49 s⁻¹），以及一个表征松弛速率对应力依赖性的常数（0.0026 ± 0.0014 Pa⁻¹）。这一新模型为被动状态下中性粒细胞对施加力的力学响应提供了全面的描述，并为白细胞及类似细胞对施加力的力学响应建模建立了新的范式。


### 核心发现


- 发现中性粒细胞在持续变形中存在持续弹性应力贡献，并建立了瞬态弹性模型来表征其力学行为。

- 细胞弹性响应呈现应变硬化，可用包含弹性模量和最大伸长参数的不压缩Gent材料模型描述。

- 模型整合了力依赖的应力松弛和细胞质粘度，为被动白细胞力学响应提供了全面的新范式。



**关键词**：Neutrophils · Elasticity · Models, Biological · Stress, Mechanical · Viscosity · Biomechanical Phenomena · Humans · Animals


---


## 48. Automation and improvement of WBC mechanical profiling in deformability cytometry.

| 字段 | 内容 |
|---|---|
| **期刊** | Biophys J |
| **发表日期** | 2026-Jun-16 |
| **作者** | Kaliman Sara, Abuhattum Shada, Hartmann Benedikt, Guck Jochen |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.bpj.2025.10.007) |
...
变形性细胞术（DC）是一种强大的生物物理技术，能以低成本、高通量表征疾病相关的血细胞力学变化。活体白细胞（WBC）的机械表型分析因其在免疫应答中的关键作用而尤为重要，但以无标记方式可靠识别和分类WBC亚型仍是一大挑战。此前分析流程依赖专家手动设门，限制了可扩展性和重复性。本研究提出一种全自动、可推广的剪切流DC实验WBC分类框架，基于盒式滤波器和细胞群的无监督聚类。两者均利用高精度分割导出的细胞形状特征以及明场图像导出的细胞纹理特征。这一无监督方法不仅提高了重现性、缩短了处理时间，还克服了监督模型需要大量训练数据且在不同成像条件下性能下降的关键局限。我们通过六组实验比较手动设门与自动分类获得的细胞特征，验证了该方法。这些实验组包含不同供体、抗凝剂（EDTA和柠檬酸盐）、采血来源（毛细血管和静脉）以及设备亮度设置，每组进行五次重复测量。结果一致证实该方法在所有测试条件下对各类WBC的可靠性和鲁棒性。重要的是，此自动化流程可纳入通常被排除的带有膜突起的WBC，从而能对潜在活化的细胞进行形态学表征。此外，通过使用源自原始轮廓而非凸包的形状特征，我们提高了形态学准确性并降低了测量变异性。因此，该方法增强了WBC力学表型分析的准确性、一致性和可扩展性，实现了大规模队列的高通量分析。


### 核心发现


- 提出全自动无监督框架，利用形状和纹理特征实现白细胞亚型的可靠分类，克服了手动设门瓶颈。

- 方法在供体、抗凝剂、采血源和亮度等多变条件下均稳健，并首次纳入膜突起细胞以捕获活化状态。

- 采用原始轮廓而非凸包的特征显著提升形态精度，降低测量变异，支撑大规模高通量力学表型分析。



**关键词**：Leukocytes · Humans · Automation · Biomechanical Phenomena · Flow Cytometry · Clustering Algorithms · Mechanical Phenomena · Cell Shape · Reproducibility of Results


---


## 49. Modeling mechanochemical coupling in optogenetically activated cell layers.

| 字段 | 内容 |
|---|---|
| **期刊** | Biophys J |
| **发表日期** | 2026-Jun-16 |
| **作者** | Wörthmüller Dennis, Ziebert Falko, Schwarz Ulrich S |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.bpj.2025.10.002) |
...
在黏附细胞中，肌动球蛋白收缩性主要由 RhoA 信号通路调控，该通路可通过光遗传学控制。为了对此类系统中的力化学耦合进行建模，我们引入了一种基于间断伽辽金法的有限元框架，使我们能够在同一概念框架内处理细胞双联体、细胞链和单层细胞。黏附细胞层被建模为弹性基底上的主动收缩黏弹性固体，同时针对 Rho 通路考虑了不同模型，从可解析求解的简单线性链到后续只能数值求解的直接反馈模型。我们的模型预测了信号传播与耦合强度和黏弹性时间尺度的函数关系，并确定了最优细胞响应和波传播的条件。总体而言，该研究提供了对生物化学与力学如何协同参与黏附细胞通讯的系统性理解。


### 核心发现


- 提出基于间断伽辽金法的统一建模框架，可处理细胞双联体、链和单层的力化学耦合。

- 模型揭示耦合强度与黏弹性时间尺度决定信号传播，并确定最优细胞响应及波传播条件。

- 该研究系统阐明了生物化学与力学如何协同介导黏附细胞间的通讯。



**关键词**：Optogenetics · Models, Biological · Biomechanical Phenomena · rhoA GTP-Binding Protein · Cell Adhesion · Mechanical Phenomena · Animals · Elasticity


---


## 50. Single-molecule FRET and tracking of transfected biomolecules in living cells.

| 字段 | 内容 |
|---|---|
| **期刊** | Biophys J |
| **发表日期** | 2026-Jun-16 |
| **作者** | Anandamurugan Abhinaya, Eidloth Antonia, Frank Veronika, Wortmann Philipp, Schrangl Lukas, Lan Chenyang, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.bpj.2025.09.024) |
| **DOI** | `10.1016/j.bpj.2025.09.024` |
...
细胞中的蛋白质和DNA呈现不同构象状态，受与其他生物分子动态相互作用的影响，并取决于分子在细胞内的区室化定位。由于仪器限制、细胞自发荧光及追踪扩散分子的困难，细胞内区室特异性动态难以测量。本研究提出自下而上的工程方法，通过交替激光激发双通道（供体与FRET受体）追踪结合HILO显微镜，实现对转染蛋白质的细胞内追踪和时间分辨单分子FRET效率分析。采用已知染料位置的定制DNA验证策略，并比较显微注射（物理法）和链球菌溶血素-O介导的转染（生物法）。体外与细胞内测量对比表明，细胞环境使FRET效率改变约25%。同时评估了热休克蛋白Hsp90的细胞内单分子FRET迹线，其FRET效率分布与已知结构和体外分布大体一致，但存在明显差异。总之，FRET-TTB为研究活细胞中转染生物分子的蛋白质状态变化及其时间分辨细胞定位开辟了新途径。


### 核心发现


- 开发交替激发双通道追踪方法，实现活细胞中转染蛋白的时间分辨单分子FRET分析。

- 比较体外与细胞内测量表明，细胞环境能改变FRET效率约25%。

- Hsp90单分子FRET分析显示细胞内分布与体外结构大体一致但有差异，证明可研究蛋白状态变化及定位。



**关键词**：Fluorescence Resonance Energy Transfer · Transfection · DNA · HSP90 Heat-Shock Proteins · Cell Survival · Animals · Single Molecule Imaging · Humans


---


## 51. Many dynein teams collectively generate high forces during the transport of large organelles.

| 字段 | 内容 |
|---|---|
| **期刊** | Biophys J |
| **发表日期** | 2026-Jun-16 |
| **作者** | Wieland Simon, Steininger Christina, Gitschier David E, Kaiser Marius M, Gross Wolfgang, Chaudhary Abdullah R, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.bpj.2025.09.012) |
| **DOI** | `10.1016/j.bpj.2025.09.012` |
...
细胞器的运输对于维持细胞组织和功能至关重要。大型细胞器（直径数微米）的有效逆行运输需要多个动力蛋白马达产生高集体力。然而，大型细胞器运输时的精确力及其对货物尺寸的依赖性尚不清楚，且运输过程中有多少动力蛋白处于活性状态以及它们如何产生足以克服细胞质阻力的高集体力也未知。我们测量了直径1至5 μm的吞噬体在逆行运输中产生的力。力随吞噬体体积增加而增大，从最小吞噬体的低于10 pN到最大吞噬体的高达160 pN。这些力与细胞质阻力相匹配，使所有尺寸的吞噬体均能以25±4 nm/s的恒定速度快速运输。为证实产生如此高力需要大量动力蛋白，我们对分离吞噬体上的动力蛋白进行了标记和定量。发现在最大的吞噬体上有多达250个动力蛋白，且动力蛋白表面密度与吞噬体大小无关。我们利用细胞器周围微管分布的理论模型将动力蛋白数量与运输力相关联。该模型表明，由于较大的细胞器会使微管移位并弯曲，因此不成比例的大量动力蛋白可以处于活性状态，从而为大型吞噬体贡献高运输力。我们的结果表明，在大型细胞器运输过程中，许多动力蛋白以货物尺寸依赖的方式与多根微管相互作用，以产生足够大的运输力。


### 核心发现


- 吞噬体逆行运输力随体积增大，最大达160 pN，而所有尺寸吞噬体均以约25 nm/s的恒定速度运输。

- 最大吞噬体上动力蛋白数量高达250个，且表面密度恒定，表明数量随表面积线性增加。

- 理论模型揭示大细胞器使微管弯曲，导致更多动力蛋白被激活，产生不成比例的高集体力。



**关键词**：Dyneins · Animals · Biological Transport · Phagosomes · Microtubules · Biomechanical Phenomena · Models, Biological · Mechanical Phenomena


---


## 52. Direct mechanical communication of cellular to nuclear shapes in oocytes.

| 字段 | 内容 |
|---|---|
| **期刊** | Biophys J |
| **发表日期** | 2026-Jun-16 |
| **作者** | Vos Bart E, Vadapalli Yamini, Muenker Till, Jentoft Ida Marie Astad, Todisco Elena, Eskandari Mohammad Amin, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.bpj.2025.08.012) |
| **DOI** | `10.1016/j.bpj.2025.08.012` |
...
细胞质和核质的力学特性对细胞的正常稳健功能至关重要，并在理解力学信号如何传递至细胞核中起关键作用。本研究展示了卵母细胞的细胞与核形状之间存在显著的形态模仿现象，即在无细胞皮层与核直接接触的情况下，细胞核形状会跟随外部施加的变形。这一效应源于核质异常柔软且呈流体状，几乎不抵抗外部应变，而粘弹性细胞质驱动了形状传递。通过对水母、海星和小鼠卵母细胞的比较研究揭示，水母较低的细胞质弹性导致核形状模仿减弱，凸显了细胞质力学在核变形中的作用。


### 核心发现


- 卵母细胞在外力作用下细胞与核形态高度相似，且无需皮层与核直接接触。

- 形状传递源于核质极其柔软流体状，而粘弹性细胞质主导变形传导。

- 水母细胞质弹性低导致核形态模仿减弱，凸显细胞质力学对核变形的关键影响。



**关键词**：Animals · Oocytes · Cell Nucleus · Biomechanical Phenomena · Mice · Cytoplasm · Elasticity · Starfish · Mechanical Phenomena


---


## 53. Tumor suppressor genotype influences the extent and mode of immunosurveillance in lung cancer.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-Jun-15 |
| **作者** | Adler Keren M, Xu Haiqing, Gladstein Amy C, Irizarry-Negron Valerie M, Robertson Maggie R, Doerig Katherine R, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-74023-x) |
...
癌症驱动突变在肿瘤发展过程中对免疫监视的影响仍知之甚少。为更好理解肿瘤基因型对免疫监视的贡献，我们构建并验证了基于慢病毒的载体，可产生免疫原性逐渐增强的新抗原。该载体系统兼容自发性Cre调控的癌症模型、CRISPR/Cas9介导的体细胞基因组编辑以及肿瘤条形码技术。本研究表明，在致癌KRAS驱动的肺癌且新抗原强表达的背景下，抑癌基因型决定了免疫细胞招募的程度、新抗原沉默肿瘤的正向选择以及肿瘤生长。通过量化11个常失活抑癌基因在不同新抗原环境下对肿瘤生长的影响，我们发现抑癌基因失活的促生长效应与对免疫监视敏感性的增加相关。重要的是，某些基因型还显著且独立于其促生长效应改变了免疫监视敏感性。我们提出一种免疫编辑模型，其中抑癌基因失活与新抗原表达协同作用，塑造肿瘤免疫监视和免疫编辑，使得相同的新抗原根据遗传背景的不同而独特地调控肿瘤免疫编辑。


### 核心发现


- KRAS驱动肺癌中，抑癌基因型决定免疫浸润程度及新抗原沉默肿瘤的阳性选择。

- 抑癌基因失活的促生长效应与免疫监视敏感性的增加呈正相关。

- 部分基因型可独立于促生长效应显著改变免疫监视敏感性，重塑免疫编辑模式。
...
## 54. Cell polarity control by an unconventional G-protein complex in bacteria.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-Jun-15 |
| **作者** | Dinet Céline, Sebban-Kreuzer Corinne, Byrne-Kodjabachian Deborah, Lhospice Sébastien, Herrou Julien, Durbesson Fabien, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-74081-1) |
| **DOI** | `10.1038/s41467-026-74081-1` |
...
在捕食性土壤细菌黄色黏球菌中，细胞极性与运动由小GTP酶MglA及其调控因子MglB（潜在GAP）和RomRX（曾被提议为GEF）调控，但其确切作用仍不清楚。利用适用于荧光各向异性测量的荧光核苷酸，我们证明RomRX并非作为GEF发挥作用，而是作为结合MglA·GTP的效应蛋白。进一步通过酶学模型发现，MglB根据局部MglA·GDP浓度表现出GAP和GEF双重特性。此外，RomRX（特别是RomR）通过螯合MglB有效解离MglA-MglB复合物。这种低亲和力相互作用在保持GAP活性的同时，仍能在体内实现蛋白分区，细胞大小系统的仿生油乳滴实验证明了这一点。整合这些相互作用和以往发现的最小数学模型，再现了这三个因子之间的动态相互作用如何在黄色黏球菌细胞中形成可反转的极性轴。


### 核心发现


- RomRX并非GEF而是MglA·GTP的效应蛋白，否定了此前对其功能的推测。

- MglB兼具GAP和GEF双重活性，其功能转换依赖于局部MglA·GDP浓度。

- RomR通过低亲和力螯合MglB解离MglA-MglB复合物，维持GAP活性并实现蛋白空间分隔，共同建立可反转的极性轴。
...
## 55. Environmental "knees" and "wiggles" as strong stabilizers of species' range limits set by interspecific competition.

| 字段 | 内容 |
|---|---|
| **期刊** | PLoS Comput Biol |
| **发表日期** | 2026-Jun |
| **作者** | Shirani Farshad, Freeman Benjamin G |
| **原文链接** | [查看原文](https://doi.org/10.1371/journal.pcbi.1014336) |
...
种间竞争是否是设定物种分布边界的主要因素，一直存在争议。理论研究提出，沿环境梯度的种间竞争与分裂性基因流相互作用，可阻止生态相似物种在相遇处扩展分布。然而，这类分布边界的稳定性尚未得到充分解答。我们利用连续栖息地中适应性分布范围演化的确定性数学模型，表明若适合度相关性状的环境最优值在空间上近乎线性变化，由种间竞争设定的分布边界在进化上不太可能稳定；即在线性环境中，无扩散障碍或第三物种时，两竞争物种间的边界会不断向弱竞争方移动。我们证明，环境非线性特征，如“膝点”（knees）和“摆动”（wiggles）——即性状最优值梯度发生孤立的急剧变化或阶梯状变化——能够强烈稳定由竞争形成的边界。其机制在于此类非线性在每一物种边缘种群中造成分裂性基因流水平的差异，并配合阿利效应等过程，阻止极小种群在优势竞争者存在时建立。我们显示，这些非线性点上的边界稳定性对中等环境干扰具有稳健性；而快速高强度的气候变化是否能破坏边界稳定，取决于物种的竞争主导地位在非线性特征处如何变化。因此，我们的发现强调了预测气候变化响应时评估物种竞争能力的重要性，并指出了已确立的分布边界可能持续存在的地理区域，以及移动边界可能最终稳定的区域。


### 核心发现


- 线性环境梯度下，竞争形成的物种分布边界不稳定，会不断移向竞争弱势方。

- “膝点”“摆动”等环境非线性可强烈稳定竞争边界，依赖分裂性基因流差异与阿利效应。

- 非线性处的边界对中等干扰稳健，强气候干扰下稳定性取决于竞争主导地位变化。



**关键词**：Animals · Ecosystem · Models, Biological · Competitive Behavior · Gene Flow · Biological Evolution · Species Specificity · Computational Biology
