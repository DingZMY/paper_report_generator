# 系统/合成生物学文献周报 2026-W23

> **时间范围**：2026/05/25 ~ 2026/06/01
> **来源期刊**：Nature / Cell / Science 系列
> **收录论文**：24 篇（已翻译 24 篇）
> **生成时间**：2026-06-01 00:46 UTC

---



## 1. Single-phage profiling illuminates viral individuality during bacterial cell fate determination.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-May-30 |
| **作者** | Homaee Ehsan, Zhu Wenqing, Yao Tianyou, Golding Ido |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-73867-7) |
...
噬菌体感染后，细胞死亡（裂解）与病毒休眠（溶原）之间的选择，是遗传均一群体中细胞异质性出现的奠基性范例。宿主命运的确定源于每个细胞内多个病毒基因组的随机转录，但这一活动一直隐藏在通常止步于全细胞水平的实验探究之下。本文使用并行连续荧光原位杂交（par-seqFISH），继而对每个细胞内的噬菌体编码转录本进行空间聚类，在λ噬菌体同步感染大肠杆菌的过程中描绘单个噬菌体的转录活性。在全细胞水平上，转录动力学捕捉到裂解与溶原之间的发育选择，并进一步证明胞内病毒复制是分歧命运决定出现的必要条件。聚焦到单噬菌体层面，揭示了感染期间病毒活性的个体性。我们发现，虽然走向溶原的细胞展现出所有驻留噬菌体的共识活性，但裂解细胞中可能含有表现出溶原活性的噬菌体。这些发现支持了早前提出的“共感染噬菌体间达成共识是细胞休眠所必需”的假说。更广泛而言，我们的结果突显了识别同一遗传回路中物理上独立拷贝的活性如何涌现出全细胞行为的必要性。


### 核心发现


- 利用par-seqFISH实现单噬菌体转录活性分析，首次揭示噬菌体感染过程中的个体性行为。

- 发现裂解细胞中存在表现溶原活性的噬菌体，表明单噬菌体行为与细胞最终命运并非完全一致。

- 表明溶原决策依赖于共感染噬菌体的转录共识，从多拷贝遗传回路涌现出全细胞命运。
...
## 2. Brieflow: an integrated computational pipeline for high-throughput analysis of optical pooled screening data.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-May-30 |
| **作者** | Di Bernardo Matteo, Kern Roshan S, Cepeda Diaz Ana Karla, Mallar Alexa, Choi Samuel J, Nutter-Upham Andrew, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-73643-7) |
...
光学混合筛选（OPS）已成为功能基因组学中的一项强大技术，使研究人员能够将遗传扰动与大规模复杂细胞形态表型联系起来。然而，由于数据量巨大、多模态整合需求复杂以及缺乏标准化框架，OPS数据分析面临挑战。在此，我们推出了Brieflow，一个用于固定细胞光学混合筛选数据端到端分析的计算流程。我们通过重新分析一项CRISPR-Cas9筛选（涵盖5072个适合度相关基因，处理超过7000万个细胞及多个表型标记）展示了Brieflow的能力。为加速生物学解读，我们还推出了MozzareLLM，一个利用大语言模型识别表型聚类中的生物学过程并优先选出候选基因进行实验验证的框架。我们的联合分析恢复了现有分析方法遗漏的连贯生物模块，包括原研究未能识别的五个核心线粒体子程序。Brieflow的模块化设计和开源实现有助于整合新的分析组件，同时确保计算可重复性并提升高内涵表型筛选在生物学发现中的使用性能。


### 核心发现


- Brieflow实现固定细胞OPS高通量端到端分析，处理超7000万细胞并整合多表型标记。

- MozzareLLM借助大语言模型从表型聚类中自动识别生物过程，提升基因候选物优先级排序。

- 联合分析恢复遗漏的连贯生物模块，包括五个核心线粒体子程序。
...
## 3. International multisite implementation of distributed cell-free protein biomanufacturing to advance health and research equity.

| 字段 | 内容 |
|---|---|
| **期刊** | Sci Adv |
| **发表日期** | 2026-May-29 |
| **作者** | da Silva Severino Jefferson Ribeiro, Matthews Quinn, Cazaux Séverine, Vigar Justin R J, Santos Bárbara N R, Carvalho Deyse C M, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1126/sciadv.aeb7039) |
...
全球在研究和医疗能力获取方面的限制削弱了公平性、可持续性和韧性，尤其是在资源有限的环境中。分子诊断和生物治疗药物有望彻底改变医学；然而，我们对集中式生物制造和冷链物流的依赖限制了这些益处的可及性。这些限制反映了一项长期且未解决的全球性挑战。在此，我们与北美、南美和亚洲的研究团队一起，通过推动社区驱动的解决方案来挑战自上而下的范式，赋能服务不足的人群参与生物经济，使其能够在需要的时间和地点生产所需产品。我们的方法利用基于无细胞蛋白质合成和开源硬件的低负担生物制造，实现高价值生物制品（包括生长因子、疫苗和诊断酶）的本地按需生产，性能可与商业金标准媲美。该平台在全球10个地点实施，支持了针对全球相关病原体的患者试验。这些努力共同为一个全球包容的生物制造生态系统奠定了基础，使创新超越地理边界。


### 核心发现


- 基于无细胞蛋白质合成和开源硬件的分布式平台，实现了生长因子、疫苗和诊断酶的本地按需生产。

- 在10个国际地点实施并支持患者试验，性能达到商业金标准，展示了社区驱动制造模式的可行性。

- 该去中心化策略绕开集中制造和冷链依赖，显著提升了资源有限地区的健康与研究公平性。



**关键词**：Humans · Cell-Free System · Biotechnology


---


## 4. Oceanic mesoscale eddies enhance the Pacific Decadal Oscillation and its predictability.

| 字段 | 内容 |
|---|---|
| **期刊** | Sci Adv |
| **发表日期** | 2026-May-29 |
| **作者** | Gan Bolan, Wang Xin, Wu Lixin, Cai Wenju, Li Shujun, Wang Hong, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1126/sciadv.adz8486) |
| **DOI** | `10.1126/sciadv.adz8486` |
...
太平洋年代际振荡（PDO）深刻影响海洋生态系统、渔业和全球水文气候。传统上它被解释为海洋对大气随机强迫的盆地尺度响应，但其动力学是否涉及主动的海洋反馈仍未解决。通过一项史无前例的多世纪涡解析全球气候模拟，我们发现黑潮延伸体（KE）区域由中尺度涡驱动的大气异常对PDO的演变至关重要。在PDO冷位相期间，向北偏移弯曲的KE产生中尺度海表温度（SST）异常，增强了非绝热加热的横向梯度，驱动深层上升气流，冷却对流层中层并削弱其北部的斜压性。这抑制了瞬变涡动量通量，促成一个盆地尺度的低压异常，从而启动向暖位相的转变。同时，中尺度涡引起的垂直热输送维持了SST异常，为PDO提供了额外的记忆。这些耦合过程显著增强了PDO的可预测性。我们的发现突显了此前被低估的海洋涡旋的升尺度效应，为年代际气候变率提供了重要见解。


### 核心发现


- 黑潮延伸体中尺度涡驱动的大气异常是太平洋年代际振荡位相转换的关键触发因素。

- 中尺度海温异常通过调节加热梯度和瞬变涡动量通量，驱动盆地尺度大气环流响应。

- 中尺度涡垂直热输送能维持海温异常，为PDO提供额外记忆，从而增强其可预测性。
...
| **期刊** | Elife |
| **发表日期** | 2026-May-29 |
| **作者** | Wu Di, Maus Natalie, Jha Anupama, Yang Kevin, Wales-McGrath Benjamin D, Jewell San, et al. |
| **原文链接** | [查看原文](https://doi.org/10.7554/eLife.106043) |
| **DOI** | `10.7554/eLife.106043` |
...
前体mRNA的可变剪接（AS）在组织特异性基因调控中起关键作用，剪接缺陷与疾病相关。预测和操控可变剪接可揭示新调控机制并辅助治疗设计。我们提出TrASPr+BOS，一种结合贝叶斯优化的生成式AI模型，用于预测和设计实现组织特异性剪接结果的RNA。可变剪接预测Transformer（TrASPr）是一种多Transformer模型，可处理不同类型AS事件并泛化至未见细胞条件，随后作为预言机生成标注数据，训练贝叶斯优化剪接（BOS）算法以设计条件特异性剪接结果。我们展示了TrASPr+BOS优于现有方法，将组织特异性AUPRC提升至多1.8倍，并捕获组织特异性调控元件。我们验证了数百个预测的新型组织特异性剪接变异，并利用dCas13确认了新的调控元件。TrASPr+BOS是一种轻量且精准的方法，可供研究者探索或用于特定任务。


### 核心发现


- 提出TrASPr+BOS模型，集成多Transformer和贝叶斯优化，实现RNA剪接预测与条件特异性序列设计。

- 模型在组织特异性剪接预测中AUPRC提升至1.8倍，并成功捕获组织特异性调控元件。

- 实验验证数百个新预测的组织特异性剪接变异，并通过dCas13确认了新调控元件。



**关键词**：Humans · RNA Splicing · Bayes Theorem · Generative Artificial Intelligence · Alternative Splicing · Prediction Algorithms · Computational Biology · Algorithms · RNA Precursors


---


## 6. A T7 RNAP regulatory toolbox for cell-free network engineering and biosensing applications.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-May-28 |
| **作者** | Lee Pao-Wan, Mottaghi Seyed Saeed, Lugnier Matthis Guillaume, Maerkl Sebastian J |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-73811-9) |
...
T7 RNA聚合酶在合成生物学和生物技术领域应用广泛，但其精确模块化调控能力仍十分有限。本研究在无细胞系统中打造了一个由可编程合成阻遏物、激活物和生物传感器组成的T7 RNAP调控工具箱。该工具箱支持基于T7 RNAP的基因调控网络的可扩展设计，并能在一锅反应中快速、灵敏且多重检测多种生物分子，包括小分子药物、抗体和蛋白质。通过整合蛋白质设计流程，利用全合成结合剂生成生物传感器，展现了快速开发蛋白基传感器的潜力。我们构建了一种诊断用无细胞系统，将SARS-CoV-2刺突蛋白传感、基于基因调控的扩增、酶促扩增和葡萄糖检测相结合，实现了高灵敏度的即时检测。这项工作为构建响应广泛生物分子的基因回路提供了一个灵活可扩展的框架，并展示了工程化即时无细胞诊断分析的前景。


### 核心发现


- 构建了由可编程合成阻遏物、激活物和生物传感器组成的T7 RNAP调控工具箱。

- 实现了在单管反应中对小分子药物、抗体和蛋白的快速、灵敏、多重检测。

- 通过集成蛋白设计生成全合成结合剂传感器，并开发了结合多级扩增的SARS-CoV-2即时无细胞检测系统。
...
## 7. Invariance under quantum permutations rules out parastatistics.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-May-28 |
| **作者** | Mekonnen Manuel, Galley Thomas D, Müller Markus P |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-73064-6) |
...
在粒子交换下不变的量子系统要么是玻色子，要么是费米子，尽管量子理论原则上允许更一般的置换行为。但为什么我们在自然界中没有观察到这类超粒子呢？此前对这一问题的分析主要局限于特定的量子场论模型。在此，我们给出两个独立于模型的论证，排除了超统计（即基本不可区分的量子系统在对称群的高维表示下变换），这些论证借鉴了量子信息理论和关于内部量子参考系的最新研究。首先，我们引入了完全不变性的概念：量子系统不仅在置换下保持其局域态，还应保持它们所携带的关于其他系统的量子信息，这类似于量子信息理论中的完全正定性概念。其次，我们要求量子系统在量子置换下保持不变，量子置换是以置换不变可观测量的值为条件的置换。对于这两者，我们表明，相应的原理被满足当且仅当粒子是玻色子或费米子。我们的结果展示了量子参考系如何能够揭示量子物理学中一个长期存在的问题，强调了量子信息的组合结构所起的关键作用，并展示了最近提出的量子协变性原理的解释力以及其微妙的局限性。


### 核心发现


- 引入完全不变性：粒子交换时局域态和关联量子信息均保持不变，仅玻色/费米子满足。

- 提出量子置换不变性：以置换不变观测量为条件进行置换，同样只允许玻色子与费米子。

- 利用量子参考系与信息组合结构，给出模型无关论证，排除超统计，阐明长期难题。
...
## 8. Accurate computation of ionic concentrations in the synaptic cleft requires the full Poisson-Nernst-Planck (PNP) equations.

| 字段 | 内容 |
|---|---|
| **期刊** | PLoS Comput Biol |
| **发表日期** | 2026-May-28 |
| **作者** | Jæger Karoline Horgmo, Tveito Aslak |
| **原文链接** | [查看原文](https://doi.org/10.1371/journal.pcbi.1014341) |
...
突触间隙是神经递质介导通信的场所，支撑正常脑功能（包括学习和记忆）。动作电位抵达突触前末梢时，释放的神经递质在扩散和电场力共同作用下穿过间隙激活突触后受体。然而，突触间隙传输通常用纯扩散模型模拟，忽略电迁移。本文我们在泊松-能斯特-普朗克（PNP）框架下量化扩散项和电项的相对贡献，评估纯扩散近似是否充分。我们在纳米级分辨率的三维计算模型中求解完整PNP系统，以全时空细节追踪五种离子（Na⁺、K⁺、Ca²⁺、Cl⁻、Glu⁻），并与纯扩散（D）模型直接比较。D和PNP模型产生显著不同的离子浓度场。离子通量分析证实，所有物种的扩散与电贡献大小相当。这些差异在参数变化下稳健，包括AMPA受体数量、谷氨酸释放量、间隙高度和间隙扩散系数，且随AMPA受体或谷氨酸离子增多、间隙变窄或扩散受限加剧而放大。但由于竞争效应，相关的AMPA电流差异不大。纯D模型与完整PNP模型的定量定性差异表明，忽略突触间隙中的电场力会产生后果。这些差异足以改变突触传递的预测动力学和生物学解释，确立了突触间隙离子浓度的准确计算需要完整的PNP方程。


### 核心发现


- 纯扩散模型与完整PNP模型预测的离子浓度场存在显著定量和定性差异，且差异在参数变化下稳健。

- 所有离子物种的扩散和电迁移通量具有可比量级，证明电效应不可忽略。

- 差异程度随AMPA受体或谷氨酸释放量增加、间隙变窄、扩散受限加剧而放大，但综合导致AMPA电流变化温和。
...
## 9. UNified FramewOrk for reguLatory Dynamics (UNFOLD): Dissecting robustness, plasticity, evolvability and canalisation of biological function.

| 字段 | 内容 |
|---|---|
| **期刊** | PLoS Comput Biol |
| **发表日期** | 2026-May-28 |
| **作者** | Chakraborty Debomita, Rengaswamy Raghunathan, Raman Karthik |
| **原文链接** | [查看原文](https://doi.org/10.1371/journal.pcbi.1014289) |
...
生物系统以看似矛盾的特性的独特平衡为特征，如稳健性与可塑性，或可演化性与功能渠化。为理解这些特性的基础，我们研究了处于生物功能核心的基因调控。我们模拟了超过1.9亿个涵盖所有可能三基因电路结构的遗传电路动力学模型。我们的计算流程通过匹配时间响应形状将这些电路分类到功能簇中，从而生成了一个连接电路结构、参数与对应功能标签的数据集。我们的核心发现是，在阶跃输入下并在所探索的参数空间内，三节点遗传电路仅能执行有限的20种功能。此外，这些电路的结构-参数空间倾向于产生在受到扰动后随时间趋于稳定的响应。每种结构都表现出多功能性的潜力，取决于参数，功能范围从2种到17种不等。我们量化了网络简并性，表明在不改变功能的情况下可以对电路进行许多结构改变。我们定义了三个量：结构多样性、参数多样性和功能多样性。利用这些多样性，我们构建了一个调控动力学统一框架（UNIFied FramewOrk for reguLatory Dynamics, UNFOLD），用以分析四种关键的生物学特性——稳健性、可塑性、可演化性和功能渠化。应用UNFOLD，并在所探索的参数空间内，我们发现只有6.5%的网络结构是不可塑的，而每个三节点网络都存在实现参数稳健性的参数集。我们从可在大量参数集下互换而不改变功能的结构对中识别出了功能渠化电路。总体而言，我们的框架通过对三节点网络的深入分析，为生物网络的基本组织提供了见解。


### 核心发现


- 三节点基因电路在阶跃输入下仅能实现20种不同的时间响应功能。

- 每种电路结构通过调整参数可呈现2至17种功能，展现高度多能性。

- 仅6.5%的网络结构完全不可塑，所有三节点网络均存在参数稳健的参数集。
...
## 10. Elucidating the Bell-Shaped Dependence of Protein Translation Activity on EF-Tu Concentration in a Reconstituted Cell-Free System Using a Mechanistic Model.
...
无细胞蛋白质合成系统常表现出非直观的输入-输出关系。在重组的PURE系统中，蛋白质产量在低延长因子Tu（EF-Tu）浓度时达到峰值，随后随浓度升高而下降，呈现特征性钟形曲线。我们利用翻译的详细机理模型ePURE，描述了数百种分子物种和反应的动力学，探究了这一行为的根源。计算分析表明，过量的EF-Tu将起始tRNA（tRNAfMet）隔离到非生产性的EF-Tu·GTP·Met-tRNAfMet复合物中，从而耗竭了可用于翻译起始的起始tRNA库。这种抑制源于对有限分子资源的竞争，而非直接抑制。基于该机制，我们预测增加tRNAfMet和甲硫氨酰-tRNA甲酰转移酶的浓度将消除钟形依赖性，并通过实验证实了该预测。在修改后的条件下，钟形响应消失，蛋白质产量提高。这些发现表明，机理计算模型能够揭示复杂生化网络中隐藏的约束，指导无细胞蛋白质合成系统的理性优化。


### 核心发现


- 过量EF-Tu将起始tRNA隔离为非生产性复合物，抑制翻译起始。

- 该抑制源于对有限分子资源的竞争，而非直接抑制作用。

- 提高tRNAfMet和甲酰转移酶浓度可消除钟形依赖性并增强蛋白质产量。
...
## 11. Characterization of a Two-Sided Orthogonal Translation System and Its Application to Violacein Biosynthesis.
...
在微生物生产高价值化合物中，一个主要挑战是细胞生长与生物合成之间的权衡。正交翻译系统利用源自工程化rRNA的正交核糖体来翻译特定mRNA，为将翻译资源从细胞生长重新分配到生物合成提供了手段。然而，正交翻译系统的体内正交性和定量有效性尚未得到充分评估。在此，我们以超折叠绿色荧光蛋白（sfGFP）为报告蛋白，在大肠杆菌中系统性地测试了正交rRNA-mRNA对，并鉴定出一对表现出稳健双面正交性和高翻译效率的配对。随后，我们将此正交翻译系统应用于紫罗兰素生物合成途径。与内源翻译系统相比，正交翻译系统使紫罗兰素产量提高了6.30至7.77倍，并在进入稳定期后仍持续生产。这些结果表明，一个经过充分鉴定的双面正交翻译系统通过重新分配体内翻译能力，成功提高了多酶途径的生产力。


### 核心发现


- 鉴定出在大肠杆菌中具备双面正交性和高翻译效率的rRNA-mRNA对。

- 正交翻译系统使紫罗兰素产量提高6.30-7.77倍，并在稳定期维持生产。

- 该双面正交系统通过重新分配翻译资源有效提升多酶途径生产力。
...
二聚化对ErbB家族受体激活至关重要，但致癌突变对其实时动态的影响尚不明确。本研究利用无光漂白的上转换纳米颗粒（UCNPs），对活细胞中EGFR、HER2和HER3进行长期多色单粒子追踪，实现受体相互作用的连续观测，揭示其二聚化动态细节。致癌EGFR突变促进配体非依赖的稳定二聚化。出乎意料的是，HER2和HER3均表现出组成性同源二聚化，这促使修正其激活机制模型。与EGFR突变相比，HER2突变仅适度增强同源二聚体稳定性，而HER3突变则破坏同源二聚体稳定性，提示HER3同源二聚化可能隔离HER3并限制其与其他受体的异源二聚化。我们还发现三种受体间存在稳定的配体非依赖异源二聚体，且配体刺激进一步增强其稳定性。这些发现提供了全面的ErbB相互作用网络，阐明了多样化的二聚化机制及其对致癌信号传导的影响。


### 核心发现


- EGFR致癌突变驱动配体非依赖的稳定同源二聚化，揭示其组成性激活机制。

- HER2和HER3均存在组成性同源二聚化，且不同突变对二聚体稳定性产生差异调控。

- 解析了ErbB受体间配体非依赖的稳定异源二聚体网络，为致癌信号调控提供新见解。



**关键词**：Humans · Single Molecule Imaging · Protein Multimerization · ErbB Receptors · Receptor, ErbB-3 · Erb-b2 Receptor Tyrosine Kinases · Mutation · Signal Transduction · Nanoparticles · Animals


---


## 13. Electromagnetic field-inducible in vivo gene switch for remote spatiotemporal control of gene expression.
...
在生物医学应用中，精确控制基因表达至关重要，但时空精度仍具挑战。本文提出一种响应电磁场（EMF）的远程控制体内基因开关，实现靶基因的精确时空激活。我们通过CRISPR-Cas9筛选揭示了EMF诱导基因开关的激活机制，发现细胞色素b5 B型（Cyb5b）作为关键介质，可能充当EMF传感器。该开关由节律性振荡钙动力学而非普通钙内流激活，界定了一种精确调谐且生物正交的诱导机制。功能上，EMF激活Oct4-Sox2-Klf4（OSK）盒可在老年小鼠中诱导体内部分重编程，人突变淀粉样前体蛋白（APP）条件表达用于阿尔茨海默病（AD）建模重现病理特征，EMF介导的Tph2表达在Tph2突变抑郁小鼠中恢复5-羟色胺能活性并改善抑郁样行为。总之，远程控制的EMF诱导基因开关是一个多功能且有效的生物医学平台。


### 核心发现


- CRISPR-Cas9筛选鉴定出Cyb5b作为电磁场传感器，是基因开关激活的关键介导因子。

- 基因开关由节律性振荡钙动力学精确激活，而非普通钙内流，实现生物正交诱导。

- 该开关成功用于衰老小鼠部分重编程、阿尔茨海默病病理建模和抑郁症行为改善。



**关键词**：Animals · Kruppel-Like Factor 4 · Humans · Mice · Electromagnetic Fields · Alzheimer Disease · Amyloid beta-Protein Precursor · Gene Expression Regulation · Genes, Switch · Tryptophan Hydroxylase


---


## 14. αKG-mediated carnitine synthesis drives DNA repair via histone acetylation.

| 字段 | 内容 |
|---|---|
| **期刊** | Nature |
| **发表日期** | 2026-May-27 |
| **作者** | Uboveja Apoorva, Yang Baixue, Buj Raquel, Amalric Amandine, Wang Hui, Tangudu Naveen Kumar, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41586-026-10584-7) |
...
同源重组（HR）缺陷会增加对常用于癌症治疗的DNA损伤剂的敏感性。在HR功能完整的癌症中，驱动对DNA损伤剂应答或耐药的代谢机制尚不清楚。本研究发现，α-酮戊二酸（αKG）的耗竭通过组蛋白乙酰化的代谢调控，使HR完整细胞对DNA损伤剂敏感。αKG是αKG依赖性双加氧酶（αKGDDs）活性所必需，以往研究几乎只关注αKGDD的去甲基化功能。利用包含64种αKGDD的靶向CRISPR敲除文库，我们发现三甲基赖氨酸羟化酶epsilon（TMLHE）——肉碱从头合成的第一个也是限速酶——是HR完整细胞在DNA损伤剂存在下存活所必需的。出乎意料的是，αKG介导的TMLHE依赖性肉碱合成是组蛋白乙酰化所必需，且与其他核胞质乙酰辅酶A生成途径不可相互替代。通过αKG-肉碱轴增强组蛋白乙酰化，借由位点特异性组蛋白乙酰化促进HR介导的DNA修复。最后，在患者样本中我们观察到TMLHE与组蛋白乙酰化水平呈正相关，并发现TMLHE或乙酰肉碱高表达与接受DNA损伤剂治疗患者较差的无进展生存期相关。据我们所知，本研究首次证明αKG影响位点特异性组蛋白乙酰化，并通过肉碱合成提供了HR完整性的机制。此外，这些数据为诱导HR缺陷、增强对DNA损伤剂的敏感性提供了一条代谢途径。


### 核心发现


- α-酮戊二酸通过TMLHE催化的肉碱合成调控组蛋白位点特异性乙酰化，促进同源重组修复。

- TMLHE依赖性肉碱合成是组蛋白乙酰化的关键代谢源，独立于其他乙酰辅酶A途径。

- 高TMLHE或乙酰肉碱水平与DNA损伤剂治疗患者不良预后相关，揭示代谢致敏新策略。
...
## 15. High-frequency spike inference with particle Gibbs sampling.

| 字段 | 内容 |
|---|---|
| **期刊** | Elife |
| **发表日期** | 2026-May-27 |
| **作者** | Diana Giovanni, Sermet B Semihcan, Broussard Gerard J, Wang Samuel S-H, DiGregorio David A |
| **原文链接** | [查看原文](https://doi.org/10.7554/eLife.94723) |
| **DOI** | `10.7554/eLife.94723` |
...
钙敏感荧光指示剂能够监测动物模型中大神经元群落的发放活动。尽管过去几十年已开发大量算法，但针对超过20 Hz发放率的精确脉冲时间推断方法仍然缺乏，且发放时间估计的统计不确定性量化未受重视。为此，我们引入一种考虑爆发性神经元活动和基线荧光调制的统计模型，并应用粒子吉布斯与祖先采样蒙特卡洛策略，估计发放时间和模型参数的联合后验分布。在CASCADE基准数据集上，本方法与最先进的有监督和无监督算法相当。分析超快基因编码钙指示剂GCaMP8f记录的荧光瞬变表明，该方法可分辨短至5 ms的发放间隔。本研究描述了一种检测神经元发放模式并量化其不确定性的贝叶斯推断方法，粒子吉布斯采样器能实现发放时间和所有模型参数的无偏估计，为测试更具体的钙指示剂模型提供了灵活的统计框架。


### 核心发现


- 提出融合爆发性活动和基线调制的统计模型，结合粒子吉布斯采样推断发放时间与参数。

- 在CASCADE基准测试中性能媲美最优算法，并能解析短至5毫秒的发放间隔。

- 建立了量化发放时间不确定性的贝叶斯框架，实现所有模型参数的无偏估计。



**关键词**：Animals · Neurons · Action Potentials · Monte Carlo Method · Calcium · Algorithms · Models, Statistical · Bayes Theorem


---


## 16. Self-charged polar nematic monopoles and hybrid topological states: intertwining and domain integration.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-May-26 |
| **作者** | Zou Yu, Yang Jidan, Ma Zhongjie, Hong Zening, Sun Aile, Huang Xiang, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-73557-4) |
...
极性拓扑孤子是凝聚态物理中至关重要的准粒子，其关键挑战在于实现拓扑多样化、操控和排列。本研究表明，约束不对称性可以定制和多样化流体铁电体中的电极性拓扑孤子，为设计大规模复杂畴结构这一长期难题提供了关键途径。我们发现了非经典的电极性单极子和杂化拓扑态，包括手性梅隆和反梅隆变体及其与嵌套斯格明子核心的复合杂化。它们表现出内禀的自电荷，并通过向列弹性和电极性相互作用进行耦合。这种独特的机制通过极性通量的交织导致多体拓扑构型的形成。我们提出利用孤子结构作为基本准粒子，在绝缘流体中设计充当局部导体的大规模荷电阵列。这种自组织为实现传统铁电体无法实现的平面畴结构的大规模工程提供了前所未有的机会。


### 核心发现


- 约束不对称性可定制流体铁电体中的极性拓扑孤子，实现大规模复杂畴结构设计。

- 发现非经典电极性单极子和手性梅隆/反梅隆与嵌套斯格明子核的杂化拓扑态。

- 孤子结构可通过自组织形成大规模荷电阵列，实现绝缘流体中的局部导电功能。
...
## 17. Asymmetric life-history trade-offs shape sex-biased longevity patterns.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-May-26 |
| **作者** | Rees-Baylis Ella, Wang Daiping, Li Richter Xiang-Yi, de Vries Charlotte |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-73633-9) |
...
衰老与寿命的性别差异在生物界中广泛存在，但其进化原因仍有争议。一个主导假说认为这类差异是性别特异性生活史权衡的适应性结果，但缺乏正式的理论支持。因此，我们构建了一个数学模型，探讨这些权衡如何在生态和交配系统背景下塑造寿命的进化。在单配制下，个体优化生存与繁殖的权衡，这种权衡由雄性的交配机会和雌性的后代产量介导。改变权衡强度表明，雄性偏向的寿命在更广泛的参数范围内出现，这与单配制物种的模式一致。这种不对称性之所以产生，是因为雌性寿命延长会减少后代产量，降低种群规模和交配率，从而限制了对雌性寿命的选择，而除非交配代价很高，雄性更长的寿命更容易进化。引入密度制约表明，生态反馈会不成比例地降低原本寿命更长性别的存活率，从而可能掩盖已进化的差异。在单配制之外，我们表明交配系统的变异通过竞争强度（取决于限制性别）和种群统计缩放（其中雌性数量决定繁殖产出和寿命的选择背景）来塑造寿命。总体而言，我们强调生活史权衡与种群统计反馈共同决定了寿命性别差异的方向与程度。


### 核心发现


- 在单配制模型中，雄性偏长寿比雌性偏长寿在更宽泛的参数范围内出现，解释了单配制物种中常见的雄性长寿现象。

- 雌性寿命延长因降低后代产量和种群密度而受到更强的选择约束，而雄性寿命延长不受此限制，导致了长寿进化的不对称性。

- 密度制约的生态反馈会优先降低内在长寿性别的存活率，可能掩盖进化形成的寿命差异；此外，交配系统可通过竞争强度和种群统计缩放重塑性别特异性寿命。
...
## 18. Uncovering spatially resolved functional genomics with CRISPR screen sequencing.
...
空间组学推动了我们对组织层面生物学的理解，然而系统性将基因功能扰动关联到空间表型及信号通路的工具依然有限。为此，我们开发了空间CRISPR筛选测序平台SPAC-seq（空间CRISPR筛选测序）以及统计空间扰动分析工具包TARDIS（空间组学扰动数据靶标优先排序工具包）。利用SPAC-seq与TARDIS，我们将基因扰动与空间表型和通路联系起来，揭示了肿瘤细胞中Icam1缺失如何通过免疫抑制和巨噬细胞极化促进转移。在CD8⁺ T细胞中，我们阐明了Cd44通过与巨噬细胞上Spp1互作来调控空间表型的作用。我们还展示了转录因子-趋化因子受体轴将细胞状态与趋化性耦合的模型。SPAC-seq与TARDIS为在不同生物学和疾病背景下研究空间分辨的功能基因组学及通路提供了有效框架。


### 核心发现


- 开发了空间CRISPR筛选测序平台SPAC-seq及配套分析工具TARDIS，实现高通量空间扰动功能基因组学研究。

- 发现肿瘤细胞中Icam1缺失通过免疫抑制和巨噬细胞极化反向促进转移进程。

- 揭示CD8⁺ T细胞中Cd44通过与巨噬细胞表面Spp1相互作用调控自身空间表型的分子机制。
...
## 19. Tunable Transcription-Level CRISPR Interference in Vibrio natriegens Using Engineered Single Guide RNAs.
...
需钠弧菌（Vibrio natriegens）是一种生长迅速的细菌，已成为微生物学和生物工程领域有前景的下一代微生物平台。虽然遗传元件和基因组工程方法的工具箱不断扩大，但精确和可预测控制基因表达的策略仍然有限。在这里，我们报道了一个基于dCas9的可调CRISPR干扰（CRISPRi）系统，可在需钠弧菌中实现多层次的转录调控。通过工程化单链引导RNA（sgRNA）的四环和侧翼区域，我们构建了一个合成的sgRNA文库，调节sgRNA与dCas9之间的结合亲和力。所得sgRNA变体在多个原间隔子靶点上表现出模块化的抑制行为。我们进一步通过重定向细胞内碳通量，展示了这种可调CRISPRi系统在代谢工程应用中的实用性。对内源基因的可调抑制使3-羟基丙酸（3-HP）产量提高了2.2倍，番茄红素产量提高了1.5倍。总的来说，这项工作为需钠弧菌中的可调基因调控提供了一种简单有效的策略，并扩展了其作为生产高附加值化学品的多功能平台的潜力。


### 核心发现


- 通过工程化sgRNA的四环和侧翼区域构建合成文库，调节与dCas9结合亲和力，实现可调转录调控。

- sgRNA变体对多个原间隔子靶点展示模块化抑制行为，实现多水平基因表达调控。

- 可调抑制内源基因重定向碳流，使3-羟基丙酸和番茄红素产量分别提高2.2倍和1.5倍。
...
## 20. The role of pragmatic mechanisms in referential communication and categorization: An emergent communication model.

| 字段 | 内容 |
|---|---|
| **期刊** | PLoS Comput Biol |
| **发表日期** | 2026-May-26 |
| **作者** | Kobrock Kristina, Ohmer Xenia, Bruni Elia, Gotzner Nicole |
| **原文链接** | [查看原文](https://doi.org/10.1371/journal.pcbi.1014326) |
...
我们在突发通信的多智能体框架中建模指称通信和范畴化的语用机制。语用理论和实验预测说话者根据语境选择指称表达，而基于效用的语用推理（考虑听话者的解读）也影响产出。我们研究这两个因素在语言涌现和语言使用中的角色，并考虑说话者与听话者效用的效率权衡。结果表明，共享语境下涌现的语言更有效、模糊但更高效：语境中足够的信息量允许生产更简单，将负担转移给听话者。此外，基于效用的语用学（理性言语行为框架）仅提升在语境中涌现的语言的使用效率，对无语境训练涌现的语言无效。这表明基于效用的语用机制依赖于语言演化中可用的语境。结论是，类人语言和范畴系统作为交互中简约性与信息性效率权衡的优化结果而涌现。


### 核心发现


- 共享语境使涌现语言通过简单化生产和转移理解负担实现更有效、模糊和高效的权衡。

- 基于效用的语用学仅提升在语境中涌现的语言的效率，对无语境涌现语言无效。

- 语境与效用语用的结合依赖语言演化中自然涌现的机制，优化指称通信与范畴化。
...
## 21. The interplay between ecological networks drives host-plasmid community dynamics.

| 字段 | 内容 |
|---|---|
| **期刊** | PLoS Comput Biol |
| **发表日期** | 2026-May-26 |
| **作者** | Wang Ying-Jie, Schaal Kaitlin A, Nauta Johannes, Liaghat Armun, De Domenico Manlio, Hall James P J, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1371/journal.pcbi.1014339) |
| **DOI** | `10.1371/journal.pcbi.1014339` |
...
质粒通过在微生物宿主间转移性状驱动进化。转移依赖宿主-质粒（感染）和质粒-质粒（相容性）相互作用，但这些网络结构如何影响转移仍知之甚少。我们假设这两个生态网络以非加性方式相互作用，影响群落结果。为验证这一假设，我们开发了一个嵌入两种网络结构的随机基于代理模型，模拟耦合的宿主-质粒动态。我们系统地改变每个网络的结构，单独和组合地，以分离结构对宿主-质粒动态的影响。模块化（相互作用组织成簇）和枢纽（相互作用集中于高度连接的节点）的质粒-质粒相容性网络促进了宿主短暂共存，而模块化的宿主-质粒感染网络促进了质粒多样性和稳定宿主共存。重要的是，结构化网络以非加性方式相互作用，当质粒携带对宿主施加中等适合度代价时影响最明显。例如，将模块化感染网络与枢纽相容性网络结合，逆转了预期的质粒流行度模式，证明了一种网络的结构可以抵消另一种网络的影响。我们进一步重新参数化模型以重现经验的宿主-质粒群落动态，显示即使存在显著的生物学异质性，感染网络结构也能强烈塑造质粒流行度。我们的结果强调，必须联合考虑宿主-质粒感染和质粒-质粒相容性网络，以理解宿主-质粒群落动态及其生态进化潜力。更广泛地，这项工作提供了产生可验证假设的初始机制框架，并强调涉及多个宿主和传染原的系统需要明确考虑不同生态网络如何相互作用塑造群落动态。


### 核心发现


- 模块化与枢纽型质粒相容性网络促进宿主短暂共存，模块化感染网络促进质粒多样性与稳定共存。

- 结构化的感染与相容性网络非加性交互，在质粒中等适合度代价时对群落动态影响最大。

- 模块感染与枢纽相容网络结合可逆转质粒流行度模式，展示网络结构间抵消效应。
...
## 22. Multiplexed perturbation enables scalable pooled screens.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Methods |
| **发表日期** | 2026-May-25 |
| **作者** | Oberlin Stefan, Tay Neil Q, Xue Albert, Mosadeghi Ruzbeh, Pimentel Harold, McManus Michael T |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41592-026-03095-w) |
...
基于CRISPR的基因扰动筛选已凭借前所未有的精度和规模，彻底改变了将基因与细胞表型联系起来的能力；然而，传统的混池CRISPR筛选需要大量细胞才能保证足够的单导向RNA（sgRNA）代表性，这带来了技术和成本上的挑战。本文我们研究了在混池CRISPR干扰筛选中，通过高感染复数（MOI）共递送多个导向RNA的影响，以期在提高筛选效率的同时减少细胞用量。我们系统评估了不同MOI下的筛选性能，考察了多重化对敲低效率、sgRNA代表性以及多个sgRNA表型潜在干扰的影响。我们的数据表明，sgRNA多重化（MOI 2.5~10）能够维持筛选性能，同时显著降低对细胞数量的需求。我们进一步应用这些优化条件，对胞内粘附分子ICAM-1的调控因子进行了全基因组CRISPR筛选，成功在仅用50万个细胞的条件下发现了新的候选基因。本研究为在资源有限的环境下采用多重sgRNA策略以精简CRISPR筛选应用提供了框架。


### 核心发现


- sgRNA多重化（MOI 2.5–10）可在维持筛选效能的同时显著减少细胞需求。

- 多重化未显著影响敲低效率、sgRNA代表性，也未引发严重的表型干扰。

- 仅用50万个细胞即完成全基因组CRISPR筛选，并鉴定出ICAM-1的新调控因子。
...
## 23. De novo design of DNA origami with a generative diffusion model.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-May-25 |
| **作者** | Truong-Quoc Chien, Jeon Kyounghwa, Kim Jinho, Kwon Seo Hyun, Seo Dongsik, Lee Chanseok, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-73578-z) |
...
生成模型在蛋白质工程中推动了逆向设计的发展，现可扩展至DNA折纸领域，以探索更广阔的设计空间，从而实现新兴纳米技术所需的复杂几何形状和功能。然而，DNA折纸生成设计的进展受限于缺乏包含结构信息的大型标准化结构数据集。为应对这一挑战，我们引入了一个基于扩散的生成设计框架，该框架利用多尺度计算模型获得的模拟平衡构象进行训练。给定用户定义的目标几何形状，我们的模型通过引导扩散采样和链路由生成物理上合理的DNA折纸设计，并集成结构预测以进行定量评估。在超过100个生成的候选结构中，选定结构经过实验验证，展示了正确的折叠和功能行为，如拉胀变形和模块化组装。我们的结果凸显了生成建模在复杂DNA折纸设计中的潜力，拓展了可访问的设计空间，并有助于创建复杂、可重构的架构。


### 核心发现


- 开发了基于扩散模型的DNA折纸生成框架，利用模拟平衡构象训练突破数据限制。

- 模型通过引导扩散采样和链路由，从目标几何生成可行设计并集成结构预测。

- 实验验证生成结构能正确折叠，并实现拉胀变形和模块化组装等复杂功能。
...
## 24. Closed-Loop Optogenetic Control in a Microplate Reader.
...
光遗传学将活细胞与电子设备整合成强大的细胞-硅系统，但对其动力学进行原型设计仍具挑战。现有工具要么需要借助机器人液体转移至流式细胞仪，要么依赖动态范围狭窄的定制传感器，限制了控制器性能。此外，当前成功的光遗传反馈控制器仅运行于强制恒定生长的恒化器或微流控装置中，因为缺乏针对分批培养的生长感知型控制器设计模型。本文提出LEMOS，一种在商用微孔板读数仪中运行的低成本LED嵌入式微孔板。结合用于控制器调谐的生长感知多尺度基因表达模型，该平台为细胞-硅系统实现快速设计-构建-测试-学习循环。我们展示了在标准微孔板读数仪中对分批培养基因表达进行闭环设定点跟踪，并揭示了生长动态如何使控制器选择与调谐复杂化。该平台减少了设置开销，加速了迭代，实现了精确的实时光遗传反馈控制。


### 核心发现


- 开发LEMOS平台，在商用酶标仪中实现低成本光遗传闭环控制，降低原型设计门槛。

- 提出生长感知多尺度模型，指导分批培养中的控制器设计，提升基因表达跟踪精度。

- 揭示生长动态对控制器选择与调谐的复杂影响，实现精确的实时设定点跟踪。
