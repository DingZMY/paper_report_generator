# 系统/合成生物学文献周报 2026-W21

> **时间范围**：2026/05/11 ~ 2026/05/18
> **来源期刊**：Nature / Cell / Science 系列
> **收录论文**：46 篇（已翻译 46 篇）
> **生成时间**：2026-05-18 03:05 UTC

---



## 1. One Chromatin, Many Structures: From Ensemble Contact Maps to Single-Cell 3D Organization.

| 字段 | 内容 |
|---|---|
| **期刊** | Biophys J |
| **发表日期** | 2026-May-15 |
| **作者** | Carignano Marcelo A, Kroger Martin, Almassalha Luay M, Backman Vadim, Szleifer Igal |
...
理解染色质如何在三维空间中折叠仍然具有挑战性，因为大多数实验检测方法捕捉到的是底层高度异质聚合物的低维投影。本文基于先前提出的自返回排斥体积（SR-EV）模型构建了一个基于系综的解释框架，该模型利用基于随机返回规则和排斥体积几何的核小体索引粗粒化表示，生成最简化的染色质构象。尽管其形式简单，SR-EV 仍重现了跨尺度的关键实验特征：与 ChromEMT 和 ChromSTEM 观察结果相似的异质性纳米级包装域、类似于单细胞染色体构象捕获（Hi-C）的稀疏且高度可变的单构象接触模式，以及与拓扑关联域（TADs）一致的稳健系综水平接触富集。在该框架中，Hi-C 环和 TAD 特征被解释为系综水平的统计富集，而非单细胞构象的不变特征。SR-EV 专为生成大规模完整三维染色质构象系综而设计，并可一致地投影到二维接触图和一维基因组轮廓上。通过仅以系综选择而非显式力引入结构蛋白效应，SR-EV 支持内源性聚合物几何与调控偏倚之间的分离，并提示 TAD 样特征可作为统计富集而非决定性的三维结构涌现。直接从 SR-EV 计算的配位数和探针可及性提供了三维包装、二维接触图和一维基因组轮廓之间的统一联系。这项工作的主要贡献在于，在单一粗粒化框架内展示这些多模态观测值如何通过平均和条件采样，作为同一异质染色质系综的相互关联投影而出现。这些结果共同确立了 SR-EV 作为一个基于几何的简约介观尺度参考框架，用以解释异质染色质系综如何产生多模态实验观测值，同时与染色质组织在单个细胞中实现的事实保持一致。


### 核心发现


- SR-EV 模型重现了跨尺度实验特征，包括异质性纳米包装域和单细胞 Hi-C 可变接触模式。

- Hi-C 环和 TAD 特征被重新解释为系综水平统计富集，而非单细胞中的固定结构。

- 通过仅使用系综选择引入结构蛋白效应，模型揭示了聚合物几何与调控偏倚的可分离性。
...
## 2. Cell-autonomous control of CAR signaling and receptor shedding via ADAM17-mediated proteolysis.

| 字段 | 内容 |
|---|---|
| **期刊** | Cell |
| **发表日期** | 2026-May-15 |
| **作者** | Bjelajac Jeremy R, Cañellas-Socias Adrià, Nehra Preeti, Reynolds Kevin, Malipatlolla Meena, Martinez Velez Naiara, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.cell.2026.04.037) |
...
我们试图通过利用T细胞活化后ADAM17的条件性蛋白水解活性，赋予T细胞对细胞表面蛋白表达的自主调控。对经典ADAM17底物的筛选获得了一个源自CD62L的15个氨基酸的最小基序，该基序能在T细胞活化后使受体快速且可逆地脱落——我们称之为活化诱导释放（AIR）。将AIR嵌入到产生tonic信号的CAR中，可降低基础CAR表达，降低程度与诱导的tonic信号强度成正比，从而减少耗竭并提高抗肿瘤效力。在非tonic信号CAR中，AIR降低了活化诱导的细胞死亡，并增强了刺激后的T细胞扩增。AIR的模块化特性支持更高阶的逻辑门控；AIR调控的肽掩蔽能够实现抗原依赖性的EGFR靶向CAR的去掩蔽。最后，通过CRISPR将AIR敲入内源性FAS或TGFBR2中，赋予它们活化诱导的脱落能力，在增强肿瘤清除的同时保留了非活化条件下的信号传导。AIR是一个紧凑的开关，为下一代细胞疗法提供了快速、自主的表面蛋白调控。


### 核心发现


- 发现源自CD62L的15aa AIR基序，可介导T细胞活化后受体的快速可逆脱落。

- 在tonic信号CAR中嵌入AIR，可降低基础表达，减少耗竭，提升抗肿瘤效力。

- 非tonic信号CAR中AIR减少活化诱导的细胞死亡，促进T细胞扩增。

- AIR模块化支持抗原依赖性肽掩蔽，实现逻辑门控CAR调控。

- CRISPR敲入内源FAS/TGFBR2的AIR化，实现活化诱导脱落，增强肿瘤清除且保留非活化信号。
...
## 3. DNA-guided CRISPR-Cas12 for cellular RNA targeting.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Biotechnol |
| **发表日期** | 2026-May-15 |
| **作者** | Orosco Carlos, Huang Boyu, Rananaware Santosh R, Bodin August P, Browning Ivy, Fang Anne, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41587-026-03129-w) |
...
我们在此提出ΨDNA，一种基于DNA的向导，能使Cas12核酸酶靶向RNA，克服了传统上对RNA向导系统的依赖。我们设计ΨDNA以反向方向模拟CRISPR RNA (crRNA)支架，使得AsCas12a和Cas12i1能识别RNA并触发强烈的单链DNA反式切割，用于灵敏检测多种RNA，包括在临床样本中实现丙型肝炎病毒RNA的100%准确检测。ΨDNA还通过在多个人类细胞系中诱导核糖体停滞，实现了对内源性细胞内RNA转录本70-95%的多重敲低。机制研究揭示，其活性依赖于一个茎环结构，该结构稳定具有催化活性的Cas12-ΨDNA-RNA复合物。最后，crRNA与ΨDNA的共递送可利用单个效应器同时实现DNA编辑和RNA敲低，而将不同酶模块化融合到AsCas12a，则将ΨDNA体系扩展至RNase H介导的RNA降解和基于METTL3的表观转录组编辑。总之，ΨDNA向导构成了一套适应性强的工具包，将Cas12系统扩展到基因组编辑和诊断之外，实现了对细胞转录组及其表观转录组标记的精确、可编程控制。


### 核心发现


- 开发ΨDNA DNA向导，使Cas12核酸酶可靶向RNA，打破对RNA向导的依赖。

- ΨDNA实现高灵敏RNA检测（临床HCV全准确）和细胞内70-95%多重RNA敲低。

- crRNA与ΨDNA共递送协同基因编辑与RNA敲低，并融合酶扩展至表观转录组编辑。
...
## 4. A synthetic cell microreactor with two types of interacting dynamic DNA-based pores.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Chem |
| **发表日期** | 2026-May-15 |
| **作者** | Fan Sisi, Ding Longjiang, Renz Benjamin, Liu Allen P, Speck Thomas, Yan Hao, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41557-026-02124-7) |
...
重建细胞复杂性是合成生物学的核心挑战，对理解生命和推进仿生纳米技术具有深远意义。关键一步是复现膜组分与其功能之间的动态相互作用。本文展示了一种双颈合成细胞微反应器（DCM），在巨型单层囊泡膜中整合了两个基于DNA的动态孔道。DCM的形成利用由巨型单层囊泡膜动力学介导的信号通路，协调光响应小孔与自组装可密封大孔之间的相互作用。该系统能够以高时空精度实现分子反应物的按需顺序递送。利用DCM，我们展示了限域生化反应，包括葡萄糖氧化酶-肌红蛋白级联反应、模拟细胞骨架的肌动蛋白聚合与成束、无细胞Spinach RNA转录，以及超越天然体系的三维DNA晶体合成。通过将协调的多步信号传导与膜通透性的动态控制相结合，DCM构建了一个通用平台，用于模拟并扩展天然细胞系统的功能复杂性。


### 核心发现


- 构建了含光响应小孔和可密封大孔的双颈合成细胞微反应器（DCM），实现膜孔道协同调控。

- 利用膜动力学信号通路协调两类DNA纳米孔，实现分子反应物的高时空精度顺序递送。

- 在DCM内展示了酶级联、肌动蛋白自组装、无细胞RNA转录和三维DNA晶体合成等复杂生化反应。
...
## 5. Photoinduced correlations in stochastic dynamics of a solid-state ionic conductor.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-May-15 |
| **作者** | McClellan Jackson, Zong Alfred, Pham Kim H, Liu Hanzhe, Iton Zachery W B, Guzelturk Burak, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-72663-7) |
| **DOI** | `10.1038/s41467-026-72663-7` |
...
超短激光脉冲的光激发在控制反应路径、产生非平衡材料性质以及探测复杂分子动力学中起着关键作用。由于时空涨落或动力学路径的随机性，每次曝光后光响应通常不同。然而，大多数超快泵浦-探测实验难以区分固有样品涨落与外在设备噪声，常遗漏对平均响应的偏离。利用同步加速器时间分辨X射线微衍射的高稳定性和高通量光子，我们表征了固态离子导体中的随机光致动力学。通过分析单个晶粒晶格参数的时间演化，我们发现脉冲间涨落并非独立，相邻脉冲的非平衡晶格轨迹存在关联，其特征相关长度约为1500次脉冲，对应0.4 ± 0.1 eV的能量势垒，该值接近锂离子扩散的活化能。


### 核心发现


- 在固态离子导体中，光致非平衡晶格轨迹的脉冲间涨落存在关联，并非随机独立。

- 相邻脉冲轨迹的相关长度约为1500次脉冲，对应能量势垒0.4±0.1 eV。

- 该能量势垒与锂离子扩散活化能接近，暗示离子扩散可能介导了轨迹相关性。
...
## 6. Bridging sequence-structure motifs and genetic variants for genome-wide dynamic RNA-protein interaction profiling.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-May-15 |
| **作者** | Wang Yubo, Zhu Haoran, Hao Gaoyang, Su Yanchi, Yu Zhuohan, Yang Yuning, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-73086-0) |
...
RNA结合蛋白（RBP）常识别复杂的RNA二级结构基序，且这些相互作用在不同细胞系间存在差异。然而，现有计算模型无法解释RNA序列-结构基序在多细胞系中的动态行为，也无法将这些动态与遗传变异的功能影响关联起来。本研究提出BRIDGE，一个端到端的统一模型，将序列-结构基序与非编码遗传变异的功能效应联系起来，从而实现全基因组动态RNA-蛋白相互作用谱的分析。评估表明，在单细胞系静态预测中，BRIDGE优于现有最先进方法。无需任何重新训练，该框架即可迁移到未见过的细胞环境，揭示出保守但可适应的RNA识别语法。通过注意力引导的解释，我们识别出3,571个整合基序，包括与剪接调控相关的基序。此外，全基因组计算机模拟扰动为各类变异（包括剪接区和致病等位基因）中RBP结合的中断提供了可解释的视角。


### 核心发现


- BRIDGE模型统一序列-结构基序与遗传变异功能影响，实现全基因组动态RBP互作谱分析。

- 模型在单细胞系静态预测性能最优，并能跨细胞环境迁移，揭示RNA识别的保守可适应语法。

- 通过注意力解释发现3571个整合基序（含剪接调控基序），并模拟扰动解析变异对RBP结合的影响。
...
## 7. PromoterAtlas: decoding regulatory sequences across Gammaproteobacteria using a transformer model.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-May-15 |
| **作者** | Coppens Lucas, Ledesma-Amaro Rodrigo |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-72837-3) |
...
深度学习的最新进展，尤其是Transformer架构，改进了生物序列分析的计算方法。尽管如此，细菌启动子预测的计算模型仍受限于数据集规模小、物种特异性训练以及二元分类方法，而非全面的注释框架。我们推出PromoterAtlas，一个拥有180万参数的Transformer模型，基于3371种γ-变形菌纲物种的900万条调控序列训练而成。该模型能够识别不同物种间的多种调控元件，包括核糖体结合位点、各类细菌启动子、转录因子结合位点和终止子。利用该模型，我们开发了一款针对γ-变形菌纲的全基因组启动子注释工具，并通过多层次验证支持了对不同σ因子相关启动子的预测。此外，我们发现模型嵌入反映了跨物种的进化关系，将启动子按σ因子类型聚类，而非按物种特异性序列特征。最后，我们证明模型嵌入编码了调控序列信息，能够有效预测转录和翻译水平。PromoterAtlas有助于我们理解并改造细菌调控序列，在细菌生物学、合成生物学和比较基因组学中具有潜在应用价值。


### 核心发现


- 开发了基于Transformer的PromoterAtlas模型，在3371种γ-变形菌纲物种的900万调控序列上训练，可识别多种调控元件。

- 模型嵌入按σ因子类型而非物种聚类，揭示了跨物种的进化关系，并编码了转录和翻译调控信息。

- 基于该模型构建了全基因组启动子注释工具，经多层次验证，可预测不同σ因子相关启动子。
...
## 8. CRISPR/Cas9 Recombineering with Bacteriophage Recombinases Enables Efficient and Scarless Genome Editing in Cupriavidus necator H16.
...
Cupriavidus necator H16 是一种有前景的一碳（C1）生物制造底盘，因其能利用氢气衍生的还原力同化 CO2，但该菌的基因组工程仍受限于低效的 DNA 递送和同源重组效率。在此，我们建立了一个模块化双质粒 CRISPR/Cas9 重组工程系统，将 Cas9 表达与 sgRNA 转换解耦，引入自剪接内含子核糖开关以严格控制 Cas9 反选，并导入一对噬菌体重组酶 Che9c60 和 Che9c61，显著提升了同源重组效率。使用 Che9c60 和 Che9c61 后，基因敲除效率从无法检测的水平提高到 58.3%，并在某些位点达到 100%。重要的是，引入这两个重组酶实现了无痕染色体敲入，将插入效率从无法检测的水平提升至近 40%，并支持长达 4.0 kb 的 DNA 载荷靶向整合。为便于快速多轮工程化，我们在 sgRNA 质粒中整合了基于 tdk 的反选标记，在 5-氟-2'-脱氧尿苷（FUDR）筛选下 24 小时内将保留质粒的细胞降至 1.1%。总之，这一精简的工具包实现了 C. necator 中高效、迭代且无痕的基因组编辑，将加速面向 C1 代谢工程和工业应用的 C. necator 菌株开发。


### 核心发现


- 构建模块化 CRISPR/Cas9 系统，通过噬菌体重组酶 Che9c60/61 将基因敲除效率从不可检测提升至 58.3%，部分位点达 100%。

- 实现无痕染色体敲入，插入效率从不可检测提升至近 40%，并支持 4.0 kb DNA 片段整合。

- 引入 tdk 反选标记使质粒清除率在 24 小时内达 98.9%，实现多轮迭代编辑。
...
## 9. Premature transcription termination modulates stochastic gene expression in bacteria.

| 字段 | 内容 |
|---|---|
| **期刊** | Sci Adv |
| **发表日期** | 2026-May-15 |
| **作者** | Moradian Shafagh, Ali Nida, Jagadeesan Rahul, Behrends Volker, Sanches Ribeiro Andre, Engl Christoph |
| **原文链接** | [查看原文](https://doi.org/10.1126/sciadv.aed0831) |
| **DOI** | `10.1126/sciadv.aed0831` |
...
细菌通过控制转录起始和通过衰减机制提前终止转录来调控转录。转录调控产生由大小和频率定义的RNA爆发。虽然通过调控起始的爆发已得到充分表征，但衰减如何塑造爆发动态仍知之甚少。我们表明，在大肠杆菌中，色氨酸操纵子（trp）中衰减与受调控的起始协同作用，调节爆发大小和频率，在色氨酸存在与缺失之间产生转录活性的开关式转变。在缺乏受调控的trp起始的枯草芽孢杆菌中，衰减调节爆发大小，随着色氨酸浓度增加逐渐改变转录活性。这些不同的架构可能反映了适应性策略：枯草芽孢杆菌在更宽的色氨酸范围内维持高表达细胞，可能在色氨酸稀缺且分布不均的土壤环境中有利；而大肠杆菌能够快速抑制，适合波动的肠道环境。值得注意的是，trp转录并非严格的细胞自主过程。高表达细胞可以抑制邻近细胞中的转录，揭示了细胞间调控并支持群体水平的合作。


### 核心发现


- 大肠杆菌中衰减与起始调控协同产生trp转录的开关式转变。

- 枯草芽孢杆菌仅通过衰减调节爆发大小，实现转录活性的渐变响应。

- trp转录非细胞自主，高表达细胞抑制邻近细胞转录，促进群体合作。



**关键词**：Gene Expression Regulation, Bacterial · Bacillus subtilis · Escherichia coli · Operon · Tryptophan · Transcription, Genetic · Transcription Termination, Genetic · Stochastic Processes


---


## 10. Fish-diversity-inspired multiple soft millirobot system with morphology-encoded selective control.

| 字段 | 内容 |
|---|---|
| **期刊** | Sci Adv |
| **发表日期** | 2026-May-15 |
| **作者** | Xin Zhengyuan, Zheng Zhiqiang, Hou Yaozhen, Sun Tao, Shi Qing, Fukuda Toshio, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1126/sciadv.aed6170) |
| **DOI** | `10.1126/sciadv.aed6170` |
...
海洋生态系统，特别是珊瑚礁群落，揭示了鱼类物种通过身体-鱼鳍协调与流体动力学适应的进化优化，实现形态多样化促进特化运动。受这些生物力学原理启发，我们开发了一种形态编码的图案化磁性微型机器人（MPMR），其前后轴（AP）长度比和体廓在制造时预定义，可在相同均匀磁场驱动下产生不同的水动力响应。这些具有不同形态的MPMR成功模拟了不同鱼类在流体环境中的波动游泳模式。MPMR的形态分化已被证明会直接影响其运动性能，最优AP比（约1:1）和流线型体廓能够最大化推进效率。此外，不同形态的MPMR对磁驱动表现出不同的频率依赖响应，形成形态特异的速度谱。通过利用这些形态编码的性能差异，我们在均匀磁场下实现了对多个MPMR的高效选择性控制和多靶点递送，无论是在体外还是离体胃肠组织中。这些发现为类似环境中柔性微型机器人的未来设计奠定了基础，并为推进均匀磁场下多个微型机器人的选择性控制方法提供了参考。


### 核心发现


- 形态编码微型机器人通过预定义前后轴比和体形，在均匀磁场下实现差异化的流体动力学响应。

- 最优前后轴比约为1:1且具流线型轮廓时，微型机器人在波动游泳中获得最大推进效率。

- 不同形态的微型机器人呈现频率依赖的形态特异性速度谱，可实现均匀磁场下的选择性多靶点递送。



**关键词**：Animals · Fishes · Swimming · Hydrodynamics · Biomechanical Phenomena · Coral Reefs · Ecosystem · Magnetic Fields


---


## 11. Temporal Programming of Cell-Free Transcription Using Orthogonal Enzyme-Responsive DNA Blockers.
...
本文报道了一种正交的、酶驱动的DNA转录计时器设计，能够在无细胞体外转录中实现精确的时间延迟编程。这些计时器利用阻断链瞬时结合启动子区域以阻止转录起始，通过选择性酶切移除阻断链，使输入DNA链结合并启动转录。通过调节酶或阻断链浓度来调控酶切降解动力学，我们实现了对转录半衰期（t1/2）从0.48 ± 0.02小时到8.4 ± 0.1小时的精细时间控制。利用三种不同的阻断链降解酶（核糖核酸酶H、尿嘧啶-DNA糖基化酶（UDG）和甲酰胺嘧啶DNA糖基化酶（Fpg）），我们还展示了在单一溶液中多个转录模板的正交时间控制。最后，我们展示了通过这些转录计时器对Cas12a酶附带切割活性进行程序化终止控制和下游调控。总之，这些正交的DNA转录计时器为在无细胞合成生物学中编程时间分辨的转录和基因表达建立了一个通用且直接的框架。


### 核心发现


- 设计了酶驱动的DNA计时器，通过调节酶/阻断链浓度精确控制转录半衰期（0.48–8.4小时）。

- 利用三种正交的阻断链降解酶，实现了同一溶液中多个转录模板的独立时间编程。

- 展示了转录计时器对Cas12a附带切割活性的程序化终止控制，拓展了级联调控应用。
...
## 12. Magnetic medical microrobots with memory-capable genetic circuits.

| 字段 | 内容 |
|---|---|
| **期刊** | Sci Adv |
| **发表日期** | 2026-May-15 |
| **作者** | Chen Haotian, Chen Yujun, Wang Yingying, Ning Peng, Li Yingze, Sun Yuantai, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1126/sciadv.aeb2528) |
...
自主微型机器人能够到达人体深处难以触及的区域，用于微创治疗。然而，其微米级尺寸限制了机载内存的集成，使其操作依赖外部控制。本文开发了一种整合了可记忆基因电路的磁性益生菌微型机器人，以执行自主抗肿瘤治疗。通过一次性磁热疗触发，机器人中的生物热传感器感知温度变化并激活记忆模块Bxb1-ssrA-attB-P7-attP，使其进入治疗状态，持续降解纤维蛋白并软化肿瘤微环境。遗传记忆至少保持12天活性。记忆编码的软化与磁控波状运动产生的物理穿透协同作用，实现深层肿瘤渗透。与无记忆机器人相比，该机器人使肿瘤基质刚度降低6.70倍，并将体内抗癌效力从21.86%提升至87.52%。除肿瘤学外，该系统建立了一个可推广的记忆编码医学微型机器人框架。


### 核心发现


- 通过磁热触发温度感知激活记忆模块，使微型机器人持续降解纤维蛋白并软化肿瘤微环境。

- 遗传记忆活性至少维持12天，与磁控波状运动协同实现深层肿瘤渗透。

- 相比于无记忆微型机器人，肿瘤基质刚度降低6.70倍，体内抗癌效力提升至87.52%。



**关键词**：Humans · Animals · Mice · Robotics · Gene Regulatory Networks · Cell Line, Tumor · Probiotics · Hyperthermia, Induced · Tumor Microenvironment · Neoplasms


---


## 13. Quantifying 3D live-cell membrane dynamics using dynamic metal-induced energy transfer spectroscopy (dynaMIET).

| 字段 | 内容 |
|---|---|
| **期刊** | Sci Adv |
| **发表日期** | 2026-May-15 |
| **作者** | Gallea José Ignacio, Karedla Narain, Wang Dongxia, Zhao Bin, Chen Lizhen, Enderlein Jörg, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1126/sciadv.aed9613) |
...
细胞膜的动态行为是信号转导、胞内运输和力传导等基本生物过程的基础，但同时量化活细胞中的横向分子扩散与垂直膜波动仍具挑战。本文提出动态金属诱导能量转移光谱（dynaMIET），将金属诱导能量转移与荧光相关光谱结合，以纳米级轴向灵敏度和微秒级时间分辨率解析三维膜动力学，可在单次采集中并行测量横向扩散与垂直波动。通过模拟和模型膜验证，并在活细胞的质膜、内质网和核膜中展示其稳健性。dynaMIET 通过捕获分子运动与膜波动，为探索膜力学与组织提供了非侵入性工具，为研究癌细胞力学、蛋白-膜相互作用及细胞器动力学等健康与疾病中的膜相关现象开辟了新途径。


### 核心发现


- 开发了dynaMIET技术，实现纳米轴向精度与微秒时间分辨的三维膜动力学测量。

- 该技术可同时量化活细胞膜的横向分子扩散和垂直波动，突破传统方法限制。

- 在质膜、内质网和核膜等多系统中验证，为膜力学和动态研究提供通用工具。



**关键词**：Cell Membrane · Humans · Endoplasmic Reticulum · Metals · Spectrometry, Fluorescence · Energy Transfer · Diffusion · Nuclear Envelope


---


## 14. Systematic Evaluation of Different Ribonucleoprotein Complexes as Posttranscriptional Biosensors in Cell-Free TX-TL Systems.
...
合成生物学已推动开发出精密的无细胞生物传感器，能够实现复杂、灵敏的信号检测，因其易于使用、快速读出、便携性和内置的生物安全性，在临床和环境应用中前景广阔。然而，核糖核蛋白（RNP）复合物作为无细胞转录-翻译（TX-TL）系统中的转录后生物传感器的潜力仍未被充分探索。本研究测试了三种不同的转录后回路（RISC:miRNA、MS2-CNOT7:ms2L 和 L7Ae:k-turn）在不同无细胞 TX-TL 平台（真核、原核和重组体系）中的性能，并评估了它们体外重建的稳健性。我们发现，体外转录（IVT）荧光素酶 RNA 中编码的 miRNA 传感器能有效工作，但难以可靠地定量报告基因的基础表达水平是基于 IVT RNA 的 miRNA 传感器的主要瓶颈。虽然 MS2-CNOT7:ms2L RNP 回路在真核裂解物中能够稳健重建，但我们发现裂解物依赖的限制因素阻碍了其作为体外生物传感器的应用，降低了其在无细胞环境中的可编程潜力。另一方面，在充分表征质粒特异性和方案依赖性对报告基因表达及其受 L7Ae 条件性抑制的影响后，L7Ae:k-turn 成功在所有测试的无细胞平台中重建。最后，我们通过在 PURE 系统中实施蛋白酶响应性 L7Ae 介导的条件抑制，成功引入了额外的控制层，表明 L7Ae:k-turn RBP 回路作为用于体外基于蛋白酶的病毒检测的无细胞生物传感器具有广阔前景。


### 核心发现


- miRNA 传感器有效，但 IVT RNA 报告基因基础表达难以准确定量，限制其应用。

- MS2-CNOT7 回路虽在真核裂解物中重建稳健，但受裂解液限制，无细胞环境可编程性低。

- L7Ae:k-turn 回路在所有无细胞平台成功重建，并通过蛋白酶调控展示出体外病毒检测潜力。



**关键词**：Biosensing Techniques · Cell-Free System · Ribonucleoproteins · MicroRNAs · Protein Biosynthesis · Transcription, Genetic · Synthetic Biology


---


## 15. Membrane Composition Influences Expression Yield of Plant Cytochrome P450s in E. coli Lysate-Based Cell-Free Systems.

| 字段 | 内容 |
|---|---|
| **期刊** | ACS Synth Biol |
| **发表日期** | 2026-May-15 |
| **作者** | Sugianto Widianti, Garenne David, Noireaux Vincent, Peralta-Yahya Pamela, Carothers James M |
| **原文链接** | [查看原文](https://doi.org/10.1021/acssynbio.5c00948) |
| **DOI** | `10.1021/acssynbio.5c00948` |
...
植物细胞色素P450酶是天然产物生物合成的核心，但由于其跨膜特性，在微生物宿主中仍难以表达。基于裂解液的无细胞表达系统允许补充人工膜，以支持跨膜蛋白的表达和转位。我们建立了一个系统测试脂质体膜组成的框架，以提高植物P450的表达产量。调整常见磷脂比例或添加植物半乳糖脂对表达影响甚微。相比之下，含有卵磷脂、固醇缀合磷脂和聚乙二醇化脂质的混合脂质体以浓度依赖的方式提高了表达。一种大肠杆菌机械敏感通道和三种植物P450的表达提高了2倍以上，某些P450甚至提高了14倍。这些发现突显了膜组成是无细胞表达系统中P450表达产量的关键决定因素。尽管未测定P450活性，但这些结果为未来实现功能性植物跨膜酶以生物生产天然产物的工作流程提供了框架。


### 核心发现


- 混合脂质体（含卵磷脂、固醇缀合磷脂和PEG化脂质）使植物P450表达量最高提升14倍。

- 仅调整常规磷脂比例或添加植物半乳糖脂对P450表达无显著影响。

- 膜组成是无细胞系统中P450表达产量的关键决定因素，但酶活性尚未验证。



**关键词**：Escherichia coli · Cytochrome P-450 Enzyme System · Cell-Free System · Liposomes · Plant Proteins · Phospholipids · Cell Membrane


---


## 16. A Versatile tRNA-gRNA Array-Based CRISPR/Cas9 Platform Enabling Multiplex Genome Editing and Large-Fragment Engineering in Acremonium chrysogenum.

| 字段 | 内容 |
|---|---|
| **期刊** | ACS Synth Biol |
| **发表日期** | 2026-May-15 |
| **作者** | Chen Zhen, Hong Wei, Wei Xiaorong, Li Yifan, Feng Tao, Ke Xiang, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1021/acssynbio.6c00052) |
...
头孢菌素C（CPC）衍生抗生素在改善人类健康和延长预期寿命方面发挥了重要作用。产黄头孢霉（Acremonium chrysogenum）至今仍是唯一能够实现CPC工业生产的微生物。然而，缺乏高效的多重基因组编辑工具限制了对其基因功能、高产机制及代谢工程的研究。为了克服这一局限，本研究开发了一种快速高效的基于CRISPR/Cas9的多重基因组编辑系统，由内源tRNA启动子驱动，可在产黄头孢霉中实现一步多位点敲除、大片段DNA删除及基因过表达。鉴于许多菌株缺乏与特定基因相关的可见表型，我们通过在强启动子下表达异源蛋白mCherry，引入了视觉可辨的红色表型。在野生型菌株中，对20个内源tRNA启动子进行了评估，并与异源构巢曲霉PgpdA和烟曲霉U6启动子进行了比较。内源tRNAVal启动子表现出最高的敲除效率（95.5%）。基于tRNA-gRNA阵列的CRISPR/Cas9系统在不使用供体DNA的情况下，在工业菌株中实现了双位点和三位点敲除，效率分别为50.0-83.3%和14.3%。这是在产黄头孢霉（尤其是工业菌株）中首次实现三位点同时敲除。利用该系统，我们以近100%的效率成功删除了一个50.7 kb的含山梨素类物质生物合成基因簇的DNA片段，并在高产菌株中实现了参与CPC生物合成的关键基因pcbAB的过表达，将CPC效价从5.59 g/L提升至6.92 g/L，提高了23.8%。总之，该基于tRNA-gRNA阵列的CRISPR/Cas9多重基因编辑系统为产黄头孢霉的功能基因组学和工业菌株改造提供了一个高效且多功能的平台。


### 核心发现


- 开发了内源tRNA启动子驱动的CRISPR/Cas9系统，实现产黄头孢霉一步多位点敲除、大片段删除和基因过表达。

- 筛选出tRNAVal启动子效率最高（95.5%），并在工业菌株中首次完成三位点同时敲除。

- 应用该系统删除50.7 kb山梨素类基因簇并过表达pcbAB，使CPC产量提升23.8%。



**关键词**：CRISPR-Cas Systems · Gene Editing · Acremonium · RNA, Transfer · RNA, Guide, CRISPR-Cas Systems · Promoter Regions, Genetic · Cephalosporins · Metabolic Engineering · Gene Knockout Techniques


---


## 17. A Streamlined and High-Osmolarity Bacillus subtilis Cell-Free Platform for Rapid Characterization of Genetic Regulatory Elements.

| 字段 | 内容 |
|---|---|
| **期刊** | ACS Synth Biol |
| **发表日期** | 2026-May-15 |
| **作者** | Delumeau Olivier, McGovern Stephen, Dervyn Etienne, Jules Matthieu |
| **原文链接** | [查看原文](https://doi.org/10.1021/acssynbio.6c00165) |
...
基于裂解液的无细胞系统（CFS）可在可控环境中模拟细胞功能，成为从基础研究到蛋白质生产及合成回路原型的通用合成生物学工具。本研究通过系统性地重新评估传统提取物制备和反应环境，建立了一种精简且稳健的枯草芽孢杆菌CFS。我们发现，常规所需的流失孵育和费力的透析步骤可被省略而不损害性能。关键进展在于反应缓冲液的重新配方：将谷氨酸钾浓度提升至400 mM以增强离子强度，并添加渗透保护剂甜菜碱，显著提高了翻译能力，sfGFP产量达到2–3 μM。与已发表的体内启动子文库数据比对显示，启动子强度的相对层级得以保持，验证了该平台可作为预测细菌生理的有效替代。我们进一步在CFS内通过磷酸化级联反应激活CcpA与HPr的复合物，实现了对含cre-box启动子的功能性阻遏，成功重现了复杂的遗传调控。最后，我们证明这种高谷氨酸框架同样有益于另一种革兰氏阳性菌——格氏乳杆菌，提示其具备广泛的宿主兼容性。综上，简化的制备流程和优化的缓冲液配方为在多种非模式细菌物种中开发CFS提供了通用工具箱。


### 核心发现


- 省略传统流失孵育和透析步骤可简化枯草芽孢杆菌CFS制备，且不影响性能。

- 优化反应缓冲液（高浓度谷氨酸钾和甜菜碱）使翻译能力显著增强，sfGFP产量达2–3 μM。

- 该平台能保留体内启动子强度层级，并重建CcpA-HPr介导的碳代谢物阻遏，高谷氨酸框架对格氏乳杆菌亦有效，表明广泛宿主兼容性。



**关键词**：Bacillus subtilis · Promoter Regions, Genetic · Osmolar Concentration · Cell-Free System · Bacterial Proteins · Gene Expression Regulation, Bacterial · Glutamic Acid · Betaine · Synthetic Biology


---


## 18. CyanOperon: An Operon Building Expansion for the CyanoGate MoClo Toolkit.

| 字段 | 内容 |
|---|---|
| **期刊** | ACS Synth Biol |
| **发表日期** | 2026-May-15 |
| **作者** | Astbury Michael J, Schiavon Osorio Alejandra A, Victoria Angelo J, McCormick Alistair J |
| **原文链接** | [查看原文](https://doi.org/10.1021/acssynbio.6c00162) |
| **DOI** | `10.1021/acssynbio.6c00162` |

### 摘要（英文）

Operons are gene clusters controlled by a single promoter that enable coordinated translation from a single mRNA. Here we describe an expansion of the CyanoGate MoClo toolkit to assemble synthetic operons. The versatile CyanOperon system includes two Level 0 acceptor vectors for building interchangeable promoter-ribosome binding site (RBS) combinations and 15 Level 1 acceptor vectors for the hierarchical assembly and expression of up to six genes within a single operon. The system also allows for operon assembly into a self-replicating vector or for chromosomal integration by homologous recombination. To showcase CyanOperon, we assembled the violacein biosynthesis pathway as an operon and demonstrated violacein production in Escherichia coli. We then constructed a 20-part RBS library to examine how spacer length between the Shine-Dalgarno sequence and start codon affects translation in E. coli and the model cyanobacterium Synechocystis sp. PCC 6803. Lastly, we compared the expression of up to three operonic fluorescent markers following chromosomal integration or from a self-replicating vector in E. coli and Synechocystis sp. PCC 6803. The CyanOperon system is publicly available and can be readily integrated with other MoClo systems to accelerate the development of standardized operon assemblies.

### 摘要（中文）

操纵子是由单个启动子控制的基因簇，能够实现从单个mRNA的协调翻译。本文描述了CyanoGate MoClo工具包的扩展——CyanOperon系统，用于组装合成操纵子。该多功能CyanOperon系统包含两个0级受体载体，用于构建可互换的启动子-核糖体结合位点（RBS）组合，以及15个1级受体载体，用于在单一操纵子内分层组装并表达多达六个基因。该系统还支持将操纵子组装到自主复制载体中，或通过同源重组进行染色体整合。为展示CyanOperon，我们组装了violacein生物合成途径操纵子，并在大肠杆菌中实现了violacein的生产。随后，我们构建了一个包含20个RBS文库，研究Shine-Dalgarno序列与起始密码子之间的间隔长度如何影响大肠杆菌和模式蓝细菌集胞藻PCC 6803中的翻译。最后，我们比较了大肠杆菌和集胞藻PCC 6803中，染色体整合或自主复制载体表达的多达三个操纵子荧光标记。CyanOperon系统已公开可用，并可轻松与其他MoClo系统集成，以加速标准化操纵子组装的开发。


### 核心发现


- 开发了CyanOperon模块化系统，实现多达六个基因的合成操纵子分层组装与表达。

- 构建RBS文库揭示了SD序列与起始密码子间距对蓝细菌和大肠杆菌翻译效率的影响。

- 验证了操纵子在自主复制载体和染色体整合下的功能，并成功应用于蓝细菌和 violacein 生产。



**关键词**：Operon · Escherichia coli · Promoter Regions, Genetic · Synechocystis · Indoles · Genetic Vectors · Ribosomes · Binding Sites · Synthetic Biology
...
合成生物学的进步为环境监测带来了变革性方法，但实际污染中多种共污染物的同时定量仍是一项重大挑战。本研究将两个基于转录因子的传感模块整合于单个质粒，并引入抗生素抗性基因以增强性能，构建了一种可同时检测四环素和红霉素的多色细菌传感器。该传感器对四环素类和大环内酯类抗生素具有高特异性，且仅需通过智能手机拍摄的细菌图像荧光颜色差异，即可视觉定量区分二元抗生素混合物。在实际水样检测中，平台展现了可靠准确性，尤其能捕获传统化学分析常忽略的生物可利用组分。这一工作建立了一种新型、可现场部署的细菌传感器，用于抗生素混合物的原位定量。


### 核心发现


- 构建了整合两种转录因子传感模块与抗性基因的单质粒多色细菌传感器，实现四环素和红霉素的同时高特异性检测。

- 通过智能手机图像荧光颜色差异，即可对二元抗生素混合物进行简单的视觉定量区分。

- 在实际水样中验证了可靠性，可检测生物可利用抗生素组分，弥补传统化学分析仅测总量的不足。



**关键词**：Anti-Bacterial Agents · Erythromycin · Tetracycline · Biosensing Techniques · Plasmids · Environmental Monitoring · Water Pollutants, Chemical · Transcription Factors · Synthetic Biology


---


## 20. Synthetic Niches Enable Coculture Bioprocessing but Are Prone To Mutational Escape.

| 字段 | 内容 |
|---|---|
| **期刊** | ACS Synth Biol |
| **发表日期** | 2026-May-15 |
| **作者** | Vandenbroucke Vincent, Martínez Juan Andrés, Henrion Lucas, Zicler Andrew, Telek Samuel, Josselin Laurie, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1021/acssynbio.5c00940) |
| **DOI** | `10.1021/acssynbio.5c00940` |
...
稳定微生物共培养是生物生产的一项核心挑战。虽然菌株间分工能提高效率，但常导致种群随时间失去稳定性。交叉喂养、群体感应和毒素-抗毒素模块等经典策略依赖复杂的生态互作，难以在生物加工条件下预测或维持。我们首次在连续生物反应器中实现了基于合成生态位的共培养，采用将生长与特定表型状态耦联的遗传拨动开关。我们构建了两种营养缺陷型菌株 TOGGLE_green 和 TOGGLE_yellow，其生长分别与 GFP 或 YFP 表达状态关联，并通过自动化响应式流式细胞术在连续生物反应器条件下评估其行为。出乎意料的是，营养缺陷压力的引入重塑了线路功能，使其不再像批次或微流控系统中报道的那样维持双稳，而是表现为单向诱导系统，并在诱导剂撤除后回复。这一涌现行为在生物反应器尺度实现了简化的单输入控制，但也暴露出长期运行的关键局限：共培养中生长受损的菌株在不到 10 代内即发生快速突变逃逸，远短于文献中通常报道的约 50 代。基于简单阻遏的常微分方程模型再现了可逆动力学，捕捉到连续培养下观察到的有效诱导行为，而长期操作中的偏差则凸显了合成控制被快速进化侵蚀。我们的发现展示了合成生态位作为可规模化共培养控制策略的潜力与局限。


### 核心发现


- 首次在连续生物反应器中实现合成生态位共培养，发现营养缺陷压力使拨动开关从双稳态变为单向诱导行为。

- 长期运行下生长受损菌株在不到10代内即发生突变逃逸，远快于文献报道的50代，成为关键限制。

- 模型复现可逆诱导动态，长期偏差表明合成控制易被快速进化侵蚀，展现策略潜力与局限性。



**关键词**：Coculture Techniques · Bioreactors · Mutation · Escherichia coli · Synthetic Biology · Quorum Sensing · Green Fluorescent Proteins


---


## 21. Engineering Rapamycin-Induced Dimerization for Control of Gene Expression in Plants.
...
植物拥有精密的基因表达调控系统，植物合成生物学旨在利用这些系统设计自然界中不存在的新功能。化学诱导系统通过实现基因表达的精确时间、空间和定量控制，为此类研究提供了宝贵工具。然而，现有的植物化学诱导系统常面临诸多挑战，例如无诱导剂存在时的高本底活性、在不受控环境中的意外激活，或由系统本身或其诱导剂引起的植物体内脱靶效应。本研究开发了一种适用于植物的雷帕霉素诱导型FKBP-FRB分裂转录因子系统。该系统将人源FRB（hFRB）结构域（人mTOR蛋白的第2025至2115位氨基酸）与DNA结合结构域融合，并将全长人源FKBP12（hFKBP12）蛋白与转录激活结构域融合。施加雷帕霉素后，hFRB与hFKBP12发生二聚化，形成功能性转录因子，结合至合成启动子，从而激活靶标基因。通过系统地表征不同DNA结合结构域、FRB重复序列和启动子架构的设计，对该系统进行了系统性优化。这些努力降低了本底活性并增强了诱导活性，使靶标基因的表达从未诱导状态到诱导状态提升了87倍，并超越了2×CaMV35S启动子驱动的组成型表达。便捷的雷帕霉素施用方法（如叶面喷洒或土壤浇灌）以及系统对纳摩尔浓度诱导剂的敏感性，进一步突显了其实用性。该系统为调控植物基因表达提供了一种稳健且精确的工具，并具有利用靶向FRB或FKBP突变体的正交配体进行扩展的潜力。


### 核心发现


- 开发了雷帕霉素诱导的FKBP-FRB分裂转录因子系统，实现植物基因表达的精确时空控制。

- 通过系统优化将本底活性最小化，诱导后表达提升87倍，超越2×CaMV35S组成型启动子水平。

- 系统对纳摩尔级雷帕霉素敏感，可通过叶面喷洒或土壤浇灌简便施用，实用性强。



**关键词**：Sirolimus · Humans · Gene Expression Regulation, Plant · Tacrolimus Binding Protein 1A · Promoter Regions, Genetic · Transcription Factors · Tacrolimus Binding Proteins · TOR Serine-Threonine Kinases · Protein Multimerization · Plants, Genetically Modified


---


## 22. PHYCUT: Scalable Multiplex CRISPR/Cas9 Editing for Genome Engineering in the Diatom Phaeodactylum tricornutum.
...
硅藻是全球重要的微藻，贡献约20%的氧气生产并展现非凡的代谢多样性。海洋硅藻三角褐指藻已成为一个有前景的合成生物学平台，用于重组蛋白的生物生产，支持类似人类的N-连接糖基化途径。然而，其α(1,3)-连接的核心岩藻糖在人体中可能具有免疫原性，因而限制了其生物制药应用。三角褐指藻高效基因组工程的一个障碍是缺乏同时进行多靶点CRISPR/Cas9编辑的稳健系统。为克服这一局限，我们开发了PHYCUT（三角褐指藻Csy4-Cas9多重工具），一种基于质粒的多功能CRISPR/Cas9系统，利用Csy4内切核糖核酸酶处理多指导RNA阵列。为突出PHYCUT的应用，我们展示了在三角褐指藻中对负责α(1,3)岩藻糖基化的三个FucT基因进行多重编辑，获得分泌蛋白岩藻糖基化水平降低的菌株。PHYCUT为硅藻提供了简便、多重化的基因组工程手段，并为人源化三角褐指藻糖基化途径以支持下一代藻类生物技术奠定了基础。


### 核心发现


- 开发了PHYCUT系统，利用Csy4内切核糖核酸酶处理多指导RNA阵列，实现硅藻高效多重基因组编辑。

- 展示了在三角褐指藻中同时编辑三个FucT基因，成功降低分泌蛋白的α(1,3)岩藻糖基化水平。

- PHYCUT为人源化三角褐指藻糖基化途径提供了简便工具，推动下一代藻类生物技术发展。



**关键词**：Diatoms · CRISPR-Cas Systems · Gene Editing · Glycosylation · Plasmids · Fucose


---


## 23. Prompt-Only Gene-Circuit Modeling with a Large Language Model Simulates the synNotch Spheroids.

| 字段 | 内容 |
|---|---|
| **期刊** | ACS Synth Biol |
| **发表日期** | 2026-May-15 |
| **作者** | Chen Kaiwen, Truong Kevin |
| **原文链接** | [查看原文](https://doi.org/10.1021/acssynbio.5c00704) |
| **DOI** | `10.1021/acssynbio.5c00704` |
...
形态发生源于基因调控回路，后者将生化信号转化为空间有序的细胞命运；然而，构建这些过程的定量模拟在技术上仍颇具挑战。在此，我们提出一种纯提示驱动流程，借助大语言模型（LLM）直接从自然语言生物学描述自动生成 CompuCell3D（CC3D）模拟。以经典的 synNotch 球体实验为基准，我们证明 LLM 生成的代码无需手动编程即可捕获接触依赖性信号、钙粘蛋白介导的粘附以及多细胞分选动力学的关键特征。基线模拟展现出预期的诱导者-响应者激活次序，但出现了碎裂为多个球体的现象，表明过早的互惠信号是一种动力学失效模式。引入累积接触持续时间阈值以触发 synNotch 激活后，恢复了稳健的单球体形成，提示持续的细胞–细胞相互作用是该模式的关键设计原则。我们的结果确立了对话式 LLM 引导作为多尺度建模的强大加速器，将 CC3D 陡峭的学习曲线简化为对话驱动的工作流。该框架使非专业人士能够在计算机上测试基因回路假说，并为快速、机制层面的发育设计原则探索提供了一条可推广的路径。


### 核心发现


- LLM 生成的代码在无手动编程下自动捕获了接触信号、粘附及多细胞分选等关键特征。

- 基线模拟意外出现多球体碎裂，揭示过早互惠信号是动力学失效模式。

- 引入累积接触时间阈值恢复单球体形成，表明持续细胞接触是核心设计原则。



**关键词**：Gene Regulatory Networks · Spheroids, Cellular · Humans · Computer Simulation · Signal Transduction · Cadherins · Large Language Models


---


## 24. Elucidating the Metabolic Landscape of p-Coumaric Acid to Deepen Biological Implication and Enhance Biomanufacturing.

| 字段 | 内容 |
|---|---|
| **期刊** | ACS Synth Biol |
| **发表日期** | 2026-May-15 |
| **作者** | Zhou Hongyi, Wang Dehang, He Hailin, Shi Zhihui, Chen Hao, Qi Xuan, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1021/acssynbio.5c00758) |
| **DOI** | `10.1021/acssynbio.5c00758` |
...
对香豆酸（p-CA）是植物木质素和多酚类物质的通用前体。全面理解 p-CA 代谢对于阐明生物进化、推进多酚生物制造以及开发木质纤维素生物精炼至关重要。然而，由于研究有限且缺乏通用预测方法，p-CA 代谢特征仍不明确。在此，我们系统性地阐明了工业酵母解脂耶氏酵母（Yarrowia lipolytica）中的 p-CA 代谢。通过转录组分析和基于反应的预测方法，我们鉴定出关键降解基因，其集体缺失使 p-CA 降解减少 81.6%，产量提高 2.51 倍。对 109 个物种的比较分析显示，p-CA 代谢潜力广泛存在，但呈现显著的栖息地依赖性差异。本研究为理解和工程改造 p-CA 代谢建立了系统框架，为优化微生物细胞工厂和更广泛探索代谢途径提供了洞见。


### 核心发现


- 在解脂耶氏酵母中鉴定p-CA关键降解基因，敲除后降解减少81.6%并增产2.51倍。

- 通过转录组分析和反应预测方法，系统解析了p-CA代谢途径并鉴定关键基因。

- 比较109个物种发现p-CA代谢潜力广泛存在，但存在显著的栖息地依赖性差异。



**关键词**：Propionates · Yarrowia · Metabolic Engineering · Coumaric Acids · Lignin · Metabolic Networks and Pathways


---


## 25. CRISPR/Cas9 and Cytidine Base-Editing Systems for Efficient Genome Engineering in Oleaginous Rhodococcus.

| 字段 | 内容 |
|---|---|
| **期刊** | ACS Synth Biol |
| **发表日期** | 2026-May-15 |
| **作者** | Duhalde Martín A, Martino Román A, Smania Andrea, Alvarez Héctor M, Hernández Martín A |
| **原文链接** | [查看原文](https://doi.org/10.1021/acssynbio.6c00019) |
...
产油红球菌菌株可降解多种化合物，并合成和积累大量胞内脂质，这使其成为生物技术应用中有吸引力的平台。然而，红球菌中的高效基因组编辑仍面临挑战，需要新的分子工具来推进对其代谢、应激反应及细胞生理学的理解。本文描述了红球菌中的非同源末端连接（NHEJ）系统，并建立了一套基于CRISPR/Cas9核酸酶的高效基因组编辑系统，该系统利用NHEJ修复机制，无需供体DNA模板。此外，我们首次在产油红球菌菌株中实现了胞苷碱基编辑（CBE）系统，能够进行精确的单核苷酸置换（C•G → T•A）。两种策略均依托于双质粒CRISPR平台，从而产生了两套质粒组合：pTipCas9/pCA71sgRNA 和 pTipBE/pCA71sgRNA。这些系统在天然基因组靶点上实现了高效率的INDEL形成和C•G → T•A碱基转换，效率分别达到70-80%和75-85%。最后，针对CBE系统，我们引入了Csy4介导的sgRNA加工模块，以支持多重基因组编辑，实现多个基因位点的同时修饰。综合而言，这些工具性能超越基于重组的策略，将有助于研究复杂代谢途径以及开发红球菌生物技术应用的遗传策略，并可用于其他放线菌。


### 核心发现


- 揭示红球菌NHEJ系统，建立CRISPR/Cas9无供体编辑方法，INDEL效率达70-80%。

- 首次在产油红球菌实现胞苷碱基编辑，精确C•G→T•A转换效率达75-85%。

- 利用Csy4介导sgRNA加工实现CBE多重基因组编辑，可同时修饰多个位点。



**关键词**：Rhodococcus · CRISPR-Cas Systems · Gene Editing · Cytidine · Plasmids · Genome, Bacterial · DNA End-Joining Repair


---


## 26. Polyvalent Guide RNAs Enhance the CRISPR-Mediated Suppression of a Human Coronavirus.

| 字段 | 内容 |
|---|---|
| **期刊** | ACS Synth Biol |
| **发表日期** | 2026-May-15 |
| **作者** | Magdy Merna, Tinker-Kulberg Rachel, Josephs Eric A |
| **原文链接** | [查看原文](https://doi.org/10.1021/acssynbio.5c00574) |
...
CRISPR酶已成为哺乳动物细胞靶向基因编辑的重要工具，也可用于特异性靶向和清除病毒核酸以治疗感染。通过递送靶向RNA的CRISPR效应蛋白Cas13和识别单链RNA病毒基因组序列的引导RNA（gRNA）即可实现。此前，我们假设通过设计单个gRNA可同时靶向多个相似但不完全相同的病毒序列（即“多价”引导RNA或pgRNA），gRNA的多价性将克服因gRNA与病毒靶标错配造成的缺陷，从而增强Cas13的抗病毒效力并防止突变逃逸。我们随后在植物病毒感染模型中证明了这一策略的有效性，但不确定该策略是否也适用于人类病毒。本研究针对人类冠状病毒229E（hCoV-229E）内的多个RNA序列设计了pgRNA，并与Cas13一起递送至感染hCoV-229E的人肺上皮细胞系。使用pgRNA的CRISPR抗病毒处理以CRISPR依赖的方式表现出显著的病毒抑制效果——优于单一靶标gRNA，即使同时使用多种单一靶标gRNA时也是如此。此外，与单一靶标gRNA相比，使用相同pgRNA的Cas13显示出更低的“附带”或非特异性RNase活性，这暗示其作为治疗剂可能具有更高的特异性和安全性。我们的发现展示了一条计算与实验相结合的流程，通过非常规的gRNA设计策略生成并验证pgRNA，从而更有效地利用CRISPR抗病毒生物技术靶向人类病毒。


### 核心发现


- 多价引导RNA（pgRNA）可同时靶向多种相似的病毒RNA序列，比单一靶标gRNA更有效抑制人类冠状病毒hCoV-229E。

- pgRNA使Cas13的旁切活性显著降低，提示其作为抗病毒治疗剂具有更高的特异性和安全性。

- 建立了非常规gRNA设计的计算与实验流程，能更有效地利用CRISPR抗病毒技术靶向人类病毒。



**关键词**：Humans · RNA, Guide, CRISPR-Cas Systems · CRISPR-Cas Systems · Coronavirus 229E, Human · Gene Editing · RNA, Viral · Clustered Regularly Interspaced Short Palindromic Repeats · HEK293 Cells


---


## 27. A tissue-scale strategy for sensing threats in barrier organs.
...
屏障器官依赖有限的模式识别受体（PRRs）来检测多种免疫原性挑战，但这些器官如何评估威胁并调整免疫应答以平衡宿主保护与附带损伤仍不清楚。本研究通过单分子成像和空间转录组学，表征了肺中不同细胞类型对流感感染感知的概率，发现了细胞类型特异的威胁感知概率：上皮细胞最低，基质细胞最高。这种差异源自核酸感应PRRs在肺组织区间中的空间梯度表达，是屏障器官广泛采用的策略。在体内选择性提高肺上皮细胞中视黄酸诱导基因I（RIG-I）的水平，会在非感染性挑战下加剧组织损伤，揭示了降低屏障上皮敏感性的重要性。这些结果共同突显了一种空间分层策略，即忍受局限于上皮的威胁，并随着威胁向组织深层侵袭，启动逐步增强的免疫应答。


### 核心发现


- 肺上皮细胞对流感感染的感知概率最低，而基质细胞最高，呈现细胞类型特异的威胁感知差异。

- 感知概率的差异源于核酸感应模式识别受体在肺组织中的空间梯度表达，为屏障器官的通用策略。

- 提高上皮细胞RIG-I表达加剧非感染性组织损伤，表明屏障上皮的低敏感性对防止过度损伤至关重要。
...
## 28. Using rotational integration of oblique interferometric scattering to track axial spatiotemporal responses of tubular membrane protrusions.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-May-14 |
| **作者** | Liu Junyu, Lim Yean Jin, Herrmann David, Timpson Paul, Phan Tri Giang, Delghingaro-Augusto Viviane, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-72302-1) |
...
管状膜突起的3D时空动态对于理解吞噬作用、细胞通讯和力学生物学至关重要。尽管共聚焦显微镜广泛用于膜突起研究，但其固有的低轴向分辨率和较高光毒性限制了管状突起沿轴向的活体成像。我们发现旋转斜向干涉散射（RO-iSCAT）利用离轴照明，使离焦iSCAT信号相比聚焦信号产生更大的横向偏移。这一现象能产生无散斑的宽场干涉散射信号，信噪比提高10倍，无需任何背景扣除。RO-iSCAT能够对多种纳米颗粒和管状膜突起进行实时、无标记成像，从而在多种细胞类型和复杂共培养体系中提供管状膜突起的生物物理特征分析。RO-iSCAT能够以数十至数百纳米的位移快速定量解析膜突起的轴向时空复杂性，无需进行三维体积成像。


### 核心发现


- RO-iSCAT利用离轴照明使离焦信号产生更大横向偏移，生成高信噪比无散斑干涉信号。

- RO-iSCAT实现10倍信噪比提升、无需背景扣除的实时无标记成像。

- RO-iSCAT无需三维成像即可解析管状膜突起纳米级轴向时空动态。



**关键词**：Interferometry · Humans · Microscopy, Confocal · Animals · Imaging, Three-Dimensional · Cell Membrane · Cell Surface Extensions · Spatio-Temporal Analysis


---


## 29. Multiplex networks-based directed graph neural network for cancer driver gene identification.

| 字段 | 内容 |
|---|---|
| **期刊** | PLoS Comput Biol |
| **发表日期** | 2026-May |
| **作者** | Li Pingting, Xie Minzhu |
| **原文链接** | [查看原文](https://doi.org/10.1371/journal.pcbi.1014275) |
...
识别癌症驱动基因在精准肿瘤学中至关重要。大多数现有方法依赖单一相互作用网络来捕捉基因关系。然而，随着多组学和生物网络数据的日益丰富，整合多层网络能够更全面地呈现基因间复杂且有方向的调控相互作用。此外，已验证的癌症驱动基因数量与大量未标记基因相比仍然较少，导致标签稀缺和类别不平衡。为应对这些局限，我们提出了一种基于多层网络的有向图神经网络（MNDGNN）。该模型通过有向图卷积，在多组学数据的多层网络上学习基因表征，并融合了邻居多样性和度多样性。我们还引入了结合正样本增强与负样本推断的数据增强方法，以缓解标签稀缺问题。实验结果表明，所提方法相比现有先进方法具有更好的预测性能和鲁棒性。预测的癌症驱动基因显著富集于癌症相关通路，并与已知的癌症驱动基因存在广泛相互作用，为癌症驱动基因发现和治疗策略设计提供了新的视角。


### 核心发现


- 提出MNDGNN模型，通过多层有向图卷积整合邻居多样性和度多样性，提升基因表征学习。

- 结合正样本增强与负样本推断的数据增强策略，有效缓解标签稀缺和类别不平衡问题。

- 预测的癌症驱动基因显著富集于癌症通路并与已知驱动基因广泛互作，性能优于现有方法。



**关键词**：Humans · Neoplasms · Neural Networks, Computer · Computational Biology · Gene Regulatory Networks · Algorithms · Graph Neural Networks


---


## 30. ProteinNetworkSight: a user-friendly platform for transforming co-expression patterns into actionable therapeutic insights through interactive network visualization.
...
ProteinNetworkSight (https://proteinnetworksight.jce.ac) 解决了现代系统生物学中一个普遍的瓶颈：无法同时分析由定量技术（如机器学习、深度学习或统计建模）产生的多个特征向量，这些技术为数据集提供了一系列模式。从PCA到深度自编码器，现代计算管道很少识别单一基因列表，而是提取代表不同患者亚群或独立组分的一系列独特模式。当前的网络服务器难以应对这种高维现实，迫使研究人员逐一分析向量或将其合并为静态共识，从而抹去了独特的拓扑特征。ProteinNetworkSight引入了一种新颖的、支持同时多模式分析的网络服务器架构。与标准工具不同，该服务器接受多列表格，并在一次运行中将每个输入向量转化为离散的交互式蛋白质间相互作用网络。这种批量向量架构允许并排可视化不同拓扑，保留了疾病异质性。此外，该服务器通过计算复合扰动评分来识别每个模式特有的关键蛋白质节点，从而实现规范性干预。通过将FDA批准的抗癌药物映射到这些靶点，它有助于快速设计个性化的组合疗法。


### 核心发现


- 开发了多向量同步批处理架构，一次将多个模式转化为交互式蛋白质互作网络。

- 通过并排可视化保留不同模式的拓扑特征，突破单向量或静态合并的分析瓶颈。

- 引入复合扰动评分识别模式特异性靶点，并映射FDA抗癌药物以设计个性化联合疗法。
...
## 31. Engineering B cells to express fully customizable antibodies with enhanced Fc functions.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-May-13 |
| **作者** | Huang Chun, Mathur Atishay, Chang Chan-Hua, Huang Xiaoli, Chen Hsu-Yu, Davis Zachary B, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-72991-8) |
| **DOI** | `10.1038/s41467-026-72991-8` |
...
通过免疫球蛋白重链基因座（IGH）恒定区的基因组编辑，可将B细胞重编程为表达含定制抗原识别域的仅重链抗体（HCAbs）。这些工程化细胞同时表达膜表面B细胞受体（BCR）和分泌型抗体，并能对抗原产生应答。本研究进一步扩展该方法，通过选择IGH内的替代编辑位点，实现对重链恒定区（Fc）的定制，产生具有增强效应功能或延长半衰期突变的HCAbs。我们还引入突变以强制形成专性HCAb同源二聚体，防止与内源抗体链发生不需要的配对。最后，我们证明可在HCAb C末端容纳额外结构域，并使其在分泌型抗体中优先表达。这些结果展现了HCAb编辑平台的灵活性，能够充分利用B细胞特性表达完全定制的分子。


### 核心发现


- 通过选择IGH内替代编辑位点，实现对重链Fc域的定制，赋予抗体增强效应功能或延长半衰期。

- 引入特定突变迫使HCAb形成专性同源二聚体，有效避免与内源抗体链的错误配对。

- 在分泌型HCAb的C末端成功整合额外功能域，扩展了平台的可定制范围。
...
## 32. Base modifications shift tertiary structure and activity in synthetic RNA origami and a natural ribozyme.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-May-13 |
| **作者** | Yadav Deepak Kumar, Yang Haoyun, Lee Sukyeong, McRae Ewan K S |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-72891-x) |
...
修饰核苷酸碱基如5-甲基胞嘧啶（m5C）和N1-甲基假尿苷（m1Ψ）被广泛用于增强治疗性RNA的稳定性和降低免疫原性，但它们对RNA三级结构的影响仍不清楚。本研究探讨了这些修饰如何在合成RNA折纸纳米结构和天然四膜虫核酶中影响折叠和功能。利用冷冻电镜、FRET和生化分析，我们发现修饰碱基通过稳定关键连接处的替代性同轴堆积，阻碍RNA折纸的正确成熟，导致二聚化。在核酶中，修饰以温度依赖的方式改变了开放与闭合构象之间的平衡，从而改变了催化活性。这些效应主要源于碱基堆积能量的变化，而非碱基配对。我们的发现揭示碱基修饰重塑了RNA折叠景观和结构-功能关系，强调了在设计用于合成生物学和治疗应用的修饰RNA时需考虑其结构后果。


### 核心发现


- 修饰碱基通过稳定替代性同轴堆积促进RNA折纸二聚化，阻碍正确折叠。

- 修饰改变四膜虫核酶的构象平衡，以温度依赖方式调节催化活性。

- 修饰对RNA结构的影响主要来自碱基堆积能而非碱基配对的改变。
...
## 33. A multi-frequency whole-brain neural mass model with homeostatic feedback inhibition.

| 字段 | 内容 |
|---|---|
| **期刊** | PLoS Comput Biol |
| **发表日期** | 2026-May-13 |
| **作者** | Coronel-Oliveros Carlos, Lehue Fernando, Herzog Rubén, Mindlin Iván, Gatica Marilyn, Kowalczyk-Grębska Natalia, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1371/journal.pcbi.1013463) |
...
全脑模型通过动态模拟检验因果机制、识别治疗靶点，是理解健康与疾病大脑动态的重要工具。受生物物理学启发的神经群体模型广泛用于模拟脑磁图和脑电图记录，但传统模型易因过度兴奋导致S型函数过饱和，限制其捕捉丰富神经动态的能力。本研究深入刻画了一种先前提出的、配备抑制性突触可塑性（ISP）的多频率Jansen-Rit神经群体模型，旨在克服这些局限。ISP通过调整对锥体神经元的抑制反馈，将其放电率钳制在目标值附近，从而精细控制神经元放电率，防止全脑模拟中的过饱和。我们分析了不同模型参数如何调控振荡频率与连接性，并以同时拟合非快速眼动睡眠期的脑电图和功能磁共振记录作为验证。分岔分析显示，ISP拓宽了模型呈现持续振荡的参数范围；目标放电率能够调节振荡动力学，产生从较慢（δ、θ和α）到较快（β和γ）节律的不同振荡模式。高频活动源于低全局耦合、高放电率以及γ与α亚群的高比例。在多频率模型中，ISP是成功拟合跨频带脑电图功能连接的必要条件。最后，ISP控制的兴奋性降低重现了非快速眼动睡眠期的慢波活动和连接性减弱。总之，本模型与兴奋性调节脑节律和连接性的生物学证据相容，正如在睡眠、神经退行性疾病及化学神经调制中所观察到的。这一具有ISP的生物物理模型为健康与疾病的真实脑模拟提供了跳板。


### 核心发现


- ISP机制通过钳制锥体神经元放电率，防止过饱和并显著拓宽产生振荡的参数区间。

- 目标放电率可调节振荡动态，实现从δ到γ频段的多种节律，高频活动依赖低全局耦合、高放电率与高γ亚群比例。

- ISP是多频率模型拟合跨频带EEG功能连接的关键，其降低的兴奋性成功重现了NREM睡眠的慢波与低连接特征。
...
## 34. A context-free model of savings in motor learning.

| 字段 | 内容 |
|---|---|
| **期刊** | Elife |
| **发表日期** | 2026-May-13 |
| **作者** | Shahbazi Mahdiyar, Codol Olivier, Michaels Jonathan A, Gribble Paul L |
| **原文链接** | [查看原文](https://doi.org/10.7554/eLife.107423) |
...
学习将自主运动适应外部扰动（无论机械扰动还是视觉扰动）时，第二次接触的适应速度比第一次更快。这一被称为“节省”现象背后的机制尚不清楚。近期研究提出，神经控制的高维度特性使学习痕迹得以保留，从而可能促进节省。为验证这一想法，我们使用MotorNet框架训练循环神经网络来控制人类上肢的生物力学模型。在NF1（基线）、FF1（适应）、NF2（消退）和FF2（再适应）的顺序中，RNN被训练执行存在速度依赖性力场（FF）和无力场（NF）的伸手运动。在没有任何显式情境输入提示力场存在与否的情况下，RNN表现出节省的行为特征。在单元数更多的RNN中，节省现象更为稳健。我们识别出与节省相关的RNN活动成分——一种在消退后仍持续存在的预备活动偏移。沿着该偏移方向位移预备活动会增强节省，而相反方向的扰动则减少或消除了节省。这些发现揭示了运动记忆保持的潜在神经基础，该基础依赖于控制神经回路的高维度特性，且独立于认知或策略学习。


### 核心发现


- 循环神经网络在无需情境线索的条件下复现了运动学习中的节省行为特征，且网络规模越大节省效应越稳健。

- 识别出与节省相关的预备活动偏移成分，该偏移在力场消退后依然持续存在。

- 正向扰动该预备偏移可增强节省，反向扰动则削弱或消除节省，表明运动记忆保持依赖于高维神经回路中的持续性活动。



**关键词**：Upper Extremity · Humans · Biomechanical Phenomena · Recurrent Neural Networks · Models, Neurological · Models, Anatomic · Motor Skills · Learning · Movement · Reaction Time


---


## 35. Quantitative Measurement of Synthetic Repression Curves Reveals Design Challenges for Genetic Circuit Engineering under Growth Arrest.

| 字段 | 内容 |
|---|---|
| **期刊** | ACS Synth Biol |
| **发表日期** | 2026-May-13 |
| **作者** | Marken John P, Prator Mark L, Hay Bruce A, Murray Richard M |
| **原文链接** | [查看原文](https://doi.org/10.1021/acssynbio.6c00123) |
...
尽管自然环境中的微生物大部分时间处于生长停滞状态，但我们对该生理状态如何影响工程化基因电路性能知之甚少。本研究在大肠杆菌中，于活跃生长和生长停滞两种条件下，以单细胞分辨率测量了一组遗传非门文库的抑制曲线，系统探究了生长停滞对电路行为的影响。我们发现，生长停滞对电路性能的影响几乎完全由未受抑制的表达水平下降超过100倍所主导。生长停滞仅适度增加了基因表达噪声，而对抑制曲线的灵敏度和陡度影响甚微。我们的工作表明，传统基因电路设计范式目前不足以开发能在生长停滞下正常工作的电路，而仅需解决单个性能参数的下降就足以克服这一问题。本研究拓展了我们对生长停滞下细菌基因调控的理解，并为新的设计范式奠定了基础，这对于确保合成生物学系统在真实环境中安全、可靠运行至关重要。


### 核心发现


- 生长停滞导致未受抑制表达水平下降超过100倍，是影响基因电路性能的主导因素。

- 生长停滞仅适度提高了基因表达噪声，对抑制曲线灵敏度和陡度影响很小。

- 仅需解决未受抑制表达水平下降这一参数，即可使电路在生长停滞下正常工作。
...
## 36. A tunable CTD grammar governs the spatial programming of the transcription cycle.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-May-13 |
| **作者** | Zhang Qian, Hardtke Haley A, Zheng Yuanmin, Yang Haopeng, Gerber Alan, Venkat Ramani Mukesh Kumar, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-72994-5) |
| **DOI** | `10.1038/s41467-026-72994-5` |
...
RNA聚合酶II负责转录所有真核生物蛋白质编码基因，其羧基末端结构域（CTD）在整个转录周期中充当调控平台。尽管序列简单，但保守的七肽重复序列编码了一种对转录起始、延伸和终止至关重要的调控语法。通过结构分析、单分子成像和合成CTD工程化的Pol II构建体，我们确定了跨越各转录阶段的最低序列要求。我们发现，尽管SP基序是必需的磷酸化位点，但其侧翼残基可以容忍变异。相比之下，酪氨酸残基的周期性定位对于通过与中介体相互作用形成前起始复合物（PIC）是不可或缺的，这些相互作用在Ser5磷酸化时被破坏，从而触发启动子逃逸。这支持了一个模型：顺序的酪氨酸参与稳定了PIC组装，而渐进式磷酸化推动Pol II逃逸。位点特异性的Ser2/5磷酸化协调了3'末端加工因子的招募。这些发现定义了Pol II CTD的功能语法，并解释了低复杂度序列如何实现调控特异性。


### 核心发现


- SP基序是RNA聚合酶II CTD的必需磷酸化位点，但其侧翼残基序列具有可变性。

- 酪氨酸周期性定位通过与中介体互作稳定前起始复合物，Ser5磷酸化破坏该作用触发启动子逃逸。

- Ser2和Ser5位点特异性磷酸化有序招募3'端加工因子，实现转录调控特异性。
...
## 37. Discovery and heterologous reconstitution of a plant noncanonical quasi-circadian gene regulatory network.
...
昼夜节律主要由转录因子（TFs）构成的基因调控网络（GRN）产生。植物与哺乳动物生物钟GRN的比较提示其网络架构而非组分在进化上保守，因此节律生成能力并不局限于经典生物钟GRN。本研究中，我们发现尽管冷藏采后草莓的生物钟GRN无节律性，但昼夜节律依然得以维持。通过系统的双荧光素酶检测和网络分析，我们发现了一个由五个未表征的转录因子支撑的非经典GRN。我们开发了一个异源重建系统，并证明了该GRN的节律生成能力。随后通过指数富集配体系统进化结合高通量测序（SELEX-seq）、电泳迁移率变动分析（EMSA）和DNA亲和纯化（DAP）-qPCR分析，表明该GRN负责下游基因的昼夜节律表达。果实特异性干扰该GRN会导致对灰葡萄孢菌（Botrytis cinerea）的易感性增强。综上，我们的研究鉴定了一个非经典准昼夜GRN，实现了真核生物昼夜GRN的异源重建，并揭示了其在免疫调控中的功能。


### 核心发现


- 在冷藏草莓中发现由五个新转录因子组成的非经典准昼夜节律基因调控网络。

- 通过异源重建系统证明该非经典GRN能自主产生节律。

- 该GRN通过调控下游基因昼夜节律参与果实对灰霉病的免疫防御。
...
## 38. D-SPIN constructs regulatory network models from scRNA-seq that reveal organizing principles of perturbation response.

| 字段 | 内容 |
|---|---|
| **期刊** | Cell |
| **发表日期** | 2026-May-12 |
| **作者** | Jiang Jialong, Chen Sisi, Tsou Tiffany, McGinnis Christopher S, Khazaei Tahmineh, Zhu Qin, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.cell.2026.04.028) |
...
基因调控网络根据信号和环境条件调节基因组表达。重建此类网络可揭示细胞维持稳态和执行状态转换的控制原理。本文提出计算框架D-SPIN（维度可扩展单细胞扰动整合网络），能从数千种扰动条件下的单细胞mRNA-seq数据中推断出机制可解释的生成式基因调控网络模型。这些模型解释了扰动如何通过重构潜在调控互作来调节细胞状态比例。利用大型Perturb-seq和药物反应数据集，D-SPIN模型揭示了细胞命运决定的关键调控因子及在基因敲低和药物处理下远端细胞通路的协同，阐明了免疫调节药物组合如何通过加性招募基因表达程序诱导组合细胞状态，并模拟了未观测药物剂量组合下免疫细胞群体结构的变化。D-SPIN为揭示细胞信息处理与生理控制原理提供了计算框架。


### 核心发现


- D-SPIN框架从大规模单细胞扰动数据中推断出可解释的基因调控网络生成模型。

- 模型揭示扰动通过重配置调控网络调节细胞状态，发现细胞命运关键调节因子及通路协调。

- 阐明药物组合通过叠加基因表达程序诱导组合状态，并模拟未观测剂量下免疫细胞群体变化。
...
## 39. Galvanin (TMEM154) is an electric-field sensor for directed cell migration.

| 字段 | 内容 |
|---|---|
| **期刊** | Cell |
| **发表日期** | 2026-May-12 |
| **作者** | Belliveau Nathan M, Footer Matthew J, Platenkamp Amy, Rodriguez Celeste, Kim Heonsu, Prinz Christopher K, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.cell.2026.04.026) |
...
免疫细胞和上皮细胞的定向迁移对于快速响应组织损伤或感染至关重要。由跨上皮电位破坏产生的内源性电场被认为能引导细胞到达伤口部位，但单个细胞如何检测这些电信号仍不清楚。我们鉴定了Galvanin (TMEM154)，一种功能表征很少的单次跨膜蛋白，是电场引导快速移动细胞迁移所必需的。表达Galvanin就足以使原本不响应的上皮细胞获得电场引导的迁移能力。在电场暴露后，Galvanin迅速重新定位到细胞的阳极侧；在人中性粒细胞中，这种重新定位立即伴随细胞突起和回缩空间模式的变化。这些数据表明，Galvanin作为电场的直接传感器，将细胞电环境的空间信息传递给胞内迁移装置，从而支持定向细胞迁移。


### 核心发现


- 鉴定出Galvanin (TMEM154)是电场引导快速细胞迁移所必需的传感器。

- 表达Galvanin足以赋予非响应细胞电场引导的定向迁移能力。

- 电场下Galvanin向阳极侧重新定位，立即改变细胞突起与回缩的模式。
...
## 40. A pseudo-dynamic paradigm for reprogramming domain-wall-lattices in architected solids.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-May-12 |
| **作者** | Zhang Yafei, Zhou Yuan, Wen Zhixuan, Liu Mingchao, Chen Chang Qing |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-73096-y) |
| **DOI** | `10.1038/s41467-026-73096-y` |
...
从文艺复兴时期的垂褶到组织形态发生，模式形成体现了几何与约束如何产生复杂结构。在软物质和结构材料中，折痕、扭结与畴壁等图案作为序参量纹理，介导结构转变。然而，对这些图案进行确定性且可重编程的控制仍是一个核心挑战：传统基于几何的策略将功能固化到结构中，导致变形模式对缺陷敏感且难以重新配置。在此，我们引入一种伪动态映射，将静态变形场解释为虚构粒子在设计的能量景观中演化的轨迹。该范式提供了一种正向设计策略：通过重塑势能对称性和分岔结构，在单一的无缺陷超材料中仅通过均匀加载即可实现多种可重编程的孤子型畴壁晶格。我们展示了由可调分岔景观控制的这些图案的启动、调制、反转、熔化和湮灭。预测通过仿真和实验得到验证，并最终实现了一种通过畴壁比特编码数字信息的机械显示器。该方法将非线性场论与实际的图案重编程联系起来，为结构材料和自适应材料的可编程设计提供了通用途径。


### 核心发现


- 提出伪动态映射范式，将静态变形场视为虚构粒子在能量景观中的轨迹，实现正向设计。

- 在单一无缺陷超材料中，通过均匀加载可重编程地产生多种孤子型畴壁晶格。

- 实验展示畴壁图案的可控演变（启动、调制、反转、熔化和湮灭），并构建了畴壁比特机械显示器。
...
## 41. Resonant microtaper leaky-mode computational spectropolarimetry with tens of femtometers spectral resolution and full Stokes measurement.

| 字段 | 内容 |
|---|---|
| **期刊** | Nat Commun |
| **发表日期** | 2026-May-12 |
| **作者** | Wan Yangyang, Zhou Qianyu, Ma Lin, Fan Xinyu, He Zuyuan |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41467-026-72313-y) |
| **DOI** | `10.1038/s41467-026-72313-y` |

### 摘要（英文）

Emerging computational measurement techniques for acquiring multi-dimensional optical field information, such as spectrum and polarization, are rapidly advancing and offer promising solutions for realizing high-performance miniature systems. The performance of these computational measurement approaches is critically influenced by the choice of random media, yet a general framework for evaluating different implementations remains absent. Here, we propose a general analytical model for computational measurement systems and reveal that the system resolution is fundamentally determined by the maximum optical path difference (OPD) permitted within the random medium. Building on this theoretical foundation, we present a resonant leaky-mode (RLM) spectropolarimeter that achieves a record high resolution-footprint-product metric. The RLM spectropolarimeter leverages the complex coupling between leaky modes in a tapered coreless optical fiber and whispering-gallery modes of microsphere to significantly enhance the maximum OPD within a compact footprint. We simultaneously achieve an ultrahigh spectral resolution of 20 fm, a spectral measurement bandwidth of 150 nm, and full-Stokes polarization measurement with a mean squared error of 4.732 × 10-6 on a sub-square-millimeter random medium. The proposed theoretical model clarifies the key factors governing the performance of computational measurement systems based on random media and may inspire the design of advanced computational measurement systems for optical field. The demonstrated RLM spectropolarimeter offers a potential approach for highly integrated, high-performance multi-dimensional optical field measurement.

### 摘要（中文）

新兴的计算测量技术可获取光谱和偏振等多维光场信息，正迅速推动高性能微型系统的发展。这些计算测量方法的性能受随机介质选择的关键影响，但目前尚缺乏评估不同实现方案的通用框架。本文提出适用于计算测量系统的通用解析模型，揭示系统分辨率本质上由随机介质中允许的最大光程差决定。基于这一理论基础，我们展示了一种谐振泄漏模式光谱偏振仪，其分辨率-尺寸乘积指标创下纪录。该光谱偏振仪利用无芯锥形光纤中的泄漏模式与微球回音壁模式之间的复杂耦合，显著增大了紧凑尺寸下的最大光程差。在亚平方毫米的随机介质上，我们同时实现了20 fm的超高光谱分辨率、150 nm的光谱测量带宽以及均方误差为4.732 × 10⁻⁶的全斯托克斯偏振测量。所提出的理论模型阐明了基于随机介质的计算测量系统性能的关键决定因素，可能启发先进的光场计算测量系统设计。展示的谐振泄漏模式光谱偏振仪为高集成、高性能多维光场测量提供了一种潜在方法。


### 核心发现


- 提出通用解析模型，指出计算测量系统分辨率由随机介质中最大光程差决定。

- 设计谐振泄漏模式光谱偏振仪，利用泄漏模式与回音壁模式耦合大幅提升最大光程差。

- 在亚平方毫米尺寸上同时实现20 fm光谱分辨率、150 nm带宽和全斯托克斯偏振测量，均方误差4.732×10⁻⁶。
...
## 42. Network structure induced bias in estimates of intrinsic generation times.

| 字段 | 内容 |
|---|---|
| **期刊** | PLoS Comput Biol |
| **发表日期** | 2026-May-12 |
| **作者** | Kollepara Pratyush K, Poletto Chiara, Miller Joel C |
| **原文链接** | [查看原文](https://doi.org/10.1371/journal.pcbi.1014239) |
...
生成间隔（感染者从自身感染到引起新感染所需的时间）是疾病爆发期间必须估算的关键量，它能揭示疫情蔓延的时间尺度，并可根据发病率数据计算基本再生数（量化传播潜力）。在没有干预和行为改变时，内在生成间隔保持稳定，但成功感染事件的“实现生成间隔”会随疫情动态和定义前向或后向间隔的数据聚合方式而变化。对于均匀混合人群，这些时变分布已有较好理解，可用于推断内在生成间隔分布；而对于异质人群，现有先进方法依赖昂贵的网络或个体仿真。我们采用基于边的隔室建模框架，推导出在异质接触网络上传播的马氏SIR感染生成时间分布的精确公式，并通过随机爆发模拟验证，将后向和前向生成间隔与内在生成间隔关联起来。最后，利用这些结果揭示了从实现生成间隔估计内在生成时间时一些先前未探索的偏差，这些偏差可能由模型中对网络结构（尤其是接触的时间结构）的错误假设导致。


### 核心发现


- 推导出异质接触网络上马氏SIR传播的生成时间分布的精确解析公式。

- 建立了前向/后向生成间隔与内在生成间隔的定量数学关系。

- 发现网络结构（尤其是接触时间结构）假设错误会导致内在生成间隔估计的系统性偏差。
...
## 43. Engineered Escherichia coli Strains for Flavoprotein Research: From Production of Apoproteins to Incorporation of Flavin Derivatives.
...
黄素蛋白参与广泛的生物过程，含有一个或多个黄素辅因子，以非共价或共价方式结合，催化多种氧化还原反应。在常用表达菌株中，黄素蛋白通常以全蛋白（结合黄素）形式产生。能够产生其脱辅基形式（不含黄素）将有助于结构与机理研究，并允许引入类黄素辅因子。为此，我们利用CRISPR/Cas9系统构建了一株大肠杆菌，该菌因缺失ribB基因而不能合成经典FMN和FAD辅因子。该核黄素营养缺陷型菌株可产生FMN和FAD依赖型黄素蛋白的脱辅基蛋白，也能通过添加核黄素衍生物掺入黄素衍生物。同时，通过基因组整合构建了一株从T7启动子表达FAD合成酶、从lac启动子表达黄素转移酶的菌株，便于应用黄素标签与黄素固定方法，使靶蛋白携带共价连接的FMN，省去了多质粒的必要。还构建了兼具核黄素营养缺陷和黄素转移酶活性的菌株，非常适合生成共价连接黄素衍生物的黄素蛋白。这些新构建的大肠杆菌BL21-AI衍生株为黄素蛋白的生产、研究和应用提供了有力工具。


### 核心发现


- 构建ribB缺失核黄素营养缺陷型大肠杆菌，可生产脱辅基黄素蛋白并掺入黄素衍生物。

- 构建基因组整合表达FAD合成酶和黄素转移酶的菌株，实现黄素标签/固定，无需多质粒生成共价FMN蛋白。

- 组合营养缺陷与转移酶活性的菌株可高效生成共价连接黄素衍生物的黄素蛋白。
...
## 44. RegVelo: Gene-regulatory-informed dynamics of single cells.
...
细胞命运转变由调控回路驱动，但RNA速度模型在建模细胞动态时未明确考虑基因调控相互作用，限制了机制洞察。相反，基因调控网络推断方法大多忽略生物系统的动态本质。为克服这一概念脱节，我们提出RegVelo，一个自下而上、可操作、可解释的深度学习框架，联合建模剪接动力学与基因调控相互作用。在多种生物系统中，RegVelo对终端状态、基因相互作用和扰动模拟提供了可靠的预测能力。通过将RegVelo应用于斑马鱼神经嵴发育，利用全长Smart-seq3以及共享的基因表达和染色质可及性测量，我们描绘了命运特化背后的调控程序。在计算机模拟扰动指导下，并经CRISPR-Cas9敲除和单细胞Perturb-seq验证，我们确定tfec为早期驱动因子，elf1为色素细胞命运的调节因子。RegVelo建立了连接基因调控与细胞命运决策的定量框架。


### 核心发现


- RegVelo联合建模剪接动力学和基因调控网络，实现可解释的细胞命运动态预测。

- 在斑马鱼神经嵴发育中揭示命运特化调控程序，发现tfec和elf1分别为色素细胞命运的早期驱动因子和调节因子。

- 通过计算机模拟与CRISPR验证，RegVelo提供可靠扰动模拟，连接基因调控与细胞命运决策。
...
## 45. A new Fight-or-Flight Pacemaker Mechanism via Ryanodine Receptor abundance and superclustering.

| 字段 | 内容 |
|---|---|
| **期刊** | PLoS Comput Biol |
| **发表日期** | 2026-May |
| **作者** | Ventura Subirachs Valeria, Tagirova Syevda, Maltsev Alexander V, Yang Dongmei, Lakatta Edward G, Stern Michael D, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1371/journal.pcbi.1014267) |
...
窦房结是心脏的主要起搏器。单个窦房结细胞（SANC）产生自发节律性动作电位，启动每一次心跳。SANC自律性及其受自主神经系统调节的机制，基于细胞膜分子（离子通道、交换体和泵）和肌浆网（SR）的偶联功能，后者产生节律性局部Ca释放（LCR）。虽然LCR由兰尼碱受体（RyRs）产生，但分子尺度上的RyR网络结构仍不清楚。在此，我们通过直接随机光学重建显微镜（dSTORM）对兔SANC在基础条件和β-肾上腺素受体（βAR）刺激（异丙肾上腺素处理5分钟）后的RyR进行单分子定位。RyR形成大小不等的簇，平均密度为67.7 ± 13.2 RyR/μm²（平均值±SEM，n=6个细胞）。虽然大多数簇的大小在3到32个RyR之间，但每个细胞都有少数明显更大的簇（>76 RyR），称为超级簇。βAR刺激显著增加了RyR密度至119.1 ± 22.6 RyR/μm²（8个细胞，p≤0.05），并产生了更多的超级簇。我们的SANC数值模型显示，在任何条件下，超级簇化通过创建Ca释放热点，引发更大的LCR，从而显著缩短动作电位周期长度（APCL）。在基础状态下，增加RyR密度延长了APCL，但在βAR刺激期间缩短了APCL。在RyR网络无变化的情况下，仅βAR对SR钙泵和离子流的刺激使APCL平均从414.9 ms缩短至284 ms。当将实际的更高RyR密度和超级簇化加入模型时，APCL进一步缩短至231.9 ms。因此，RyR网络的动态纳米尺度变化提供了一种新的强大起搏机制。


### 核心发现


- 发现RyR在窦房结细胞中形成簇和超级簇，β-肾上腺素刺激增加其密度和超级簇数量。

- 超级簇通过产生钙释放热点引发更大的局部钙释放，从而显著缩短动作电位周期。

- 动态RyR网络重塑（密度和超级簇化）为心脏起搏提供了新的调节机制，加速心率。



**关键词**：Ryanodine Receptor Calcium Release Channel · Rabbits · Animals · Sinoatrial Node · Action Potentials · Sarcoplasmic Reticulum · Computational Biology · Isoproterenol · Biological Clocks · Calcium


---


## 46. A Linear Mixed Effects Model for Evaluating Synthetic Gene Circuits.
...
合成生物学的一项重大进展是开发具有可预测布尔逻辑的合成基因电路。然而，尚无普遍接受或应用的统计检验来分析这些电路的性能。许多基本统计检验未能考虑预测的逻辑（或、与等），且许多研究完全忽略统计分析。随着合成基因电路转向高级应用，尤其是在计算、生物传感和人类健康领域，标准化评估逻辑门成功与否的统计方法至关重要。在此，我们提出应用线性混合效应模型（LMM）来分析和量化遗传布尔逻辑门的性能。首先，我们通过无监督机器学习（k均值聚类）分析了144个已发表的布尔逻辑门以揭示新兴趋势。然后，我们模拟了代表每个k均值聚类质心的数据。接着，利用线性混合效应模型估计电路的布尔特性，其中固定效应 β 表示开启与关闭状态之间的差异，随机效应为输入分组（即 +/-, +/+）。我们首先在模拟数据上验证了该模型，并使用蒙特卡洛模拟推荐了评估门性能所需的样本量。随后，我们通过一系列嵌套抑制子或门（性能人为降低）评估了固定效应点估计值 β̂ 作为衡量电路性能的整体指标。我们观察到 β̂ 与预测翻译速率之间存在关联，并利用 β̂ 指导了三输入门的设计，突显了该建模方法用于新布尔门前向设计的潜力。最后，我们将分析扩展至多输入（>2）和多输出门，强调线性混合效应模型可应用于高阶逻辑。总之，我们利用线性混合效应模型评估合成基因电路，确定固定效应点估计值 β̂ 是描述门行为的合适指标，可用于统计评估性能。


### 核心发现


- 提出线性混合效应模型，以固定效应点估计β̂作为全面量化基因电路布尔逻辑性能的指标。

- 通过模拟数据与蒙特卡洛方法验证模型，并确定评估逻辑门性能所需的最佳样本量。

- 发现β̂与预测翻译速率关联，可指导多输入门的前向设计，模型适用于高阶逻辑电路。
