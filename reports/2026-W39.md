# 系统/合成生物学文献周报 2026-W39

> **时间范围**：2026/09/14 ~ 2026/09/21
> **来源期刊**：Nature / Cell / Science 系列
> **收录论文**：41 篇（已翻译 36 篇）
> **生成时间**：2026-09-21 02:59 UTC

---



## 1. Using endocytosis to switch between chemoattraction and chemorepulsion.

| 字段 | 内容 |
|---|---|
| **期刊** | Biophys J |
| **发表日期** | 2026-Sep-18 |
| **作者** | Luettgen Grace K, Camley Brian A |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.bpj.2026.09.021) |
| **DOI** | `10.1016/j.bpj.2026.09.021` |
...
白细胞可被化学吸引信号引导至靶点，但这种反应较为复杂，包括朝向和远离炎症部位的引导。我们模拟了细胞如何在同一种化学信号下切换吸引与排斥。我们研究了恶性B细胞系实验，这些实验发现根据环境不同，B细胞可被CCL19等信号吸引或排斥。化学排斥的出现还取决于CCL19受体能否通过内吞作用被内化。我们构建了受体-配体结合与内化的随机模型，其中结合受体驱动细胞内信号分子的非线性前馈回路，决定细胞方向。我们重现了关键实验结果：改变CCL19浓度或抑制受体内化可切换细胞方向。我们的模型意味着细胞可通过受体内化调节并趋向信号的目标浓度。我们提出了检验这一设想的实验。


### 核心发现


- 构建随机模型，揭示结合受体通过非线性前馈回路决定细胞趋化方向。

- 模型重现实验：改变CCL19浓度或抑制受体内化可切换细胞吸引与排斥。

- 提出细胞通过受体内化调节并趋向信号的目标浓度，并设计验证实验。
...
## 2. Systems biology framework for the rational design of operational conditions for in vitro/in vivo translation of tissue models.

| 字段 | 内容 |
|---|---|
| **期刊** | Sci Adv |
| **发表日期** | 2026-Sep-18 |
| **作者** | Cadavid Jose L, Meimetis Nikolaos, Matsuzaki Tyler, Tevonian Erin N, Griffith Linda G, Lauffenburger Douglas A |
| **原文链接** | [查看原文](https://doi.org/10.1126/sciadv.aef7756) |
...
**关键词**：Humans · Systems Biology · Signal Transduction · Microphysiological Systems · Transforming Growth Factor beta · Models, Biological · Animals · Fatty Liver · Machine Learning · Algorithms


---


## 3. Universal synthesis of high-entropy alloy nanostructures with femtosecond lasers.

| 字段 | 内容 |
|---|---|
| **期刊** | Sci Adv |
| **发表日期** | 2026-Sep-18 |
| **作者** | Su Zikang, Jiang Lan, Dai Yiheng, Yuan Shilong, Zhang Xianze, Gai Haozhe, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1126/sciadv.aef9030) |
| **DOI** | `10.1126/sciadv.aef9030` |
...
高熵合金纳米结构（HEA-NSs）在先进催化和材料科学领域具有广阔前景，但缺乏在原子层面上设计的稳健且普适的合成策略。本研究利用飞秒激光制备HEA-NSs，涵盖从单原子到100 nm纳米颗粒，既可自由存在，也可负载于多种基底。超快激发能够精确控制电子动力学，瞬态形成的溶剂化电子在辐照后约100皮秒内诱导快速、非选择性的离子还原，随后在纳秒之后通过扩散控制动力学，发生随机原子成核并形成原子级分散的HEA-NSs。这些过程受时空限制效应（STCE）调控，原子扩散被限制在纳秒尺度内，从而能够动力学捕获具有严格原子级分散性的亚稳态纳米团簇。作为示例，FeCoNiRuPt在氧还原反应（ORR）中实现了2.1 W cm⁻²的峰值功率密度。飞秒激光通过控制电子动力学，为普适合成难以制备的亚稳态纳米粒子提供了稳健而有效的工具。


### 核心发现


- 飞秒激光实现从单原子到100 nm高熵合金纳米结构的普适合成，可自由或负载于多种基底。

- 溶剂化电子在辐照后约100皮秒诱导非选择性离子还原，时空限制效应实现亚稳态纳米团簇的动力学捕获。

- FeCoNiRuPt在氧还原反应中峰值功率密度达2.1 W cm⁻²。
...
## 4. A high-density CRISPR activation platform for mapping cancer dependencies and resistance pathways ex vivo and in vivo.

| 字段 | 内容 |
|---|---|
| **期刊** | Sci Adv |
| **发表日期** | 2026-Sep-18 |
| **作者** | Diepstraten Sarah T, Deng Yexuan, Potts Margaret A, Heidersbach Amy, König Christina, Dorighi Kristel M, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1126/sciadv.aec0722) |
...
CRISPR 激活（CRISPRa）能够在离体和体内应用中精准上调基因表达。然而，缺乏可扩展、高覆盖度的工具限制了小鼠模型中的全面遗传筛选。在此，我们推出 Partita，一个下一代小鼠全基因组 CRISPRa sgRNA 平台，旨在实现基因激活研究中无与伦比的效率。Partita 采用高密度靶向策略，在每个转录起始位点部署 10 条 sgRNA，并构建为五个基因类别特异性子库，以最大化转录诱导。为展示 Partita 的能力，我们进行了一系列大规模筛选：一项体外富集/耗竭筛选；在双打击淋巴瘤模型中开展全基因组 CRISPRa 筛选，以揭示对促凋亡药物（venetoclax、nutlin-3a 和 etoposide）的耐药因子；以及一项体内筛选，以鉴定 MYC 驱动的淋巴瘤发生的加速因子。每项实验均揭示了预期之中和意料之外的调控因子，且验证率很高。通过实现稳健的功能获得性筛选，Partita 开辟了功能基因组学的新途径，并扩展了在多个研究领域发现生物过程关键驱动因子的工具箱。


### 核心发现


- 开发高密度小鼠全基因组CRISPRa平台Partita，每个转录起始位点部署10条sgRNA并分五类子库。

- 全基因组CRISPRa筛选在双打击淋巴瘤模型中鉴定出对venetoclax、nutlin-3a和etoposide的耐药因子。

- 体内筛选发现MYC驱动淋巴瘤发生的加速因子，各实验验证率高且揭示预期与意外调控因子。



**关键词**：Animals · Mice · Drug Resistance, Neoplasm · CRISPR-Cas Systems · Clustered Regularly Interspaced Short Palindromic Repeats · Gene Expression Regulation, Neoplastic · Humans · Lymphoma · Proto-Oncogene Proteins c-myc · RNA, Guide, CRISPR-Cas Systems


---


## 5. Linear antibody epitope prediction using AlphaFold2.

| 字段 | 内容 |
|---|---|
| **期刊** | Elife |
| **发表日期** | 2026-Sep-18 |
| **作者** | DeRoo Jacob, Terry James S, Zhao Ning, Stasevich Timothy J, Snow Christopher, Geiss Brian J |
| **原文链接** | [查看原文](https://doi.org/10.7554/eLife.98369) |
...
确定抗体的结合表位对于理解抗体如何结合抗原并发挥分子功能至关重要。然而，虽然可以利用成熟的实验程序确定单克隆抗体的线性表位，但这些方法通常费力、耗时且昂贵。为了利用科学界可用的蛋白质结构预测算法的最新进展，我们开发了一种基于 AlphaFold2 的 localColabFold 实现的计算流程，通过预测抗体重链和轻链与源自抗原的靶肽序列之间复合物的结构来预测线性抗体表位。我们发现，这个我们称之为 PAbFold 的 AlphaFold2 流程，当把靶序列分割成小的重叠线性肽，并将抗体互补决定区移植到单链抗体片段形式中的几种不同抗体框架区上时，能够准确标记出若干众所周知的抗体靶标（HA/Myc）的已知表位序列。为了确定该流程是否能够识别没有公开结构信息的新型抗体的表位，我们使用我们的方法确定了一种靶向 SARS-CoV-2 核衣壳的新型抗体的表位，然后通过肽竞争 ELISA 实验验证了我们的计算结果。这些结果表明，我们开发的基于 AlphaFold2 的 PAbFold 流程能够仅使用抗体和靶蛋白序列在短时间内准确识别线性抗体表位。该方法的这种新兴能力对方法学细节敏感，例如肽长度、AlphaFold2 神经网络版本和多序列比对数据库。PAbFold 可在 https://github.com/jbderoo/PAbFold 获取。


### 核心发现


- 开发了基于AlphaFold2的PAbFold计算流程，仅需抗体和靶蛋白序列即可预测线性抗体表位。

- PAbFold在HA/Myc等已知靶标上准确识别出已知表位序列，验证了方法可行性。

- 对靶向SARS-CoV-2核衣壳的新型抗体预测表位，并通过肽竞争ELISA实验证实。



**关键词**：Epitopes · Prediction Algorithms · Epitope Mapping · Antibodies, Monoclonal · Algorithms · Computational Biology · Humans


---


## 6. The power of theory in the life sciences.

| 字段 | 内容 |
|---|---|
| **期刊** | Elife |
| **发表日期** | 2026-Sep-18 |
| **作者** | Fine Jacob L, Creeggan Finn, Gertsvolf Joshua, Gupta Purav, Groh Adam M R |
| **原文链接** | [查看原文](https://doi.org/10.7554/eLife.112987) |
| **DOI** | `10.7554/eLife.112987` |
...
过去二十年里，高通量生物学和基因组学的快速发展已帮助编目了不同细胞类型和条件下基因功能的诸多方面。近年来，人工智能和深度学习的进展在准确预测功能基因组学测量方面展现出巨大潜力。这些进展容易让人把实验编目和准确预测等同于对生物过程理论理解的增长——我们认为这种等同最终具有误导性。


### 核心发现


- 高通量生物学与基因组学已系统编目多种细胞类型和条件下的基因功能。

- 人工智能与深度学习在功能基因组学测量预测中表现优异。

- 将实验编目和预测能力等同于理论理解的增长具有误导性。



**关键词**：Biological Science Disciplines · Genomics · Humans · Artificial Intelligence


---


## 7. Proportional limb strengths signal an adaptive shift in arboreality in early human evolution.

| 字段 | 内容 |
|---|---|
| **期刊** | Sci Adv |
| **发表日期** | 2026-Sep-18 |
| **作者** | Carlson Kristian J, Ruff Christopher B, Burgess M Loring, Sylvester Adam D, Jashashvili Tea, Heaton Jason L, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1126/sciadv.aeh1752) |
...
**关键词**：Animals · Humans · Hominidae · Biological Evolution · Locomotion · Fossils · Extremities · Adaptation, Physiological · Biomechanical Phenomena


---


## 8. Rocking Without a Tune: Ex vivo and in vivo Responses to Sound in the Basilar Papilla of the Tokay Gecko.

| 字段 | 内容 |
|---|---|
| **期刊** | Biophys J |
| **发表日期** | 2026-Sep-17 |
| **作者** | Frost Brian L, Vázquez Yuriria, Horii Kazuhiro, Fabella Brian A, Hudspeth A J |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.bpj.2026.09.020) |
| **DOI** | `10.1016/j.bpj.2026.09.020` |

### 摘要（英文）

Like mammals, some reptiles possess sensitive and frequency-selective hearing at mid- to high-kilohertz frequencies. Both groups employ a place-frequency map within the inner ear to encode sound stimuli, but the mechanisms by which they achieve tuned nerve activation along the cochlea are believed to be distinct. To investigate the mechanical origins of auditory sensation in the reptiles, we measured sound-evoked displacement responses in the hearing organ of the tokay gecko (Gekko gecko), known as the basilar papilla. Using optical coherence tomography, we were able to resolve sub-nanometer-scale vibrations throughout the organ both in vivo and ex vivo. We saw that tuning was not present in the tissue-scale mechanics at any position within the organ; it instead exhibits an untuned rotational motion. We developed a mathematical model which predicted that this motion induces hair-bundle-stimulating fluid displacement, and is due to an anatomical asymmetry seen in many reptile species. These results are consistent with the theory that hair-bundle-level mechanics are primarily responsible for tuning in the tokay gecko cochlea, and we argue that our proposed mechanisms generalize to many other species across the Reptilia class.

### 摘要（中文）

与哺乳动物类似，一些爬行动物在中高频（几千赫兹）具有灵敏且频率选择性的听觉。两类动物都在内耳中使用位置-频率映射来编码声音刺激，但它们在耳蜗沿线实现调谐神经激活的机制被认为不同。为研究爬行动物听觉感觉的力学起源，我们测量了大壁虎（Gekko gecko）听觉器官——基乳突——中声音诱发的位移响应。利用光学相干断层扫描，我们能够在体内和离体分辨整个器官中亚纳米尺度的振动。我们发现，调谐并不存在于器官内任何位置的组织尺度力学中；相反，它表现出一种非调谐的旋转运动。我们建立了一个数学模型，预测这种运动会诱发刺激毛束的流体位移，并且源于许多爬行动物物种中可见的解剖学不对称性。这些结果与毛束层面的力学主要负责大壁虎耳蜗调谐的理论一致，并且我们认为所提出的机制可推广至爬行纲的许多其他物种。


### 核心发现


- 基乳突组织尺度力学无调谐，仅呈非调谐旋转运动。

- 数学模型表明旋转运动源于解剖不对称，可诱发刺激毛束的流体位移。

- 结果支持毛束层面力学是大壁虎耳蜗调谐主因，并可能适用于其他爬行纲物种。
...
## 9. SmartHisto: Bayesian active learning for histology images.

| 字段 | 内容 |
|---|---|
| **期刊** | PLoS Comput Biol |
| **发表日期** | 2026-Sep-17 |
| **作者** | Vijendran Sriram, Arruda Bailey, Anderson Tavis K, Eulenstein Oliver |
| **原文链接** | [查看原文](https://doi.org/10.1371/journal.pcbi.1013611) |
...
生物图像的准确高效表征对于推进系统生物学和医学研究至关重要。深度学习和图像处理的最新进展使得神经网络模型能够利用大型专家标注数据集快速加速图像分析。然而，在组织病理学中，全切片图像的尺寸使专家标注成本高昂，限制了获取足够大的标注数据集，成为开发自动化、AI驱动的图像分析流程的主要挑战。为了解决这一局限，我们提出了一种新颖的基于主动学习的框架，以交互方式训练图像分割模型。我们的方法采用贝叶斯神经网络识别未标注图像中的信息区域而非整个图像，从而使专家标注更具成本效益。我们在多个具有可变染色且固定放大倍数的基准数据集上验证了框架，证明了标注需求的大幅减少。值得注意的是，我们的方法实现了0.75的平均IoU，显著优于平均0.60的竞争方法。


### 核心发现


- 提出基于贝叶斯神经网络的主动学习框架，选择未标注图像中的信息区域进行标注，提高成本效益。

- 在可变染色固定放大倍数的多个基准数据集上验证，大幅减少标注需求。

- 该方法平均IoU达0.75，显著优于竞争方法的0.60。
...
## 10. Monte Carlo modeling of the formation and organization of ion channel clustering.

| 字段 | 内容 |
|---|---|
| **期刊** | PLoS Comput Biol |
| **发表日期** | 2026-Sep-17 |
| **作者** | Moise Nicolae, Weinberg Seth H |
| **原文链接** | [查看原文](https://doi.org/10.1371/journal.pcbi.1014791) |
...
离子通道在细胞膜上的空间组织对心脏和神经元兴奋性、细胞信号传导等许多关键生理过程具有重要影响，但调控通道聚集的机制仍不清楚。本研究提出了一个随机计算框架，通过蒙特卡洛模拟对离子通道的动态组织进行建模，纳入了膜插入、移除、通道-通道相互作用及扩散过程。该模型揭示了膜域形成的若干基本原理。在单通道系统中，我们证明相互作用能量与簇大小之间存在双相性关系，最佳聚集出现在中等相互作用强度下，表明过强的相互作用会限制通道移动性从而阻碍簇生长。在双通道系统中，我们发现同型与异型相互作用的共同作用决定了通道形成混合簇还是分离簇，当通道类型之间的同型相互作用强度不同时，会出现不对称聚集行为。三通道系统的模拟展示了产生层级聚集模式和特化域形成的涌现组织原则。这些发现提出了可检验的预测，说明通道密度、运输动力学和相互作用能量如何在生理功能和病理生理条件下共同改变离子通道的空间组织。


### 核心发现


- 单通道系统中，相互作用能量与簇大小呈双相性关系，中等强度最有利于聚集。

- 双通道系统中，同型与异型相互作用共同决定混合或分离簇，同型强度差异导致不对称聚集。

- 三通道系统涌现层级聚集模式与特化域，揭示多通道空间组织原理。
...
## 11. A noncontiguous code for RNA-guided DNA recognition at the origin of CRISPR-Cas.

| 字段 | 内容 |
|---|---|
| **期刊** | Science |
| **发表日期** | 2026-Sep-17 |
| **作者** | Yoon Peter H, Loi Kenneth J, Zhang Zeyuan Terry, Docter Trevor A, Lopez Santiago C, Langeberg Conner J, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1126/science.aei0498) |
...
CRISPR-Cas 提供 RNA 介导的适应性免疫，但其最早由 RNA 引导的效应蛋白如何起源尚不清楚。本研究报道了病毒干扰可编程重复序列（VIPR）系统的发现，该系统由一个是最早 CRISPR-Cas 效应蛋白祖先的 Vipr 蛋白和含交替 GGY/NN 基序的 VIPR RNA（vrRNA）组成。与通过连续互补配对识别靶核酸的典型向导 RNA 不同，vrRNA 通过一种非连续编码识别双链 DNA，其中可变的 NN 二核苷酸共同指定一段带缺口的靶序列。天然 vrRNA 靶标提示 VIPR 系统作用于竞争性噬菌体，我们通过将该复合物重定向用于转录抑制，展示了可编程的噬菌体防御。这些结果表明适应性免疫起源于病毒之间的古老战争，揭示了一种先前未知的序列信息编码逻辑。


### 核心发现


- 发现 VIPR 系统，由最早 CRISPR-Cas 效应蛋白的祖先 Vipr 蛋白和含 GGY/NN 交替基序的 vrRNA 组成。

- vrRNA 通过非连续编码识别双链 DNA，可变 NN 二核苷酸共同指定带缺口的靶序列。

- 证明 VIPR 系统可对抗竞争噬菌体并实现可编程防御，揭示适应性免疫可能源于病毒间古老战争。



**关键词**：CRISPR-Cas Systems · DNA · RNA, Guide, CRISPR-Cas Systems · Evolution, Molecular · Viral Proteins · RNA, Viral · Bacteriophages


---


## 12. Neurofilament transport kinetics can be explained by the activity of small numbers of independent kinesin and dynein motors.

| 字段 | 内容 |
|---|---|
| **期刊** | Biophys J |
| **发表日期** | 2026-Sep-17 |
| **作者** | Nowier Rawan M, Johnson Christopher M, Fenn J Daniel, Brown Anthony, Jung Peter |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.bpj.2026.09.016) |
| **DOI** | `10.1016/j.bpj.2026.09.016` |
...
神经丝是中间丝，为长而柔韧的蛋白质多聚体，在轴突中沿微管轨道双向运输。对神经丝的动态影像分析显示其快速间歇运动，平均运行长度与单马达的持续运动段一致，频繁停顿和反向提示相反方向马达之间快速切换。为探究此现象，我们使用驱动蛋白和动力蛋白马达的离散随机模型，模拟轴突中由独立马达团队驱动的神经丝运动。我们描述了一种无偏自动采样算法，用以确定匹配大规模数据集中神经丝停顿时间、运行时间、运行长度和速度范围所需的驱动蛋白和动力蛋白数量；该数据集以30.3毫秒分辨率采集了约37,000次运行和停顿。我们发现，神经丝多样的动力学行为可由1-2个驱动蛋白或动力蛋白马达的活性解释，这些马达通过施加在神经丝货物上的机械力间接相互作用。在我们的模拟中，相反方向马达同时接合的情况很少，表明运动行为可由驱动蛋白和动力蛋白交替活动解释，而无需相反方向马达之间的竞争。


### 核心发现


- 神经丝多样的动力学行为可由1-2个驱动蛋白或动力蛋白马达的活性解释。

- 马达通过施加在神经丝货物上的机械力间接相互作用。

- 相反方向马达同时接合罕见，运动由驱动蛋白和动力蛋白交替活动而非竞争解释。
...
## 13. Predicting cellular responses to perturbation across diverse contexts with State.

| 字段 | 内容 |
|---|---|
| **期刊** | Cell |
| **发表日期** | 2026-Sep-17 |
| **作者** | Adduri Abhinav K, Gautam Dhruv, Bevilacqua Beatrice, Naghipourfar Mohsen, Imran Alishba, Shah Rohan, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.cell.2026.07.052) |
...
虽然机器学习模型在预测扰动的转录组效应方面具有潜力，但目前难以跨细胞环境泛化。在此，我们引入 State，一种机器学习模型，在考虑实验内和实验间细胞异质性的同时预测扰动效应。State 使用单细胞基因表达数据训练，以预测跨细胞集合的扰动效应。State 将大型数据集上的效应判别能力提高了 30% 以上，并且在遗传、信号和化学扰动中识别差异表达基因的准确性相比基线显著提高。其基于 1.67 亿个细胞的观察性数据训练得到的细胞嵌入，能够在训练期间未观察到扰动的细胞环境中识别强扰动。我们还引入了 Cell-Eval，一个可用于评估未来模型的综合评估框架。总体而言，State 的性能和灵活性为规模化开发细胞状态 AI 模型奠定了基础。


### 核心发现


- State将大型数据集扰动效应判别能力提升超过30%，在遗传、信号和化学扰动中差异表达基因识别准确率显著优于基线。

- 其基于1.67亿细胞观察数据训练的细胞嵌入可在训练未见扰动的细胞环境中识别强扰动。

- 引入Cell-Eval综合评估框架，State性能与灵活性为规模化开发细胞状态AI模型奠定基础。



**关键词**：Machine Learning · Humans · Single-Cell Gene Expression Analysis · Transcriptome · Predictive Learning Models · Single-Cell Analysis · Animals · Gene Expression Profiling


---


## 14. Synthetic transcription factors designed by domain recombination enhance CAR T cell antitumor function.

| 字段 | 内容 |
|---|---|
| **期刊** | Cell |
| **发表日期** | 2026-Sep-17 |
| **作者** | Takacsi-Nagy Oliver, Kasinathan Sivakanthan, Hartman Austin, Yin Yajie, Wu Lujing, Chen Andy Y, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.cell.2026.07.054) |
...
人类蛋白质编码基因通过祖先基因结构域的重排进化而来。我们开发了一种可扩展的、受进化引导的方法，从蛋白质家族内的组成结构域组装新基因，称为DESynR（通过合成与重组工程化结构域）基因。在原代人T细胞中，DESynR激活蛋白-1（AP-1）转录因子（TFs）在体外和体内抗肿瘤试验中显著优于天然AP-1 TFs。DESynR AP-1 TFs诱导广泛的转录和表观遗传重编程，并建立非天然T细胞状态，优化耗竭、效应和细胞毒性功能以及持久性特征——有时借用不同细胞类型的基因模块。重编程主要由对已建立的AP-1结合调控元件的差异性调控驱动，而非独特结合。最后，我们筛选了DESynR红细胞转化特异性（ETS）和叉头框（FOX）TFs，以支持跨蛋白质家族的普适性。总体而言，我们证明重新配置现有蛋白质结构域可能发掘出非自然进化形成的基因，用于编程治疗相关的细胞状态。


### 核心发现


- 开发DESynR方法，通过结构域重组组装蛋白质家族新基因。

- DESynR AP-1转录因子在体内外抗肿瘤功能优于天然AP-1，建立优化效应与持久性的非天然T细胞状态。

- 重编程源于对既有AP-1结合元件的差异性调控，且方法在ETS和FOX家族中具普适性。



**关键词**：Humans · Animals · T-Lymphocytes · Protein Domains · Transcription Factor AP-1 · Mice · Transcription Factors · Receptors, Chimeric Antigen


---


## 15. A retinoic acid autoregulatory loop governing prefrontal-motor arealization.

| 字段 | 内容 |
|---|---|
| **期刊** | Nature |
| **发表日期** | 2026-Sep-16 |
| **作者** | Yang Lin, Shibata Mikihito, Park Saejeong, Liu Yuting, Salamon Iva, Liu Jia, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41586-026-11014-4) |
...
额叶包括支持复杂认知和目标导向行为的前额叶联合皮层（PFC），以及执行运动的运动皮层（MC）。沿感觉运动-联合轴建立不同的区域身份和连接，为皮层区域组织和功能提供了基本支架。维甲酸（RA）信号已成为PFC发育的关键调节因子。然而，将RA信号空间限制在发育中PFC内的机制，以及下游RA响应基因网络，仍知之甚少。在此，我们定义了发育中人类PFC中与RA相关的基因调控网络，并确定MEIS2——一种与智力障碍和自闭症谱系障碍相关的转录因子——是该网络的关键枢纽。在小鼠有丝分裂后皮层兴奋性神经元中条件性删除Meis2，导致预期的前额联合区域部分重新特化为类似运动区的分子和连接特征，突出有丝分裂后神经元在建立和维持皮层区域身份中的关键作用。伴随Meis2缺失，表达RA合成酶ALDH1A3的兴奋性神经元群体，及其RA信号本身，在发育中的内侧PFC（mPFC）中显著减少。这些发现揭示了一个保守的自调控环路，RA→MEIS2→ALDH1A3→RA，它强化PFC富集的RA梯度并组织MC-PFC轴。总之，我们的发现揭示了一种有丝分裂后机制：神经元身份的特定特征强化RA信号，从而定义前额叶和运动皮层区域的关键特征，将经典形态发生素与转录身份、神经回路形成和功能，以及潜在的神经精神疾病联系起来。


### 核心发现


- 在发育人类PFC中定义RA相关基因调控网络，识别MEIS2为关键枢纽。

- 条件删除小鼠Meis2使前额联合区域部分重塑为运动样特征，证明有丝分裂后神经元关键作用。

- 揭示RA→MEIS2→ALDH1A3→RA自调控环路，强化PFC富集RA梯度并组织MC-PFC轴。
...
## 16. Evolution and heterogeneity of lethal metastatic bladder cancer subtypes.

| 字段 | 内容 |
|---|---|
| **期刊** | Nature |
| **发表日期** | 2026-Sep-16 |
| **作者** | Itagi Pushpa, Schuster Samantha L, Arora Sonali, Persse Thomas W, Waters Jennifer A, Yang Michael, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41586-026-11035-z) |
...
## 17. Targeted genomic integration and rearrangement using prime assembly.

| 字段 | 内容 |
|---|---|
| **期刊** | Nature |
| **发表日期** | 2026-Sep-16 |
| **作者** | Levesque Sébastien, Kawashima Nozomu, Hwang Gue-Ho, Zeng Jing, Toskov Vasil, Barry Timothy, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41586-026-11024-2) |
...
## 18. Mutational constraints on RSV F and its neutralization by antibodies.

| 字段 | 内容 |
|---|---|
| **期刊** | Nature |
| **发表日期** | 2026-Sep-16 |
| **作者** | Simonich Cassandra A L, McMahon Teagan E, Juviler Gavin, Kampman Lucas, Chu Helen Y, Bloom Jesse D |
| **原文链接** | [查看原文](https://doi.org/10.1038/s41586-026-11030-4) |
...
针对呼吸道合胞病毒（RSV）F蛋白的新抗体已大幅降低婴儿住院率。然而病毒耐药性令人担忧：一种抗体因耐药株未通过临床试验，最广泛使用的抗体（nirsevimab）也已发现散发性耐药突变。本研究明确了RSV F突变如何影响抗体中和作用。我们首先提出一个生物物理模型，阐明双价IgG结合的缓冲作用与nirsevimab对B亚型较低的Fab效力相结合，使该抗体的耐药性在B亚型中比A亚型更常见。随后，我们通过假病毒深度突变扫描，安全测量了F蛋白几乎所有突变对其细胞进入功能以及nirsevimab、clesrovimab和其他几种关键抗体的IgG和Fab形式中和活性的影响。我们利用这些测量结果实现对RSV序列的实时监测以追踪抗体耐药性，并表明耐药株虽散发性出现但目前仍罕见。总之，我们的工作展示了Fab效力和表位特异性如何共同决定病毒突变对抗体中和的影响，使监测对公共卫生重要的天然RSV耐药株成为可能，并有助于指导未来开发具有抵御病毒逃逸韧性的抗体。


### 核心发现


- 建立生物物理模型，解释双价IgG结合缓冲与B亚型较低Fab效力共同促使nirsevimab耐药更常见于B亚型。

- 假病毒深度突变扫描测量F蛋白几乎所有突变对细胞进入及多种抗体中和的影响。

- 阐明Fab效力与表位特异性共同决定突变影响，使RSV耐药株监测成为可能并指导开发抗逃逸抗体。
...
## 19. Contractile to extensile transitions and mechanical adaptability enabled by activity in cytoskeletal structures.

| 字段 | 内容 |
|---|---|
| **期刊** | Biophys J |
| **发表日期** | 2026-Sep-16 |
| **作者** | Lamtyugina Alexandra, Banerjee Deb Sankar, Qiu Yuqing, Vaikuntanathan Suriyanarayanan |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.bpj.2026.09.011) |
| **DOI** | `10.1016/j.bpj.2026.09.011` |
...
细胞骨架网络结构对于决定涌现形态与动力学至关重要，特别是在应力纤维成核等过程中，细胞对力学信号作出响应。然而，目前对结构、动力学与力学响应之间联系的理解仍不清晰。在本研究中，我们考察自组装的细胞骨架结构如何响应外部力学扰动，聚焦于二维的纤丝与交联剂混合物。利用基于主体的模型并结合粗粒化热力学分析，我们揭示了分子马达活性如何使细胞骨架结构能够稳健地适应变化的力学条件。我们的模拟表明，在拉伸力作用下，自组装的活性星状体转变为束状结构，类似活细胞中从头应力纤维的形成，而预先存在的活性束则以可重复且受调控的方式进一步伸长。相比之下，被动组装体未出现这种定性形态重组，凸显了活性在力学适应中的关键作用。我们推导出一个简单关系，将主动应力近似为平均相对马达排列（一种介观结构度量）的函数，从而能够预测在存在或不存在形态重组的情况下应力特性对力学扰动的响应变化。我们的结果展示了介观结构与马达活性如何协同工作，使细胞骨架结构具备稳健的形态与力学适应能力，为理解细胞力学感知和应力纤维形成提供了见解。


### 核心发现


- 活性星状体在拉伸力下转变为束状结构，类似应力纤维从头形成。

- 被动组装体无形态重组，证明分子马达活性是力学适应的关键。

- 推导出主动应力与相对马达排列的关系，可预测力学扰动下应力特性变化。
...
## 20. A diversified consortium of engineered biosensors enables multi-target quantification.

| 字段 | 内容 |
|---|---|
| **期刊** | Cell Syst |
| **发表日期** | 2026-Sep-16 |
| **作者** | Chiang Alyssa Jasmine, Csicsery Nicholas, O'Laughlin Richard, Baumgart Leo, Stasiowski Elizabeth, Nguyen Phuc, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.cels.2026.101702) |
| **DOI** | `10.1016/j.cels.2026.101702` |
...
**关键词**：Biosensing Techniques · Escherichia coli · Microfluidics · Machine Learning


---


## 21. Quantifying protein unfolding kinetics with a high-throughput microfluidic platform.

| 字段 | 内容 |
|---|---|
| **期刊** | Cell Syst |
| **发表日期** | 2026-Sep-16 |
| **作者** | Atsavapranee Beatriz, Sunden Fanny, Herschlag Daniel, Fordyce Polly M |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.cels.2026.101681) |
...
**关键词**：Kinetics · Protein Unfolding · Protein Folding · Microfluidics · Tetrahydrofolate Dehydrogenase · High-Throughput Screening Assays · Proteolysis · Proteins


---


## 22. An optogenetic smart microscopy platform reveals signaling dynamics-dependent control over collective cell migration.
...
在细胞生物学中，光学技术可以测量细胞内部状态（生物传感器）并刺激细胞反应（光遗传学）。然而，全光学实验的设计通常是手动的：将预定的刺激模式施加于细胞，随时间测量生物传感器，并离线处理数据。在此，我们开发了 PyCLM，一个基于 Python 的套件，可在每次实验中实现对数千个细胞的闭环测量、图像分割和光遗传控制。我们展示了 PyCLM 的多种应用，包括对单细胞执行反馈控制以及向果蝇胚胎递送发育信号模式。我们比较了单细胞与组织尺度的光遗传控制对上皮细胞迁移的影响，揭示受体酪氨酸激酶活性的快波和慢波决定了组织运动方向，这与之前斑马鱼和小鼠体内观察结果一致。PyCLM 能够简便地搭建动态实验，以探究细胞和组织特性，并为在组织尺度上实时控制单细胞状态迈出了第一步。


### 核心发现


- 开发了PyCLM平台，实现每实验数千细胞的闭环测量、图像分割与光遗传控制。

- 比较单细胞与组织尺度光遗传控制，发现受体酪氨酸激酶活性的快慢波决定组织运动方向。

- 该方向与斑马鱼和小鼠体内观察一致，为组织尺度实时控制单细胞状态奠定基础。



**关键词**：Optogenetics · Animals · Cell Movement · Signal Transduction · Microscopy · Drosophila · Biosensing Techniques


---


## 23. Multimolecular proofreading overcomes the activity-fidelity trade-off.
...
准确的信号处理对细胞正常功能至关重要，并且可以通过动力学校对（kinetic proofreading，KPR）实现，其中酶经历连续状态转变和不可逆失活以实现高保真度。然而，由于工程化单分子状态转变存在困难，合成构建生物校对系统一直受到阻碍。在此，我们设计了一种蛋白质回路，将扩散和内吞结合起来，在多分子水平上实现动力学校对，而无需经典动力学校对中隐含假定的总酶守恒。模拟揭示了一个在实验上至关重要的回路活性与保真度之间的权衡，理论分析证实该权衡在所有动力学校对系统中都是根本性的。通过整合自激活和相互抑制机制，该回路在生物学上可行的参数区间内克服了这种活性-保真度权衡。我们的结果将校对方案从单酶扩展到多分子背景，并代表了一种构建高保真合成生物回路的实用且可推广的策略。本文透明同行评审过程的记录见补充信息。


### 核心发现


- 设计结合扩散与内吞的蛋白质回路，在多分子水平实现动力学校对，摆脱总酶守恒假设。

- 模拟与理论分析揭示回路活性与保真度之间的根本性权衡，普遍存在于所有动力学校对系统。

- 引入自激活与相互抑制机制，可在生物可行参数范围内克服活性-保真度权衡。



**关键词**：Endocytosis · Enzymes · Kinetics · Models, Biological · Signal Transduction


---


## 24. Quantification of beta-cell carrying capacity in prediabetes.

| 字段 | 内容 |
|---|---|
| **期刊** | PLoS Comput Biol |
| **发表日期** | 2026-Sep-15 |
| **作者** | Woller Aurore, Tamir Yuval, Bar Alon, Mayo Avi, Rein Michal, Godneva Anastasia, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1371/journal.pcbi.1014589) |
...
## 25. Synchronization properties in C. elegans: Relating behavioral circuits to structural and functional neuronal connectivity.

| 字段 | 内容 |
|---|---|
| **期刊** | PLoS Comput Biol |
| **发表日期** | 2026-Sep |
| **作者** | Sar Gourab Kumar, Patton Andrew, Towlson Emma, Davidsen Jörn |
| **原文链接** | [查看原文](https://doi.org/10.1371/journal.pcbi.1014152) |
...
**关键词**：Animals · Caenorhabditis elegans · Nerve Net · Models, Neurological · Neurons · Connectome · Locomotion · Computer Simulation · Behavior, Animal · Computational Biology


---


## 26. Cyclic tri-adenylate controls a CARF-TM effector in type II Panoptes anti-phage systems.

| 字段 | 内容 |
|---|---|
| **期刊** | PLoS Biol |
| **发表日期** | 2026-Sep |
| **作者** | Grüschow Sabine, Wotherspoon Peter, Hilton-Balfe Emma, Graham Shirley, White Malcolm F |
| **原文链接** | [查看原文](https://doi.org/10.1371/journal.pbio.3003934) |
...
**关键词**：Bacteriophages · Nucleotides, Cyclic · Bacterial Proteins · CRISPR-Cas Systems · Second Messenger Systems · Viral Proteins · Cyclic AMP · Signal Transduction


---


## 27. Comprehensive RNA velocity by modeling the cascade of gene regulation, transcription, and splicing from single-cell RNA sequencing data with TSvelo.

| 字段 | 内容 |
|---|---|
| **期刊** | Elife |
| **发表日期** | 2026-Sep-15 |
| **作者** | Li Jiachen, Wang Zhe, Shen Hong-Bin, Yuan Ye |
| **原文链接** | [查看原文](https://doi.org/10.7554/eLife.108950) |
...
RNA velocity（RNA 速率）方法通过使用单细胞 RNA 测序（scRNA-seq）数据对剪接过程进行建模，来拟合基因动态并推断细胞命运。然而，由于剪接过程时间尺度短、噪声高且数据复杂性大，现有 RNA velocity 方法通常无法精确捕捉单个基因和单细胞的复杂速度动态，导致其下游分析的可靠性和鲁棒性较差。我们提出 TSvelo，一个全面的 RNA velocity 数学框架，该框架使用高度可解释的神经常微分方程对基因调控、转录和剪接的级联过程进行建模。TSvelo 能够同时精确捕捉所有基因的转录-未剪接-剪接三维动态，推断单个细胞内基因共享的统一潜在时间，并可应用于多谱系数据集。在六个 scRNA-seq 数据集（包括两个多谱系数据集）上的实验证明了 TSvelo 的优越性。


### 核心发现


- 提出 TSvelo 框架，利用可解释神经常微分方程建模基因调控、转录与剪接级联。

- TSvelo 能同时精确捕捉所有基因的转录-未剪接-剪接三维动态，并推断单细胞统一潜在时间。

- 在六个 scRNA-seq 数据集（含两个多谱系）上验证，TSvelo 性能优于现有方法。



**关键词**：RNA Splicing · Single-Cell Analysis · Sequence Analysis, RNA · Transcription, Genetic · Gene Expression Regulation · Animals · Single-Cell Gene Expression Analysis · RNA · Mice


---


## 28. Thermodynamically consistent modeling of ATP-driven cross-bridge dynamics in muscle contraction.
...
**关键词**：Adenosine Triphosphate · Models, Biological · Thermodynamics · Muscle Contraction · Hydrolysis · Biomechanical Phenomena


---


## 29. Thermophoresis induced by photothermal shock alters reversal potential and capacitance in neurons.

| 字段 | 内容 |
|---|---|
| **期刊** | Biophys J |
| **发表日期** | 2026-Sep-15 |
| **作者** | Habibi Matin Meisam, Xiao Shulan, Jayant Krishna |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.bpj.2026.04.031) |
| **DOI** | `10.1016/j.bpj.2026.04.031` |
...
**关键词**：Neurons · Animals · Electric Capacitance · Temperature · Membrane Potentials · Lasers


---


## 30. Mutation-induced free-energy remodeling of recovery stroke and ATP hydrolysis in human cardiac β-myosin.

| 字段 | 内容 |
|---|---|
| **期刊** | Biophys J |
| **发表日期** | 2026-Sep-15 |
| **作者** | Ghanta Krishna Prasad, Roy Rakesh Kumar, Tardiff Jil C, Schwartz Steven D |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.bpj.2026.03.056) |
| **DOI** | `10.1016/j.bpj.2026.03.056` |
...
**关键词**：Humans · Hydrolysis · Adenosine Triphosphate · Mutation · Thermodynamics · Kinetics · Ventricular Myosins · Models, Molecular · Molecular Dynamics Simulation · Protein Conformation


---


## 31. Spatio-temporal multiscale cell modeling of skin wound healing.

| 字段 | 内容 |
|---|---|
| **期刊** | Biophys J |
| **发表日期** | 2026-Sep-15 |
| **作者** | Cao Youfang, Zhao Jieling, Chen Lin, DiPietro Luisa A, Liang Jie |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.bpj.2026.03.021) |
| **DOI** | `10.1016/j.bpj.2026.03.021` |
...
**关键词**：Wound Healing · Models, Biological · Skin · Humans · Transforming Growth Factor beta · Cell Movement · Extracellular Matrix · Collagen · Computer Simulation · Spatio-Temporal Analysis


---


## 32. Hook stiffness as a mechanical switch for torque regulation in the bacterial flagellar motor.

| 字段 | 内容 |
|---|---|
| **期刊** | Biophys J |
| **发表日期** | 2026-Sep-15 |
| **作者** | Das Biswajit, Xing Jianhua, Sharma Ajeet K, Ge Hao |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.bpj.2026.02.018) |
| **DOI** | `10.1016/j.bpj.2026.02.018` |
...
**关键词**：Torque · Flagella · Molecular Motor Proteins · Elasticity · Biomechanical Phenomena · Rotation · Bacterial Proteins · Models, Biological · Thermodynamics


---


## 33. Nanoscale co-condensation and stretch resistance of FUS and DNA.

| 字段 | 内容 |
|---|---|
| **期刊** | Biophys J |
| **发表日期** | 2026-Sep-15 |
| **作者** | Dong Weiqian, Chu Wen-Ting, Wang Erkang, Wang Jin |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.bpj.2026.02.012) |
| **DOI** | `10.1016/j.bpj.2026.02.012` |
...
蛋白质-DNA共凝聚体（其中蛋白质凝聚体与DNA凝聚体共定位）通过液-液相分离形成，在基因表达、癌症和发育缺陷中发挥关键作用。近期实验研究揭示了机械应力对这些共凝聚体的影响，但其动态过程和微观结构仍缺乏充分表征。利用粗粒化分子动力学模拟，我们研究了DNA拉伸条件下FUS与DNA共凝聚的纳米尺度特性。我们发现FUS与DNA对共凝聚的贡献不同。相分离和超螺旋结构导致拉伸诱导的共凝聚体解体过程中出现滞后，滞后是指在转变前持续一段时间的延迟结构响应。拉力对共凝聚的影响呈现阶梯式模式，且增大拉力可降低超螺旋水平。这些发现阐明了FUS-DNA共凝聚响应机械应力的机制，并为其在生物过程中的调控作用提供了新见解。
...
- 相分离和超螺旋结构导致拉伸解体过程中的延迟结构响应（滞后）。

- 拉力对共凝聚的作用呈阶梯式，增大拉力可降低超螺旋水平。



**关键词**：RNA-Binding Protein FUS · DNA · Molecular Dynamics Simulation · Stress, Mechanical · Phase Separation · Biomechanical Phenomena · Nucleic Acid Conformation · Biomolecular Condensates


---


## 34. Membrane and proteome allocation constraints in Escherichia coli models during overflow metabolism.

| 字段 | 内容 |
|---|---|
| **期刊** | Biophys J |
| **发表日期** | 2026-Sep-15 |
| **作者** | Garcia-Benitez Mauricio, Scott Matthew, Carlson Ross, Mahadevan Radhakrishnan |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.bpj.2026.01.028) |
...
有限细胞资源的分配是决定微生物代谢策略并引发复杂现象（如溢流代谢，其特征是在快速生长过程中产生包括乙酸在内的呼吸发酵副产物）的基本原理。尽管蛋白质组约束模型已成功预测大肠杆菌的溢流代谢，但它们往往忽略了与蛋白质定位相关的独特生物物理和能量成本。特别是细胞膜是一个关键且受限的区室，其中对空间和合成机器的竞争可能造成显著的代谢瓶颈。为研究这一问题，我们开发了膜相关约束通量平衡分析（MAFBA），这是一种可扩展的基因组尺度代谢模型，引入了对分配到细胞膜的蛋白质总质量的可调约束。我们的模型表明，整体和膜相关蛋白质组分配约束相互作用，提高了预测溢流代谢起始的准确性。它从机制上揭示了在高生长速率下，对有限膜分配的竞争迫使生长必需功能与呼吸能力之间进行权衡，从而导致乙酸产生。此外，MAFBA 定量解释了广泛观察到的实验现象，即表达异源膜蛋白比表达胞质蛋白施加显著更高的代谢负担。本研究将膜资源分配确立为约束细菌生理的关键因素，与整体蛋白质组限制协同作用。由此产生的 MAFBA 框架为合成生物学和代谢工程提供了强大且易用的工具，能够预测表达膜结合蛋白相关的代谢成本并指导菌株设计策略，有望在生物生产和代谢工程中应用。


### 核心发现


- 开发MAFBA模型，引入膜蛋白总量可调约束，提高溢流代谢起始预测准确性。

- 揭示高生长率下膜分配竞争迫使生长必需功能与呼吸能力权衡，导致乙酸产生。

- 定量解释异源膜蛋白表达比胞质蛋白造成更高代谢负担，为菌株设计提供工具。



**关键词**：Escherichia coli · Proteome · Models, Biological · Cell Membrane · Escherichia coli Proteins


---


## 35. Dependence of mesenchymally transitioned tumor niche fitness on cell-cell and cell-matrix adhesions.

| 字段 | 内容 |
|---|---|
| **期刊** | Biophys J |
| **发表日期** | 2026-Sep-15 |
| **作者** | Prasanna C Venkata Sai, Jolly Mohit Kumar, Bhat Ramray |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.bpj.2026.01.002) |
| **DOI** | `10.1016/j.bpj.2026.01.002` |
...
**关键词**：Cell Adhesion · Models, Biological · Extracellular Matrix · Epithelial-Mesenchymal Transition · Neoplasm Invasiveness · Tumor Microenvironment · Cell-Matrix Junctions


---


## 36. Nanoparticle-induced lipid membrane deformation influences the design of biomedicine.

| 字段 | 内容 |
|---|---|
| **期刊** | Biophys J |
| **发表日期** | 2026-Sep-15 |
| **作者** | Pincus Isaac, Qi Qin M |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.bpj.2025.10.030) |
| **DOI** | `10.1016/j.bpj.2025.10.030` |

### 摘要（英文）

Controlling the physicochemical properties of nanoparticles is important for their performance as drug carriers, pharmaceuticals, or imaging contrast agents in nanomedicine. Predictive models can accelerate experimental designs at reduced time and costs compared with a brute force approach conventionally used. However, physical principles underlying particle-cell interactions are still poorly understood due to their large size contrast, hindering the model development. In this work, we describe a model that examines the interaction between multiple particles and the membrane of a mammalian cell or an artificial vesicle, thus influencing the outcomes of surface adsorption, detachment, or uptake of particles. Compared with existing biophysical models on particle-membrane interactions accounting for membrane adhesion, stretching, and bending energies, we make several important updates that are essential to reaching quantitative agreement with existing experimental data. Particle-induced membrane tension changes are crucial to the membrane deformation even at very low surface concentrations (0.1%); we explain this surprising finding using a new length scale previously neglected. Furthermore, a multistep and nonequilibrium endocytosis mechanism is proposed in the absence of specific receptor-ligand interactions, inspired by recent experimental evidence on the dynamic regulation of membrane tension through the active transport of lipid molecules. We demonstrate the predictive power of our model in generating the adsorption isotherms and shear-induced particle detachment from cell surfaces and the size-dependent rate of particle uptake. Our research provides a framework to design tailor-made nanoparticles with controllable interaction outcomes with various cell types based on a quantitative and fundamental understanding.

### 摘要（中文）

在纳米医学中，控制纳米粒子的物理化学性质对其作为药物载体、药物或成像造影剂的性能至关重要。与通常使用的蛮力法相比，预测模型能够以更少的时间和成本加速实验设计。然而，由于粒子与细胞之间存在巨大的尺寸差异，粒子-细胞相互作用的物理原理仍未被充分理解，这阻碍了模型的发展。在本工作中，我们描述了一个模型，该模型考察多个粒子与哺乳动物细胞或人工囊泡膜之间的相互作用，从而影响粒子的表面吸附、脱离或摄取结果。与现有的考虑膜粘附、拉伸和弯曲能的粒子-膜相互作用生物物理模型相比，我们进行了几项重要更新，这些更新对于与现有实验数据达到定量一致至关重要。即使在非常低的表面浓度（0.1%）下，粒子诱导的膜张力变化对膜变形也至关重要；我们利用一个此前被忽视的新长度尺度来解释这一令人惊讶的发现。此外，受近期关于脂质分子主动转运动态调控膜张力的实验证据启发，我们提出了一种在无特定受体-配体相互作用情况下的多步非平衡内吞机制。我们展示了该模型在生成吸附等温线、剪切诱导的粒子从细胞表面脱离以及尺寸依赖的粒子摄取速率方面的预测能力。我们的研究提供了一个框架，可基于定量和基础的理解，设计具有可控的与各种细胞类型相互作用结果的定制纳米粒子。


### 核心发现


- 粒子诱导的膜张力变化对膜变形至关重要，即使表面浓度低至0.1%。

- 提出无特定受体-配体相互作用的多步非平衡内吞机制，涉及脂质主动转运调控膜张力。

- 模型可定量预测吸附等温线、剪切诱导脱离和尺寸依赖摄取速率，指导定制纳米粒子设计。



**关键词**：Nanoparticles · Cell Membrane · Models, Biological · Animals · Adsorption · Endocytosis · Nanomedicine · Membrane Lipids


---


## 37. Insights into fibrinogen mechanics under cyclic high-strain loading.

| 字段 | 内容 |
|---|---|
| **期刊** | Biophys J |
| **发表日期** | 2026-Sep-15 |
| **作者** | Ibrahim Mayar Tarek, Norouzi Sajjad, Paul Uma, Parekh Sapun H, Ren Pengyu |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.bpj.2025.11.017) |
| **DOI** | `10.1016/j.bpj.2025.11.017` |
...
纤维蛋白原在血液凝固以及缺血性脑卒中等过程中发挥核心作用，并经常暴露于机械力。本研究采用原子级分子动力学模拟，使纤维蛋白原经历三个循环的高应变加载（约17.5–27.5%）和卸载，从而探究其在循环应力下的力学响应。为捕捉牵拉方向和结构不对称性的影响，我们模拟了晶体学晶胞中存在的两个不同纤维蛋白原分子。力以相反方向施加于分子1的γ1结节和分子2的γ2结节。模拟揭示了相反的力学行为：γ1结节表现出更大的伸长且具有部分弹性，而富含β折叠的γ2结节则表现出更高的抗力，但遭受了不可逆的结构损伤。力松弛后，两个分子均保留残余应变（独立重复模拟中为6.52–15.62%），证实了部分不可逆性。各向异性简正模式分析进一步发现局部刚度降低，这与包括β折叠和α螺旋在内的二级结构元件去折叠有关。对卸载曲线进行的补充性Kelvin-Voigt建模进一步量化了这些效应，显示有效弹簧常数（Kspec）和阻尼器系数（Cspec）在各循环中逐步下降。该模型捕捉到了粘弹性弛豫动力学。总之，这些发现表明纤维蛋白原的循环响应受其内在结构异质性影响，揭示其粘弹性行为，这对血栓形成和卒中发病机制具有重要意义。


### 核心发现


- γ1结节延伸更大且部分弹性，γ2结节抗性较高却发生不可逆结构损伤。

- 卸载后残余应变为6.52–15.62%，二级结构去折叠导致局部刚度降低。

- Kelvin-Voigt模型显示有效弹簧常数和阻尼系数逐循环下降，揭示粘弹性行为。



**关键词**：Fibrinogen · Molecular Dynamics Simulation · Biomechanical Phenomena · Stress, Mechanical · Elasticity


---


## 38. Steric effects of central dogma processes on the compaction and segregation of bacterial nucleoids.

| 字段 | 内容 |
|---|---|
| **期刊** | Biophys J |
| **发表日期** | 2026-Sep-15 |
| **作者** | Chang Mu-Hung, Lavrentovich Maxim O, Carrillo Jan-Michael Y, Männik Jaan |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.bpj.2025.11.008) |
| **DOI** | `10.1016/j.bpj.2025.11.008` |
...
细菌细胞质以一种独特的无膜细胞器——拟核为特征，其中容纳染色体DNA。我们利用包含非平衡反应的粗粒化分子动力学模拟，研究与转录和翻译相关的动态过程对该细胞器结构的空间位阻效应。我们的模型捕捉全细胞尺度，并纳入核糖体和多聚核糖体的反应-扩散系统，将其非平衡动力学与DNA的排体积相互作用耦合。研究结果表明，非平衡反应增大了拟核的尺寸以及该亚细胞区域中的核糖体数量。此外，我们发现拟核大小与转录活性成正比。模型重现了利福平处理实验中观察到的拟核尺寸随时间的变化，该实验中多聚核糖体池被耗竭。进一步地，我们发现这些主动过程对于姐妹染色体的完全分离和拟核在细胞内的正确定位至关重要。总体而言，我们的研究揭示了中心法则过程对细菌拟核内部组织与定位的影响。


### 核心发现


- 非平衡反应增大拟核尺寸及该亚细胞区域中的核糖体数量。

- 拟核大小与转录活性成正比，模型重现利福平处理下拟核尺寸的动态变化。

- 这些主动过程是姐妹染色体完全分离和拟核正确定位所必需的。



**关键词**：Molecular Dynamics Simulation · Ribosomes · Diffusion · DNA, Bacterial · Chromosomes, Bacterial · Transcription, Genetic · Polyribosomes · Chromosome Segregation


---


## 39. Role of Hsp70 chaperone in client-protein folding elucidated by Markov state modeling and NMR restraint-assisted molecular dynamics simulations.

| 字段 | 内容 |
|---|---|
| **期刊** | Biophys J |
| **发表日期** | 2026-Sep-15 |
| **作者** | O'Connor Michael S, Konovalov Kirill A, Duvall Josephine L, Jang Jinoh, Lao Yichong, Cavagnero Silvia, et al. |
| **原文链接** | [查看原文](https://doi.org/10.1016/j.bpj.2025.08.022) |
...
热休克蛋白70（Hsp70）是一种分子伴侣，通过协助蛋白质折叠和防止聚集在细胞过程中发挥关键作用。在客户蛋白折叠过程中，Hsp70经历一个依赖ATP的伴侣循环，涉及柔性盖子的打开和关闭。尽管Hsp70的开盖和闭盖状态已被广泛研究，但其盖子在与客户蛋白相互作用时的具体作用仍不清楚。本研究通过Hsp70从开盖到闭盖状态的粗粒化分子动力学（MD）模拟，对柔性盖域构象系综进行采样，构建了马尔可夫状态模型。从具有不同盖子打开程度的亚稳态Hsp70构象出发，我们进行了核磁共振距离约束辅助的全原子MD模拟（显式溶剂），以研究结合在无核苷酸Hsp70上的SH3客户蛋白的折叠。全原子MD模拟在SH3结合于Hsp70和从Hsp70释放两种情况下进行，并施加核磁共振约束以引导SH3折叠。结果表明，SH3在Hsp70闭盖状态内采样构象空间后折叠更有效。进一步分析揭示，Hsp70的闭盖状态通过特定且高度保守的非极性残基与SH3相互作用，防止SH3客户蛋白在从伴侣蛋白释放后发生非天然疏水塌缩。该研究为未来实验可靶向的特定原子级相互作用提供了见解，以更好地理解Hsp70辅助蛋白折叠的分子机制。


### 核心发现


- SH3在Hsp70闭盖状态采样构象空间后折叠效率更高。

- Hsp70闭盖状态通过特定且高度保守的非极性残基与SH3相互作用。

- 该相互作用防止SH3释放后发生非天然疏水塌缩。



**关键词**：Molecular Dynamics Simulation · HSP70 Heat-Shock Proteins · Protein Folding · Markov Chains · src Homology Domains · Protein Conformation · Nuclear Magnetic Resonance, Biomolecular · Magnetic Resonance Spectroscopy


---


## 40. Clonal stochasticity in early NK cell response to mouse cytomegalovirus is generated by mature subsets of varying proliferative ability.

| 字段 | 内容 |
|---|---|
| **期刊** | Elife |
| **发表日期** | 2026-Sep-14 |
| **作者** | Wethington Darren, Ahmad Saeed, Potempa Marc, Giuliani Giuseppe, Aguilar Oscar A, Poudel Maheshwor, et al. |
| **原文链接** | [查看原文](https://doi.org/10.7554/eLife.104951) |
| **DOI** | `10.7554/eLife.104951` |
...
自然杀伤（NK）细胞通常被定义为固有免疫细胞，但实验表明，C57BL/6小鼠感染小鼠巨细胞病毒（MCMV）后，NK细胞可发生抗原特异性增殖和记忆形成，类似于适应性CD8⁺ T细胞。CD8⁺ T细胞与NK细胞的一个共同行为是克隆扩增，即单个受刺激细胞快速增殖，形成多样的细胞群体。例如，在扩增期，单细胞来源的克隆最为丰富，此时NK细胞主要为CD27⁻，T细胞主要为CD62L⁻，这些表型分别来源于前体CD27⁺和CD62L⁺细胞。本研究确定了内源性和过继转移NK细胞在MCMV感染应答扩增阶段中增殖、细胞死亡和分化的机制规则。我们发现，成熟CD27⁻ NK细胞与一个高增殖性CD27⁻Ly6C⁻成熟亚型的细胞增殖和细胞死亡之间的相互作用，以及这些过程中固有的随机波动，在调控NK细胞亚群的异质性和数量方面发挥关键作用。此外，我们估算了内源性NK细胞在稳态和MCMV感染中的成熟速率，发现MCMV仅显著提高了NK细胞的生长速率，而非分化速率。综上所述，这些结果量化了NK细胞抗原特异性扩增动力学与CD8⁺ T细胞的差异，以及单个NK细胞在扩增阶段产生NK细胞克隆异质性的独特机制。


### 核心发现


- 成熟CD27⁻ NK细胞与高增殖CD27⁻Ly6C⁻亚型的增殖、死亡互作及随机波动调控亚群异质性。

- MCMV感染仅提高NK细胞生长速率，不提高分化速率。

- 量化了NK细胞抗原特异性扩增动力学与CD8⁺ T细胞的差异及其单细胞克隆异质性机制。



**关键词**：Animals · Killer Cells, Natural · Muromegalovirus · Cell Proliferation · Mice · Mice, Inbred C57BL · Cell Differentiation · Tumor Necrosis Factor Receptor Superfamily, Member 7 · Stochastic Processes


---


## 41. Sarcomere dynamic instability and stochastic heterogeneity drive robust cardiomyocyte contraction.

| 字段 | 内容 |
|---|---|
| **期刊** | Elife |
| **发表日期** | 2026-Sep-14 |
| **作者** | Haertter Daniel, Hauke Lara, Driehorst Til, Nishi Kengo, Zimmermann Wolfram H, Schmidt Christoph F |
| **原文链接** | [查看原文](https://doi.org/10.7554/eLife.97321) |
| **DOI** | `10.7554/eLife.97321` |
...
心脏收缩由心肌细胞（CMs）的集体作用驱动...


### 核心发现


- 基质硬度增加抑制心肌细胞整体收缩，却不减弱单个肌节动力学，反致肌节拉锯式异质性增强。

- 异质性动力学非静态结构差异所致，而是源于随机波动和临界屈服力处的动态不稳定性。

- 随机异质性通过逐搏随机化屈服事件防止特定肌节受损，病理状态可能使之转为确定性持续过载。



**关键词**：Myocytes, Cardiac · Sarcomeres · Humans · Myocardial Contraction · Stochastic Processes · Induced Pluripotent Stem Cells
